...
troshx_tros_ai_TROSAiBot.prototype = {
	...
	,declareManueverAgainstOpponent: function(index) {
		...
		if(this.currentExchange != 2) {
			...
			if(troshx_tros_ai_TROSAiBot.getComboAction(this.plannedCombos[index],this.cp,opponent.cp,threatManuever,this.get_initiative(),true)) {
		...
		if(this.get_initiative()) return troshx_tros_ai_TROSAiBot.tryBestPossibleAttackOrDefBoliao(this.cp,opponent.cp,threatManuever,this.currentExchange == 2); else return troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao(this.cp,opponent.cp,threatManuever,this.currentExchange == 2);
		return false;
	}

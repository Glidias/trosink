...
troshx_tros_ai_TROSAiBot.getFleeOrDefend = function(favorable,availableCP,againstRoll,againstTN,heuristic,flags,customThreshold,secondExchange) {
	...
	} else return troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
};
troshx_tros_ai_TROSAiBot.getDesperateFleeOrDefend = function(availableCP,againstRoll,againstTN,heuristic,flags,customThreshold,secondExchange) {
	if(secondExchange == null) secondExchange = false;
	if(customThreshold == null) customThreshold = 0;
	if(flags == null) flags = 0;
	if(heuristic == null) heuristic = true;
	...
	var fleeLikelihood = troshx_util_TROSAI.getChanceToSucceedContest(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP,troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverTN,againstRoll,againstTN,1,true);
	flags |= 4;
	var resultSecond = troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCP,againstRoll,againstTN,heuristic,flags);
	...
	if(resultSecond) return resultSecond; else return troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
};
...
troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao = function(cp,cp2,threatManuever,secondExchange) {
	...
	return troshx_tros_ai_TROSAiBot.getSpecialDefWithReturnAttack(cp,cp2,threatManuever.manueverCP,threatManuever.manueverTN,0,secondExchange) || troshx_tros_ai_TROSAiBot.getFBDefense(true,cp,threatManuever.manueverCP,threatManuever.manueverTN,true,secondExchange?2:0) || troshx_tros_ai_TROSAiBot.getDesperateFleeOrDefend(cp,threatManuever.manueverCP,threatManuever.manueverTN,true,secondExchange?2:0,0,secondExchange);
};
troshx_tros_ai_TROSAiBot.tryBestPossibleAttackBoliao = function(cp,cp2,threatManuever,secondExchange) {
	troshx_tros_ai_TROSAiBot.MANUEVER_COMBO_SET = 0;
	var atk = troshx_tros_ai_TROSAiBot.getRegularAttackOrAdvantageMove(cp,0,threatManuever != null?threatManuever.manueverCP:-1,threatManuever != null?1:threatManuever.manueverTN);
	if(atk != null) {
		var cost;
		cp -= cost = troshx_tros_ai_TROSAiBot.getCostOfManuever(atk);
		if(cp > 0) {
			var atn = troshx_tros_ai_TROSAiBot.getATNOfManuever(atk);
			troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack(atk,secondExchange?cp:Std["int"](Math.ceil(cp * .5)),atn,troshx_tros_ai_TROSAiBot.getRegularTargetZone(atk,atn,cp),cost,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
			return true;
		}
	}
	return false;
...
troshx_tros_ai_TROSAiBot.tryBestPossibleAttackOrDefBoliao = function(cp,cp2,threatManuever,secondExchange) {
	...
	troshx_tros_ai_TROSAiBot.MANUEVER_COMBO_SET = 0;
	return troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao(cp,cp2,troshx_tros_ai_TROSAiBot.MOCK_RETURN_ATTACK,secondExchange) || troshx_tros_ai_TROSAiBot.tryBestPossibleAttackBoliao(cp,cp2,null,secondExchange);
};

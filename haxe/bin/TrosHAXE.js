...
troshx_tros_ai_TROSAiBot.setTypicalAVAILCostsForTesting = function() {
	...
	troshx_tros_ai_TROSAiBot.AVAIL_hook = 3;
...
troshx_tros_ai_TROSAiBot.getAdvantageManuever = function(manueverName,favorable,availableCP,againstRoll,againstTN,flags,customThreshold,preferedRS,defensive) {
	...
	if(availableCP > 0) {
		...
		if(costing > 0) {
			if(defensive) {
				troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY_GET = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
				troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend(manueverName,costing,tn,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
			} else {
				troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY_GET = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
				troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack(manueverName,costing,tn,0,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
			}
			return true;
		}
	}
...
troshx_tros_ai_TROSAiBot.getDisarm = function(favorable,availableCP,againstRoll,againstTN,flags,customThreshold,preferedRS,defensive) {
	...
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	if(troshx_tros_ai_TROSAiBot.AVAIL_disarm > 0) return troshx_tros_ai_TROSAiBot.getAdvantageManuever("disarm",favorable,availableCP,againstRoll,againstTN,flags,customThreshold,preferedRS,defensive); else return false;
};
...
troshx_tros_ai_TROSAiBot.getComboAction = function(combo,cp,cp2,threatManuever,hasInitiative,secondExchange) {
	...
	case -2:
		...
		break;
	case -4:
		if(threatManuever != null) if(troshx_tros_ai_TROSAiBot.AVAIL_disarm > 0) return troshx_tros_ai_TROSAiBot.getAdvantageManuever("disarm",true,cp,threatManuever.manueverCP,threatManuever.manueverTN,0,0,2,true); else return false;
		break;
	case -3:
		break;
	case -5:

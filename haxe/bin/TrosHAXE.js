...
var MainSOS = function() {
	...
	if((__map_reserved["ranging"] != null ? params.getReserved("ranging") : params.h["ranging"]) != null) {
		troshx_sos_vue_Globals.DOMAIN_RANGECALC = __map_reserved["ranging"] != null ? params.getReserved("ranging") : params.h["ranging"];
	}
	...
	if((__map_reserved["host"] != null ? params.getReserved("host") : params.h["host"]) != null || window["USE_HOST"]) {
		var h;
		var tmp;
		h = __map_reserved["host"] != null ? params.getReserved("host") : params.h["host"];
		if(h != null) {
			if(HxOverrides.substr(h,0,7) != "http://") {
				tmp = HxOverrides.substr(h,0,8) == "https://";
			} else {
				tmp = true;
			}
		} else {
		...
		if(tmp) {
			troshx_sos_vue_Globals.CURL_DOMAIN = h;
		} else {
			troshx_sos_vue_Globals.CURL_DOMAIN = window.location.protocol + "//" + window.location.hostname;
		}
	}
	var url = urlSplit[0];
	...
	if(hash == "chargen") {
		...
		this.boot.startVueWithRootComponent("#app",new troshx_sos_vue_RangeCalculator());
		haxevx_vuex_core_VxBoot.notifyStarted();
	} else {
...
troshx_sos_vue_CharGen.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,get_maxBuyableProfSlots: function() {
		...
		var totalAvailProfSlots = this.totalAvailProfSlots;
		if(cost > 0) {
			var canBuyWithinMax = rm / cost | 0;
			if(canBuyWithinMax > totalAvailProfSlots) {
				canBuyWithinMax = totalAvailProfSlots;
			}
			rm -= canBuyWithinMax * cost;
			rm = canBuyWithinMax + (rm >= 2 ? rm / 2 | 0 : 0);
		} else {
			rm = totalAvailProfSlots + (rm / 2 | 0);
		}
		if(this["char"].school != null) {
			return rm;
		} else {
	...
	,get_redirectLink: function() {
		return window.location.pathname + "?charsheet&redirectload";
	}
	,gotoRedirectLink: function() {
	...
	,isValidAll: function(warnings) {
		...
		if(warnings != null) {
			if(f) {
				...
				if(this.profCoreListMelee.length < this.maxMeleeProfSlots) {
					warnings.push("Profeciencies: You still have have melee profeciency slots to use.");
				}
				if(this.profCoreListRanged.length < this.maxRangedProfSlots) {
	...
	,saveFinaliseAll: function(finalising) {
		...
		if(this.isValidAll(warnings)) {
			if(warnings != null && warnings.length > 0) {
				...
				var arr;
				var generatedCharsStr = window.localStorage.getItem("generatedCharacters");
				if(generatedCharsStr != null) {
					arr = new haxe_Unserializer(generatedCharsStr).unserialize();
					if(arr.length >= 8) {
						arr.shift();
					}
					arr.push({ name : this["char"].get_uid(), data : savedCharString});
				} else {
					arr = [{ name : this["char"].get_uid(), data : savedCharString}];
				}
				var s = new haxe_Serializer();
				s.serialize(arr);
				window.localStorage.setItem("generatedCharacters",s.toString());
			}
	...
	,executeCopyContents: function() {
		...
		if(new EReg("/ipad|ipod|iphone","i").match(window.navigator.userAgent)) {
			var el = textarea;
	...
	,get_totalProfSlotExpenditure: function() {
		var _this = this.$data;
		var c = (_this["char"].race != null ? _this["char"].race.name : "") == "Human" ? 0 : 1;
		var available = _this["char"].school != null ? _this["char"].school.profLimit : 0;
		var total = _this.profCoreListRanged.length + _this.profCoreListMelee.length;
		var totalCap = total > available ? available : total;
		return totalCap * c + 2 * (total - totalCap);
	}
	,get_beyondMaxProfs: function() {
		var _this = this.$data;
		return _this.profCoreListRanged.length + _this.profCoreListMelee.length > (_this["char"].school != null ? _this["char"].school.profLimit : 0);
	}
	,get_atOrBeyondMaxProfString: function() {
		var _this = this.$data;
		if(_this.profCoreListRanged.length + _this.profCoreListMelee.length > (_this["char"].school != null ? _this["char"].school.profLimit : 0)) {
			return "Beyond Max";
		} else {
			return "Max Reached";
		}
	...
	,get_profPointsLeft: function() {
		...
		var total = _this.profCoreListRanged.length + _this.profCoreListMelee.length;
		var totalCap = total > available ? available : total;
		return (_this.categories[5].pcp - 1) * 3 - (totalCap * c + 2 * (total - totalCap)) - (_this["char"].school != null ? _this["char"].school.costArc : 0) - (_this["char"].school != null && _this["char"].schoolLevel > 0 ? _this.schoolLevelCosts[_this["char"].schoolLevel - 1] : 0);
	}
	,get_traceProfCoreRangedCurrent: function() {
...
troshx_sos_vue_CharSheetVue.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,get_redirectChargenLink: function() {
		return window.location.pathname + "?chargen";
	}
	,Mounted: function() {
		...
		if(params.get("redirectload") != null) {
			params["delete"]("redirectload");
			var str = "" + location.pathname + "?" + Std.string(params);
			if(str.charAt(str.length - 1) == "=") {
				str = HxOverrides.substr(str,0,str.length - 1);
			}
			window.history.replaceState({ },window.document.title,str);
			this.openSavedCharacterList();
			if(this.generatedCharacters != null && this.generatedCharacters.length > 0) {
				this.selectedCharIndex = 0;
				this.loadCharacterFromList();
			}
		}
	}
	...
	,openFromTreeBrowser: function(contents,filename,disableCallback) {
		if(this.loadCharContents(contents)) {
			...
			this.setWindowTitleWithCharUID();
		}
		this.autoLoadChar = null;
	}
	,setWindowTitleWithCharUID: function() {
		var uid = this["char"].get_uid();
		if(uid == "") {
			return;
		}
		window.document.title = uid;
		this.theWindowTitle = uid;
	}
	...
	,get_maxGeneratedSavedChars: function() {
		return 8;
	}
	,openSavedCharacterList: function() {
		this.lastSavedSession = window.localStorage.getItem("lastSavedCharacter");
		var str = window.localStorage.getItem("generatedCharacters");
		if(str != null) {
			var arr = new haxe_Unserializer(str).unserialize();
			arr.reverse();
		...
		this.$refs.showSavedCharacters.open();
	}
	,loadCharacterFromList: function() {
		if(this.selectedLastSaved) {
			if(this.loadCharContents(this.lastSavedSession)) {
				this.$refs.showSavedCharacters.close();
				this.setWindowTitleWithCharUID();
			}
		} else if(this.loadCharContents(this.generatedCharacters[this.selectedCharIndex].data)) {
			this.$refs.showSavedCharacters.close();
			this.setWindowTitleWithCharUID();
		}
	}
	,saveCharacter: function() {
		...
		window.localStorage.setItem("lastSavedCharacter",this.savedCharContents);
	}
	...
	,deleteStaticModifierAt: function(i,li) {
		this["char"].staticModifierTable[li].splice(i,1);
	}
	,deleteSituationalCharModifierAt: function(i,li) {
	...
	,setModifierTabIndex: function(refId) {
		var dyn = troshx_sos_core_Modifier;
		this.customModifier.index = dyn[refId];
	}
	,openModifiersWindow: function() {
		this.customModifier = troshx_sos_core_StaticModifier.create(this.customModifier != null ? this.customModifier.index : 0,"",0,1);
		this.customModifier.custom = true;
		this.$refs.modifiersWindow.open();
	}
	,addCustomModifier: function() {
		var theModifier = this.customModifier;
		this.customModifier = troshx_sos_core_StaticModifier.create(this.customModifier != null ? this.customModifier.index : 0,"",0,1);
		this.customModifier.custom = true;
		var _this = this["char"];
		if(theModifier != null) {
			var t = _this.staticModifierTable[theModifier.index];
			t.push(theModifier);
			var m = theModifier.next;
			while(m != null) {
				t = _this.staticModifierTable[m.index];
				t.push(m);
				m = m.next;
	...
	,watch_char: function(newVal) {
		...
		Vue.nextTick($bind(this,this.postCharChange));
	}
	,postCharChange: function() {
	...
	,_Init: function() {
		...
		this.components = this.Components();
		this.template = this.Template();
		this.computed = { maxAvailableProfSlots : clsP.get_maxAvailableProfSlots, maxMeleeProfSlots : clsP.get_maxMeleeProfSlots, maxRangedProfSlots : clsP.get_maxRangedProfSlots, maxTalentSlots : clsP.get_maxTalentSlots, maxSuperiorSlots : clsP.get_maxSuperiorSlots, domainId : clsP.get_domainId, redirectChargenLink : clsP.get_redirectChargenLink, availableTypes : clsP.get_availableTypes, hasSampleWound : clsP.get_hasSampleWound, sampleWoundGotSide : clsP.get_sampleWoundGotSide, woundFlagLabels : clsP.get_woundFlagLabels, damageTypeLabels : clsP.get_damageTypeLabels, maxGeneratedSavedChars : clsP.get_maxGeneratedSavedChars, talentsAvailable : clsP.get_talentsAvailable, superiorsAvailable : clsP.get_superiorsAvailable, placeholder : clsP.get_placeholder};
		this.methods = { setInventory : clsP.setInventory, get_domainId : clsP.get_domainId, get_redirectChargenLink : clsP.get_redirectChargenLink, recoverFatique : clsP.recoverFatique, openTreeBrowser : clsP.openTreeBrowser, openClipboardWindow : clsP.openClipboardWindow, openFromTreeBrowser : clsP.openFromTreeBrowser, setWindowTitleWithCharUID : clsP.setWindowTitleWithCharUID, get_availableTypes : clsP.get_availableTypes, get_hasSampleWound : clsP.get_hasSampleWound, get_sampleWoundGotSide : clsP.get_sampleWoundGotSide, deleteWound : clsP.deleteWound, checkConfirmDeleteWound : clsP.checkConfirmDeleteWound, confirmDeleteWound : clsP.confirmDeleteWound, confirmAddWound : clsP.confirmAddWound, get_woundFlagLabels : clsP.get_woundFlagLabels, get_damageTypeLabels : clsP.get_damageTypeLabels, matchExistingSampleWound : clsP.matchExistingSampleWound, openAddNewWound : clsP.openAddNewWound, exitInventory : clsP.exitInventory, proceedToInventory : clsP.proceedToInventory, saveFinaliseSkills : clsP.saveFinaliseSkills, loadCharacter : clsP.loadCharacter, loadCharacterClipboardWindow : clsP.loadCharacterClipboardWindow, loadCharContents : clsP.loadCharContents, get_maxGeneratedSavedChars : clsP.get_maxGeneratedSavedChars, openSavedCharacterList : clsP.openSavedCharacterList, loadCharacterFromList : clsP.loadCharacterFromList, saveCharacter : clsP.saveCharacter, saveFinaliseCleanupChar : clsP.saveFinaliseCleanupChar, deleteStaticModifierAt : clsP.deleteStaticModifierAt, deleteSituationalCharModifierAt : clsP.deleteSituationalCharModifierAt, executeCopyContents : clsP.executeCopyContents, saveCharToBox : clsP.saveCharToBox, get_maxAvailableProfSlots : clsP.get_maxAvailableProfSlots, get_maxMeleeProfSlots : clsP.get_maxMeleeProfSlots, get_maxRangedProfSlots : clsP.get_maxRangedProfSlots, get_talentsAvailable : clsP.get_talentsAvailable, get_superiorsAvailable : clsP.get_superiorsAvailable, get_maxTalentSlots : clsP.get_maxTalentSlots, get_maxSuperiorSlots : clsP.get_maxSuperiorSlots, addSkill : clsP.addSkill, setModifierTabIndex : clsP.setModifierTabIndex, openModifiersWindow : clsP.openModifiersWindow, addCustomModifier : clsP.addCustomModifier, deleteSkillInput : clsP.deleteSkillInput, get_placeholder : clsP.get_placeholder, watch_injectChar : clsP.watch_injectChar, watch_char : clsP.watch_char, postCharChange : clsP.postCharChange};
...
var troshx_sos_vue_CharSheetVueData = function($char) {
	this.generatedCharacters = null;
	this.lastSavedSession = null;
	this.selectedCharIndex = -1;
	this.selectedLastSaved = false;
	this.theWindowTitle = "";
	this.arcSpendQty = 0;
	this.arcAwardQty = 0;
	this.forceNewSampleWound = false;
...
troshx_sos_vue_CharVueMixin.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,get_pain: function() {
		var r = this.totalPain - this.GRIT;
		if(r < 0) {
			return 0;
		} else {
			return r;
	...
	,get_perceptionPenalized: function() {
	...
	,get_dynModifiers: function() {
		return troshx_sos_core_Modifier;
	}
	,get_customModifierListings: function() {
		var modTable = this["char"].staticModifierTable;
		var dyn = troshx_sos_core_Modifier;
		var arr = [];
		var modVars = this.modifierVars;
		var _g1 = 0;
		var _g = modVars.length;
		while(_g1 < _g) {
			var i = _g1++;
			var index = dyn[modVars[i]];
			var prefix = "(" + this.getModifierTabTitle(modVars[i],true) + ") ";
	...
	,getModifierTabTitle: function(li,noCount) {
		if(noCount == null) {
			noCount = false;
		}
		var dyn = troshx_sos_core_Modifier;
		var index = dyn[li];
		var arrStatic = this["char"].staticModifierTable[index];
		var arrChar = this["char"].situationalModifierTable[index];
		var totalLen = noCount ? 0 : arrStatic.length + arrChar.length;
		var liStr = HxOverrides.substr(li,0,4) == "CMP_" ? HxOverrides.substr(li,4,null) : HxOverrides.substr(li,0,5) == "ATTR_" ? HxOverrides.substr(li,5,null) : li;
		if(totalLen > 0) {
			return liStr + "(" + totalLen + ")";
		} else {
			return liStr;
		}
	}
	,get_modifierVars: function() {
		var arr = [];
		arr.push("ATTR_STR");
		arr.push("ATTR_AGI");
		arr.push("ATTR_END");
	...
	,get_baseCP: function() {
		var _this = this["char"];
		return (_this.schoolLevel >= 1 ? _this.schoolLevel : 0) + _this.clampIntZero(_this.getModifiedValue(8,(_this.get_AGI() + _this.get_WIT()) / 2 | 0));
	}
	,get_CP: function() {
		var _this = this["char"];
		var row = _this.get_encumbranceLvlRow();
	...
	,get_meleeCP: function() {
		var _this = this["char"];
		var row = _this.get_encumbranceLvlRow();
		return row.cpMult * _this.clampIntZero(_this.getModifiedValue(13,(_this.schoolLevel >= 1 ? _this.schoolLevel : 0) + _this.clampIntZero(_this.getModifiedValue(8,(_this.get_AGI() + _this.get_WIT()) / 2 | 0))) * (_this.prone ? 0.5 : 1) + row.cp + troshx_sos_sheets_FatiqueTable.getTable()[_this.get_fatiqueLevel()].cp) - _this.get_pain();
	}
	,get_schoolCP: function() {
		var _this = this["char"];
		if(_this.schoolLevel >= 1) {
			return _this.schoolLevel;
		} else {
			return 0;
		}
	}
	...
	,get_beyondMaxProfs: function() {
		if(this.hasSchool) {
			return this["char"].getTotalProfeciencies() > this["char"].school.profLimit;
		} else {
			return false;
		}
	}
...
troshx_sos_vue_InventoryVue.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,getTags: function(item,entry) {
		var arr = [];
		var gotVariant = entry != null && entry.holding1H && entry.weapon.variant != null;
		if(!gotVariant) {
	...
	,getDefGuard: function(wpn,entry) {
		if(entry != null && entry.holding1H && entry.weapon.variant != null) {
			wpn = entry.weapon.variant;
		}
		return wpn.dtn + "(" + wpn.guard + ")";
	}
	,getSwingAtkStr: function(wpn,entry) {
		if(entry != null && entry.holding1H && entry.weapon.variant != null) {
			wpn = entry.weapon.variant;
		}
		return wpn.atnS + "(" + wpn.damageS + this.damageTypeSuffixes[wpn.damageTypeS] + ")";
	}
	,getThrustAtkStr: function(wpn,entry) {
		if(entry != null && entry.holding1H && entry.weapon.variant != null) {
			wpn = entry.weapon.variant;
		}
	...
	,get_shouldCalcMeleeAiming: function() {
		if(!this.calcArmorMissile) {
			return this.calcArmorMeleeTargeting;
		} else {
			return false;
		}
	}
	,get_shouldCalcRangedAiming: function() {
		if(this.calcArmorMissile) {
			return this.selectedMissileTargetZoneIndex >= 0;
		} else {
			return false;
		}
	}
	...
	,get_showTargetingZoneIndex: function() {
		if(this.shouldCalcMeleeAiming) {
			return this.calcMeleeTargetingZoneIndex;
		} else if(this.shouldCalcRangedAiming) {
			return this.missileHitTargetZones[this.selectedMissileTargetZoneIndex].index;
		} else {
			return -1;
		}
	}
	,isDisabledHitLocation: function(i) {
		if(this.shouldCalcMeleeAiming || this.shouldCalcRangedAiming) {
			return (1 << i & this.targetZoneHitAreaMasks[this.showTargetingZoneIndex]) == 0;
		} else {
			return false;
		}
	}
	...
	,get_hitLocationArmorValues: function() {
		...
		var nonFirearmMissile = this.calcArmorMissile && !this.calcArmorFirearm;
		var _g1 = 0;
		var _g = ch.length;
		while(_g1 < _g) {
			var i = _g1++;
	...
	,get_missileHitTargetZones: function() {
		var b = this.body;
	...
	,get_missileTargetHitZone: function() {
		var mt = this.missileHitTargetZones;
		if(this.selectedMissileTargetZoneIndex >= 0) {
			return mt[this.selectedMissileTargetZoneIndex].tz;
		} else {
			return null;
		}
	}
	...
	,get_calcMeleeTZProbabilities: function() {
		var bod = this.body;
		var tz = bod.targetZones[this.calcMeleeTargetingZoneIndex];
		var str = "";
		var count = 0;
		var _g1 = 0;
		var _g = tz.partWeights.length;
		while(_g1 < _g) {
			var i = _g1++;
			var part = bod.hitLocations[tz.parts[i]];
			str += "[" + part.name + ": ";
			str += ++count;
			var _g3 = 1;
			var _g2 = tz.partWeights[i] | 0;
			while(_g3 < _g2) {
				var i1 = _g3++;
				str += "," + ++count;
			}
	...
	,get_calcRangedTZProbabilities: function() {
		var bod = this.body;
		var tz = this.selectedMissileTargetZoneIndex >= 0 ? this.missileHitTargetZones[this.selectedMissileTargetZoneIndex].tz : null;
		if(tz == null) {
			return "";
		}
		var str = "";
...
var troshx_sos_vue_InventoryVueData = function() {
	...
	this.calcArmorMissile = false;
	this.calcArmorFirearm = false;
	this.focusValueText = "";
	this.itemTransitionName = "fade";
	...
	this.privateData = { };
	this.selectedMissileTargetZoneIndex = -1;
...
var troshx_sos_vue_InventoryVueArmor = function() {
	troshx_sos_vue_InventoryVue.call(this);
};
$hxClasses["troshx.sos.vue.InventoryVueArmor"] = troshx_sos_vue_InventoryVueArmor;
troshx_sos_vue_InventoryVueArmor.__name__ = ["troshx","sos","vue","InventoryVueArmor"];
troshx_sos_vue_InventoryVueArmor.__super__ = troshx_sos_vue_InventoryVue;
troshx_sos_vue_InventoryVueArmor.prototype = $extend(troshx_sos_vue_InventoryVue.prototype,{
	Template: function() {
		return "<div class=\"inventory\" v-on:click=\"onBaseInventoryClick\">\r\n\t\r\n\t<h4>Dropped:</h4>\r\n\t<table border=\"1\">\r\n\t\t<thead>\r\n\t\t\t<tr>\r\n\t\t\t\t<td>Item Name</td>\r\n\t\t\t\t<td class=\"qty\">Qty</td>\r\n\t\t\t\t<td class=\"weight\">Weight</td>\r\n\t\t\t\t<td class=\"munition-type\">Type</td>\r\n\t\t\t\t<td class=\"actions\"><i>Actions</i></td>\r\n\t\t\t</tr>\r\n\t\t</thead>\r\n\t\t<tbody :name=\"itemTransitionName\" is=\"transition-group\" v-on:mousedown.native=\"setCurWidgetSection('dropped', $event)\" v-on:touchstart.native=\"setCurWidgetSection('dropped', $event)\">\r\n\t\t\t\r\n\t\t\t<tr v-for=\"(entry,i) in inventory.dropped.list\" :key=\"entry.item.get_uid()\">\r\n\t\t\t\t<td><input-name-qty :itemQty=\"entry\" v-on:updated=\"onInputNameUpdated\" :customValidateName=\"validateQtyNameDropped\" /></td>\r\n\t\t\t\t<td><InputInt :obj=\"entry\" prop=\"qty\" :min=\"1\"></InputInt></td>\r\n\t\t\t\t<td><InputNumber :floating=\"true\" :step=\"0.5\" :obj=\"entry.item\" prop=\"weight\" :min=\"0\"></InputNumber></td>\r\n\t\t\t\t<td>{{ entry.item.getTypeLabel() }}</td>\r\n\t\t\t\t<td>\r\n\t\t\t\t\t<a href=\"javascript:;\" v-on:click=\"inventory.equipItemEntryFromGround(entry)\">Equip</a>\r\n\t\t\t\t\t<a href=\"javascript:;\" v-on:click=\"inventory.deleteDropped(entry)\">Delete</a>\r\n\t\t\t\t</td>\r\n\t\t\t</tr>\t\r\n\t\t</tbody>\r\n\t</table>\r\n\r\n\t<h4>Shields:</h4>\r\n\t<table border=\"1\">\r\n\t\t<thead>\r\n\t\t\t<tr>\r\n\t\t\t\t<td style=\"width:155px\">Shield Name</td>\r\n\t\t\t\t<td style=\"width:35px\">AV</td>\r\n\t\t\t\t<td style=\"width:35px\">Block</td>\r\n\t\t\t\t<td style=\"width:45px\">Weight</td>\r\n\t\t\t\t<td style=\"width:67px\">Size</td>\r\n\t\t\t\t<td class=\"tags\">Tags</td>\r\n\t\t\t\t<td style=\"width:130px\">Kept at?</td>\r\n\t\t\t\t<td class=\"held\">Held?</td>\r\n\t\t\t\t<td class=\"actions\"><i>Actions</i></td>\r\n\t\t\t</tr>\r\n\t\t</thead>\r\n\t\t<tbody :name=\"itemTransitionName\" is=\"transition-group\" v-on:mousedown.native=\"setCurWidgetSection('shield',$event)\" v-on:touchstart.native=\"setCurWidgetSection('shield',$event)\">\r\n\t\t\t<tr :key=\"entry.key\" v-for=\"(entry,i) in inventory.shields\">\r\n\t\t\t\t<td><input-name :item=\"entry.shield\" v-on:updated=\"onInputNameUpdated\" /></td>\r\n\t\t\t\t<td><InputInt :obj=\"entry.shield\" prop=\"AV\" :min=\"1\"></InputInt></td>\r\n\t\t\t\t<td><InputInt :obj=\"entry.shield\" prop=\"blockTN\" :min=\"0\"></InputInt></td>\r\n\t\t\t\t<td><InputNumber :step=\"0.5\" :obj=\"entry.shield\" prop=\"weight\" :min=\"0\"></InputNumber></td>\r\n\t\t\t\t<td>\r\n\t\t\t\t\t<select number v-model.number=\"entry.shield.size \">\r\n\t\t\t\t\t\t<option :value=\"0\">Small</option>\r\n\t\t\t\t\t\t<option :value=\"1\">Medium</option>\r\n\t\t\t\t\t\t<option :value=\"2\">Large</option>\r\n\t\t\t\t\t</select>\r\n\t\t\t\t</td>\r\n\t\t\t\t<td-widget v-on:focus-widget-value=\"focusWidgetValue\" widgetName=\"tags\" widgetTagName=\"w-tags\" :value=\"getTags(entry.shield)\" :index=\"i\" :showWidget=\"isVisibleWidget('shield','tags', i)\" :entry=\"entry\" :requestCurWidget=\"requestCurWidget\" />\r\n\t\t\t\t<td-unheld :entry=\"entry\" :requestCurWidget=\"requestCurWidget\" :index=\"i\" :showWidget=\"isVisibleWidget('shield', 'td-unheld', i)\" />\r\n\t\t\t\t<td>\r\n\t\t\t\t\t<select-held :entry=\"entry\" :inventory=\"inventory\" />\r\n\t\t\t\t</td>\r\n\t\t\t\t<td>\r\n\t\t\t\t\t<a href=\"javascript:;\" v-on:click=\"dropEquipedShield(entry)\">Drop</a>\r\n\t\t\t\t\t<a href=\"javascript:;\" v-on:click=\"dropEquipedShield(entry, true)\">Delete</a>\r\n\t\t\t\t</td>\r\n\t\t\t</tr>\r\n\t\t\t<tr class=\"entry-row\" :key=\"-1\">\r\n\t\t\t\t<td><input type=\"text\" v-on:focus=\"focusInRowField(shieldEntry, 1)\" v-on:blur=\"focusOutRowField(shieldEntry, 1)\" v-model=\"shieldEntry.e.shield.name\" required></input></td>\r\n\t\t\t\t<td><InputInt v-on:focus.native=\"focusInRowField(shieldEntry, 2)\" v-on:blur.native=\"focusOutRowField(shieldEntry, 2)\" :obj=\"shieldEntry.e.shield\" prop=\"AV\" :min=\"1\"></InputInt></td>\r\n\t\t\t\t<td><InputInt v-on:focus.native=\"focusInRowField(shieldEntry, 4)\" v-on:blur.native=\"focusOutRowField(shieldEntry, 4)\" :obj=\"shieldEntry.e.shield\" prop=\"blockTN\" :min=\"0\"></InputInt></td>\r\n\t\t\t\t<td><InputInt v-on:focus.native=\"focusInRowField(shieldEntry, 8)\" v-on:blur.native=\"focusOutRowField(shieldEntry, 8)\" :obj=\"shieldEntry.e.shield\" prop=\"weight\" :min=\"0\"></InputInt></td>\r\n\t\t\t\t<td>\r\n\t\t\t\t\t<select v-model.number=\"shieldEntry.e.shield.size\" v-on:focus=\"focusInRowField(shieldEntry, 16)\" v-on:blur=\"focusOutRowField(shieldEntry, 16)\">\r\n\t\t\t\t\t\t<option :value=\"0\">Small</option>\r\n\t\t\t\t\t\t<option :value=\"1\">Medium</option>\r\n\t\t\t\t\t\t<option :value=\"2\">Large</option>\r\n\t\t\t\t\t</select>\r\n\t\t\t\t</td>\r\n\t\t\t\t<td-widget v-on:focus-widget-value=\"focusWidgetValue\" widgetName=\"tags\" widgetTagName=\"w-tags\" :value=\"getTags(shieldEntry.e.shield)\" :index=\"-1\" :showWidget=\"(shieldEntry.focusedFlags & (32|(1<<21))) !=0\" :entry=\"shieldEntry.e\" :requestCurWidget=\"requestCurWidget\"\r\n\t\t\t\tv-on:focus-in-row=\"focusInRowField(shieldEntry, 32)\" v-on:focus-out-row=\"focusOutRowField(shieldEntry, 32)\" v-on:focus-widget=\"focusInRowField(shieldEntry, (1<<21))\" />\r\n\t\t\t\t<td><input type=\"text\" v-model=\"shieldEntry.e.unheldRemark\" v-on:focus=\"focusInRowField(shieldEntry,64)\" v-on:blur=\"focusOutRowField(shieldEntry,64)\"></input></td>\t\r\n\t\t\t\t\r\n\t\t\t</tr>\r\n\t\t</tbody>\r\n\t</table>\r\n\t\r\n\t<h4>Armor:</h4>\r\n\t<table border=\"1\">\r\n\t\t<thead>\r\n\t\t\t<tr>\r\n\t\t\t\t<td>Armor Name</td>\r\n\t\t\t\t<td>AVC</td>\r\n\t\t\t\t<td>AVP</td>\r\n\t\t\t\t<td>AVB</td>\r\n\t\t\t\t<td>Coverage</td>\r\n\t\t\t\t<td class=\"weight\">Weight</td>\r\n\t\t\t\t<td class=\"tags\">Tags</td>\r\n\t\t\t\t<td class=\"actions\"><i>Actions</i></td>\r\n\t\t\t</tr>\r\n\t\t</thead>\r\n\t\t<tbody :name=\"itemTransitionName\" is=\"transition-group\" v-on:mousedown.native=\"setCurWidgetSection('armor', $event)\"  v-on:touchstart.native=\"setCurWidgetSection('armor', $event)\">\r\n\t\t\t<tr v-for=\"(entry, i) in inventory.wornArmor\" :key=\"i\">\r\n\t\t\t\t<td><input-name :item=\"entry.armor\" v-on:updated=\"onInputNameUpdated\" /></td>\r\n\t\t\t\t<td><InputInt :min=\"0\" :obj=\"entry.armor\" prop=\"AVC\"></InputInt></td>\r\n\t\t\t\t<td><InputInt :min=\"0\" :obj=\"entry.armor\" prop=\"AVP\"></InputInt></td>\r\n\t\t\t\t<td><InputInt :min=\"0\" :obj=\"entry.armor\" prop=\"AVB\"></InputInt></td>\t\t\t\r\n\t\t\t\t<td-widget v-on:focus-widget-value=\"focusWidgetValue\" widgetName=\"coverage\" widgetTagName=\"w-coverage\" :value=\"getCoverage(entry.armor)\" :index=\"i\" :showWidget=\"isVisibleWidget('armor','coverage', i)\" :entry=\"entry\" :requestCurWidget=\"requestCurWidget\" />\r\n\t\t\t\t<td><InputNumber :min=\"0\" :step=\"0.5\" :obj=\"entry.armor\" prop=\"weight\"></InputNumber></td>\r\n\t\t\t\t<td-widget v-on:focus-widget-value=\"focusWidgetValue\" widgetName=\"tags\" widgetTagName=\"w-tags\" :value=\"getTags(entry.armor)\" :index=\"i\" :showWidget=\"isVisibleWidget('armor','tags', i)\" :entry=\"entry\" :requestCurWidget=\"requestCurWidget\" />\r\n\t\t\t\t\r\n\t\t\t\t<td>\r\n\t\t\t\t\t<a href=\"javascript:;\" v-on:click=\"dropWornArmor(entry)\">Drop</a>\r\n\t\t\t\t\t<a href=\"javascript:;\" v-on:click=\"dropWornArmor(entry, true)\">Delete</a>\r\n\t\t\t\t</td>\r\n\t\t\t</tr>\r\n\t\t\t<tr class=\"entry-row\" :key=\"-1\">\r\n\t\t\t\t<td><input type=\"text\" v-model=\"armorEntry.e.armor.name\" v-on:focus=\"focusInRowField(armorEntry,1)\" v-on:blur=\"focusOutRowField(armorEntry,1)\"></input></td>\r\n\t\t\t\t<td><InputInt :min=\"0\" :obj=\"armorEntry.e.armor\" prop=\"AVC\" v-on:focus.native=\"focusInRowField(armorEntry,2)\" v-on:blur.native=\"focusOutRowField(armorEntry,2)\"></InputInt></td>\r\n\t\t\t\t<td><InputInt :min=\"0\" :obj=\"armorEntry.e.armor\" prop=\"AVP\" v-on:focus.native=\"focusInRowField(armorEntry,4)\" v-on:blur.native=\"focusOutRowField(armorEntry,4)\"></InputInt></td>\r\n\t\t\t\t<td><InputInt :min=\"0\" :obj=\"armorEntry.e.armor\" prop=\"AVB\" v-on:focus.native=\"focusInRowField(armorEntry,8)\" v-on:blur.native=\"focusOutRowField(armorEntry,8)\"></InputInt></td>\r\n\t\t\t\t<td-widget v-on:focus-widget-value=\"focusWidgetValue\" widgetName=\"coverage\" widgetTagName=\"w-coverage\" :value=\"getCoverage(armorEntry.e.armor)\" :index=\"-1\" :showWidget=\"(armorEntry.focusedFlags & (16|(1<<20))) !=0\" :entry=\"armorEntry.e.armor\" :requestCurWidget=\"requestCurWidget\"\r\n\t\t\t\t\tv-on:focus-in-row=\"focusInRowField(armorEntry, 16)\" v-on:focus-out-row=\"focusOutRowField(armorEntry, 16)\" v-on:focus-widget=\"focusInRowField(armorEntry, (1<<20))\" />\r\n\t\t\t\t\r\n\t\t\t\t\r\n\t\t\t\t<td><InputNumber :min=\"0\" :step=\"0.5\" :obj=\"armorEntry.e.armor\" prop=\"weight\" v-on:focus.native=\"focusInRowField(armorEntry,32)\" v-on:blur.native=\"focusOutRowField(armorEntry,32)\"></InputNumber></td>\r\n\t\t\t\t<td-widget v-on:focus-widget-value=\"focusWidgetValue\" widgetName=\"tags\" widgetTagName=\"w-tags\" :value=\"getTags(armorEntry.e.armor)\" :index=\"-1\" :showWidget=\"(armorEntry.focusedFlags & (64|(1<<21))) !=0\" :entry=\"armorEntry.e.armor\" :requestCurWidget=\"requestCurWidget\"\r\n\t\t\t\tv-on:focus-in-row=\"focusInRowField(armorEntry, 64)\" v-on:focus-out-row=\"focusOutRowField(armorEntry, 64)\" v-on:focus-widget=\"focusInRowField(armorEntry, (1<<21))\" />\r\n\t\t\t\t<td><input type=\"text\" readonly class=\"focusable\" v-on:focus=\"focusInRowField(armorEntry,64)\" v-on:blur=\"focusOutRowField(armorEntry,64)\"></input></td>\r\n\t\t\t</tr>\r\n\t\t</tbody>\r\n\t</table>\t\t\r\n\t\r\n\t\r\n\t<hr/>\r\n\t\t\r\n\t\t<div class=\"overall-armor-coverage\" v-if=\"showArmorCoverage\">\r\n\t\t\t<h4>Overall Armor Coverage:</h4>\t\r\n\t\t\t<div v-if=\"carriedShield!=null\">Held Shield: <b>{{carriedShield.name}}</b> <span class=\"shield-icon-inv\">☗</span></div>\r\n\t\t\t<div v-if=\"carriedShield!=null\">Size: <b>{{shieldSizeLabels[carriedShield.size]}}</b></div>\r\n\t\t\t<div><label>Prefered Shield Position:\r\n\t\t\t\t<select v-model.number=\"inventory.shieldPosition\" number>\r\n\t\t\t\t\t<option :value=\"0\">Low</option>\r\n\t\t\t\t\t<option :value=\"1\">High</option>\r\n\t\t\t\t</select>\t\r\n\t\t\t</label> <span v-show=\"carriedShield==null\"> (if held)</span></div>\r\n\t\t\t<div style=\"display:inline-block;vertical-align:top;\">\r\n\t\t\t\t<table border=\"1\" class=\"table-coverage\">\r\n\t\t\t\t\t<thead>\r\n\t\t\t\t\t\t<tr>\r\n\t\t\t\t\t\t\t<td></td>\r\n\t\t\t\t\t\t\t<td></td>\r\n\t\t\t\t\t\t\t<td>AVC</td>\r\n\t\t\t\t\t\t\t<td>AVP<span v-show=\"calcArmorMissile\"> ↢</span></td>\r\n\t\t\t\t\t\t\t<td>AVB</td>\r\n\t\t\t\t\t\t\t<td v-if=\"carriedShield !=null\">sAV\t<span class=\"shield-icon-inv\">☗</span></td>\r\n\t\t\t\t\t\t</tr>\r\n\t\t\t\t\t</thead>\r\n\t\t\t\t\t<tbody>\r\n\t\t\t\t\t\t<tr v-for=\"(li, i) in coverageHitLocations\">\r\n\t\t\t\t\t\t\t<td><i>{{getHitLocationMaskNameOf(i)}}</i></td>\r\n\t\t\t\t\t\t\t<td>{{i+1}}. {{li.name}}</td>\r\n\t\t\t\t\t\t\t<td><input type=\"number\" readonly :value=\"hitLocationArmorValues[li.id].avc\" :disabled=\"isDisabledHitLocation(i)\" v-on:focus=\"calcAVColumnRowIndex(1, i)\" v-on:blur=\"focusOutAVColumnRowIndex(1,i)\"></input></td>\r\n\t\t\t\t\t\t\t<td><input type=\"number\" readonly :value=\"hitLocationArmorValues[li.id].avp\" :disabled=\"isDisabledHitLocation(i)\" v-on:focus=\"calcAVColumnRowIndex(2, i)\" v-on:blur=\"focusOutAVColumnRowIndex(2,i)\"></input></td>\r\n\t\t\t\t\t\t\t<td><input type=\"number\" readonly :value=\"hitLocationArmorValues[li.id].avb\" :disabled=\"isDisabledHitLocation(i)\" v-on:focus=\"calcAVColumnRowIndex(3, i)\" v-on:blur=\"focusOutAVColumnRowIndex(3,i)\"></input></td>\r\n\t\t\t\t\t\t\t<td v-if=\"carriedShield!=null\" v-show=\"shieldCoverage[li.id]!=null\" class=\"td-shield\" :class=\"{active:calcAVRowIndex==i && shieldAVHigherOrEqual }\">{{ shieldCoverage[li.id] ===false ? \"~\" : \"\"}}{{ carriedShield.AV }}</td>\r\n\t\t\t\t\t\t</tr>\r\n\t\t\t\t\t</tbody>\r\n\t\t\t\t</table>\r\n\t\t\t</div>\r\n\t\t\t<div style=\"display:inline-block;vertical-align:top;margin-left:8px;\">\r\n\t\t\t\t<div><i class=\"consider\">Consider against:</i></div>\r\n\t\t\t\t<br/>\r\n\t\t\t\t<div><label><input type=\"checkbox\" v-model=\"calcArmorCrushing\"></input>Crushing attack</label></div>\r\n\t\t\t\t<div><label><input type=\"checkbox\" v-model=\"calcArmorMissile\"></input>Missile attack ↢</label> <label :class=\"{disabled:!calcArmorMissile}\" :style=\"{'pointer-events':!calcArmorMissile ? 'none': 'auto'}\"><input type=\"checkbox\" v-model=\"calcArmorFirearm\"></input><i>Firearm?</i></label> Hit at:<select number v-model.number=\"selectedMissileTargetZoneIndex\"><option :value=\"-1\"></option><option :value=\"i\" v-for=\"(li, i) in missileHitTargetZones\" :key=\"i\">{{i+2}}:{{li.tz.name}}</option></select> <a v-on:click=\"selectedMissileTargetZoneIndex=-1\" href=\"javascript:;\" v-show=\"selectedMissileTargetZoneIndex>=0\">[x]</a></div>\r\n\t\t\t\t<div><label :class=\"{disabled:calcArmorMissile}\" :style=\"{'pointer-events':calcArmorMissile ? 'none': 'auto'}\"><input type=\"checkbox\" v-model=\"calcArmorMeleeTargeting\"></input> <span :class=\"{disabled:!shouldCalcMeleeAiming}\">Melee Targeting:</span> </label><select number :disabled=\"!shouldCalcMeleeAiming\" v-model.number=\"calcMeleeTargetingZoneIndex\">\r\n\t\t\t\t\t<option :value=\"i\" v-for=\"(li, i) in body.targetZones\">{{ body.getDescLabelTargetZone(i) }}</option>\r\n\t\t\t\t</select></div>\r\n\t\t\t\t<div style=\"margin-top:5px\" v-show=\"calcArmorMeleeTargeting && shouldCalcMeleeAiming\">{{ calcMeleeTZProbabilities }}</div>\r\n\t\t\t\t<div style=\"margin-top:5px\" v-show=\"calcArmorMissile && selectedMissileTargetZoneIndex>=0\">{{ calcRangedTZProbabilities }}</div>\r\n\t\t\t\t<br/>\r\n\t\t\t\t<div><i>Result: </i><b>{{ calcArmorResults.av + calcArmorResults.layer }} AV</b></div>\r\n\t\t\t\t<i class=\"cta\" v-show=\"calcAVColumn==0\">&lt;- Focus-click on table cells to inspect result.</i>\r\n\t\t\t\t<br/>\r\n\t\t\t\t<div v-show=\"calcAVColumn!=0\">\r\n\t\t\t\t\t<div><span class=\"outline-dominant\">AV Armor protecting: <b>{{ calcArmorResults.av }} AV</b></span></div>\r\n\t\t\t\t\t<ul v-show=\"hasArmorResultProtecting\">\r\n\t\t\t\t\t\t<li v-for=\"li in calcArmorResults.armorsProtectable\">{{ li.name }}</li>\r\n\t\t\t\t\t</ul>\r\n\t\t\t\t\t<br v-show=\"!hasArmorResultProtecting\" />\r\n\t\t\t\t\t<div><span class=\"outline-layer\">Layer protecting: <b>+{{ calcArmorResults.layer }} AV</b></span></div>\r\n\t\t\t\t\t<ul v-show=\"hasArmorResultLayers\">\r\n\t\t\t\t\t\t<li v-for=\"li in calcArmorResults.armorsLayer\">{{ li.name }}</li>\r\n\t\t\t\t\t</ul>\r\n\t\t\t\t\t<br v-show=\"!hasArmorResultLayers\" />\r\n\t\t\t\t\t<div v-show=\"calcArmorCrushing\">\r\n\t\t\t\t\t\t<div><span class=\"outline-crushed\">Crushable Hard Armor: <b><span v-show=\"!hasArmorCrushables\"> <i>none</i></span></b></span></div>\r\n\t\t\t\t\t\t<ul v-show=\"hasArmorCrushables\">\r\n\t\t\t\t\t\t\t<li v-for=\"li in calcArmorResults.armorsCrushable\">{{ li.name }}</li>\r\n\t\t\t\t\t\t</ul>\r\n\t\t\t\t\t</div>\r\n\t\t\t\t\t\r\n\t\t\t\t</div>\r\n\t\t\t</div>\r\n\t\t</div>\r\n\t\r\n\t<div class=\"focus-value-bar\"><input type=\"text\" :value=\"focusValueText\" readonly></input></div>\r\n</div>";
	}
	,_Init: function() {
		var cls = troshx_sos_vue_InventoryVueArmor;
		var clsP = cls.prototype;
		troshx_sos_vue_InventoryVue.prototype._Init.call(this);
		this.template = this.Template();
	}
	,__class__: troshx_sos_vue_InventoryVueArmor
});
var troshx_sos_vue_RangeCalculator = function() {
	haxevx_vuex_core_VComponent.call(this);
	this.mixins = [troshx_sos_vue_input_MixinInput.getInstance()];
};
$hxClasses["troshx.sos.vue.RangeCalculator"] = troshx_sos_vue_RangeCalculator;
troshx_sos_vue_RangeCalculator.__name__ = ["troshx","sos","vue","RangeCalculator"];
troshx_sos_vue_RangeCalculator.__super__ = haxevx_vuex_core_VComponent;
troshx_sos_vue_RangeCalculator.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	Data: function() {
		return new troshx_sos_vue_RangeCalculatorData();
	}
	,Components: function() {
		var _m_ = { };
		_m_["inventory"] = new troshx_sos_vue_InventoryVueArmor();
		_m_["tree-browser"] = new troshx_sos_vue_widgets_GingkoTreeBrowser();
		return _m_;
	}
	,get_chanceToAtLeastGraze: function() {
		return troshx_util_TROSAI.getAtLeastXSuccessesProb(this.shooterTotalMP,this.shooterTN,this.targetTotalRS);
	}
	,get_chanceToAtLeastGrazeAtLeastOnce: function() {
		var a = this.chanceToAtLeastGraze;
		var b = this.chanceToAtLeastGraze;
		return a + b - a * b;
	}
	,get_chanceToAtLeastGrazeTwice: function() {
		return this.chanceToAtLeastGraze * this.chanceToAtLeastGraze;
	}
	,roundPerc: function(val) {
		return Math.round(val * 100);
	}
	,get_chanceToHitFully: function() {
		return troshx_util_TROSAI.getAtLeastXSuccessesProb(this.shooterTotalMP,this.shooterTN,this.targetTotalRS + 1);
	}
	,get_chanceToHitFullyAtLeastOnce: function() {
		var a = this.chanceToHitFully;
		var b = this.chanceToHitFully;
		return a + b - a * b;
	}
	,get_chanceToHitFullyTwice: function() {
		return this.chanceToHitFully * this.chanceToHitFully;
	}
	,get_grazePercMargin: function() {
		return this.roundPerc(this.chanceToAtLeastGraze) - this.roundPerc(this.chanceToHitFully);
	}
	,get_lowShieldCoverage: function() {
		return troshx_sos_core_Shield.getLowCoverage();
	}
	,get_highShieldCoverage: function() {
	...
	,get_hitLocationArmorValues: function() {
		...
		var nonFirearmMissile = !this.shooterFirearm;
		var _g1 = 0;
		var _g = ch.length;
		while(_g1 < _g) {
			var i = _g1++;
	...
	,get_rangedOutfitChanceToBreach: function() {
		var hitAVs = this.hitLocationArmorValues;
		var hitFields = Reflect.fields(hitAVs);
		var lowestAV = 999999999;
		var _g1 = 0;
		var _g = hitFields.length;
		while(_g1 < _g) {
			var i = _g1++;
			var f = hitFields[i];
			var av = hitAVs[f].avp;
		...
		var shieldPosition = heldShield != null ? inventory.shieldPosition : 0;
		var si = shieldSize == 0 ? 0 : shieldSize - 1;
		var coverageTruth = shieldPosition != 1 ? this.lowShieldCoverage[si] : this.highShieldCoverage[si];
		var totalMP = this.shooterTotalMP;
		var tn = this.shooterTN;
		var targetRS = this.targetTotalRS;
		...
		while(_g11 < _g2) {
			var i1 = _g11++;
			var tz = targetZones[i1];
			var _g3 = 0;
			var _g21 = tz.parts.length;
			while(_g3 < _g21) {
				var p = _g3++;
	...
	,get_chanceToBypassShield: function() {
		var shieldSize = this.targetShield;
		var shieldPosition = this.targetShieldPosition;
		var si = shieldSize == 0 ? 0 : shieldSize - 1;
		var coverageTruth = shieldPosition != 1 ? this.lowShieldCoverage[si] : this.highShieldCoverage[si];
		var body = troshx_sos_core_BodyChar.getInstance();
		var targetZones = body.targetZones;
		var totalProb = 1;
		var _g1 = 1;
		var _g = body.missileHitLocations.length;
		while(_g1 < _g) {
			var i = _g1++;
			var tz = targetZones[i];
			var _g3 = 0;
			var _g2 = tz.parts.length;
			while(_g3 < _g2) {
				var p = _g3++;
				var prob = tz.partWeights[p] / tz.weightsTotal;
				var hitLoc = body.hitLocations[tz.parts[p]];
				prob *= coverageTruth[hitLoc.id] != null ? coverageTruth[hitLoc.id] ? 0 : 0.5 : 1;
				totalProb += prob;
			}
		}
		if(shieldSize > 0) {
	...
	,get_shooterTotalMP: function() {
		return this.shooterProf + this.aimBonus + this.shooterMiscMPMod;
	}
	,get_breachArmorRSChance: function() {
		var inven = this.targetInventory;
		var detailResult = 0;
		if(inven != null) {
			detailResult = this.rangedOutfitChanceToBreach;
		}
		var plainResult = troshx_util_TROSAI.getAtLeastXSuccessesProb(this.shooterTotalMP,this.shooterTN,this.breachArmorRS);
		if(inven != null) {
			return detailResult;
		} else {
			return plainResult;
	...
	,get_breachArmorRSChanceAtLeastOnce: function() {
		var a = this.breachArmorRSChance;
		var b = this.breachArmorRSChance;
		return a + b - a * b;
	}
	,get_breachArmorRSChanceTwice: function() {
		return this.breachArmorRSChance * this.breachArmorRSChance;
	}
	,get_breachArmorRS: function() {
		return this.targetTotalRS + this.additionalRSForAV;
	}
	,get_aimBonus: function() {
		return Math.floor(this.shooterAim * 2 * this.aimMultiplier);
	}
	,get_targetTotalRS: function() {
		return this.targetMiscRSMod + this.targetBehaviourRS + this.targetCoverRS + this.targetDistanceRS + 4;
	}
	,get_targetDistanceRS: function() {
		var q = this.targetDistance / this.shooterRange;
		if(this.distRounding == 1) {
			return Math.round(q);
		} else if(this.distRounding == 2) {
			return Math.ceil(q);
		} else {
			return Math.floor(q);
		}
	}
	,get_aimMultiplier: function() {
		if(this.shooterRapidShot) {
			return 0.5;
		} else {
			return 1;
		}
	...
	,get_domainId: function() {
		return troshx_sos_vue_Globals.DOMAIN_RANGECALC;
	}
	,loadCharacterClipboardWindow: function() {
		if(this.emptyClipboard) {
			this.avValueToOvercome = 0;
			this.targetInventory = new troshx_sos_core_Inventory();
	...
	,loadCharContents: function(contents) {
		...
		if(!js_Boot.__instanceof(newItem,troshx_sos_sheets_CharSheet) && !js_Boot.__instanceof(newItem,troshx_sos_core_Inventory)) {
			console.log(newItem);
			js_Browser.alert("Sorry, unserialized type isn't CharSheet/Inventory!");
			return false;
		}
		var me = js_Boot.__instanceof(newItem,troshx_sos_sheets_CharSheet) ? newItem : null;
		if(me != null) {
		...
		var inv = js_Boot.__instanceof(newItem,troshx_sos_core_Inventory) ? newItem : null;
		if(inv != null) {
	...
	,get_emptyClipboard: function() {
		return StringTools.trim(this.clipboardLoadContents) == "";
	}
	,get_availableTypes: function() {
		return { "troshx.sos.sheets.CharSheet" : true, "troshx.sos.core.Inventory" : true};
	}
	,get_breachLabel: function() {
...
var troshx_sos_vue_RangeCalculatorData = function() {
	this.roundingOptions = [{ label : "Round Down", value : 0},{ label : "Round Off", value : 1},{ label : "Round Up", value : 2}];
	this.coverOptions = [{ label : "None", value : 0},{ label : "Half", value : 2},{ label : "3/4", value : 3},{ label : "Murderhole", value : 4}];
	this.treeBrowserInited = false;
	this.clipboardLoadContents = "";
	this.autoLoadChar = troshx_sos_vue_Globals.AUTO_LOAD;
	this.avValueToOvercome = 0;
	...
	this.shooterRange = 1;
	this.shooterRapidShot = false;
	this.shooterMiscMPMod = 0;
	this.shooterTN = 7;
	this.shooterAim = 0;
	this.shooterProf = 1;
};
$hxClasses["troshx.sos.vue.RangeCalculatorData"] = troshx_sos_vue_RangeCalculatorData;
troshx_sos_vue_RangeCalculatorData.__name__ = ["troshx","sos","vue","RangeCalculatorData"];
troshx_sos_vue_RangeCalculatorData.prototype = {
	__class__: troshx_sos_vue_RangeCalculatorData
};
...
troshx_sos_vue_TDHands.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	Data: function() {
		return { handOffCache : null};
	}
	,get_isTwoHandedHandOff: function() {
		var weap = this.entry.weapon;
		var a = weap.meleeFlags;
		var b = weap.flags;
		var c = !weap.ranged && !weap.isAmmo;
		if(c && (a & 128) != 0) {
	...
	,watch_isTwoHandedHandOff: function(newVal) {
		if(!newVal) {
			if(this.entry.weapon.ranged) {
				this.handOffCache = null;
				this.entry.weapon.variant = null;
				this.entry.holding1H = false;
			} else {
				this.handOffCache = this.entry.weapon.variant;
				this.entry.weapon.variant = null;
			}
		} else if(this.entry.weapon.variant == null) {
			this.entry.weapon.variant = this.handOffCache;
		}
	}
	,get_gotVariant: function() {
		var a = this.entry.holding1H;
	...
	,onSelectChange: function(e,weapon) {
		...
		if(this.isTwoHandedHandOff) {
			if(this.entry.weapon.variant == null) {
				this.entry.weapon.variant = new troshx_sos_core_Weapon(this.entry.weapon.id,this.entry.weapon.name);
				this.entry.weapon.variant.profs = troshx_sos_core_Profeciency.matchHandOffProfs(this.entry.weapon.profs);
			}
			if(val == 1) {
				this.entry.holding1H = false;
			} else {
				this.entry.holding1H = true;
			}
		} else if(val == 1) {
	...
	,_Init: function() {
		...
		this.data = clsP.Data;
		this.template = this.Template();
		this.computed = { isTwoHandedHandOff : clsP.get_isTwoHandedHandOff, gotVariant : clsP.get_gotVariant, isTwoHanded : clsP.get_isTwoHanded};
		this.methods = { get_isTwoHandedHandOff : clsP.get_isTwoHandedHandOff, watch_isTwoHandedHandOff : clsP.watch_isTwoHandedHandOff, get_gotVariant : clsP.get_gotVariant, get_isTwoHanded : clsP.get_isTwoHanded, onSelectChange : clsP.onSelectChange, shiftIndex : clsP.shiftIndex, emit : clsP.emit};
		this.props = { entry : { }};
		this.watch = { isTwoHandedHandOff : clsP.watch_isTwoHandedHandOff};
	}
...
var troshx_sos_vue_TDWeapProfSelect = function() {
	haxevx_vuex_core_VComponent.call(this);
	this["mixins"] = [troshx_sos_vue_widgets_MeleeVariantMixin.getInstance()];
};
...
troshx_sos_vue_TDWeapProfSelect.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,get_gotVariant: function() {
		var weaponAssign = this.weaponAssign;
		var result = false;
	...
	,get_weaponVar: function() {
		if(this.gotVariant) {
			return this.weaponAssign.weapon.variant;
		} else {
			return this.weapon;
		}
	}
	,get_gotCustomMultiCheck: function() {
		var weap = this.weaponVar;
		var a;
		if(weap.profs != 0) {
			var x = weap.profs;
			a = (x & x - 1) != 0;
	...
	,_Init: function() {
		...
		this.mounted = clsP.Mounted;
		this.template = this.Template();
		this.computed = { hasNoProf : clsP.get_hasNoProf, weaponOptionValue : clsP.get_weaponOptionValue, gotVariant : clsP.get_gotVariant, weaponVar : clsP.get_weaponVar, gotCustomMultiCheck : clsP.get_gotCustomMultiCheck, showSelectMultipleFirst : clsP.get_showSelectMultipleFirst, multiSelectedOptionLabel : clsP.get_multiSelectedOptionLabel, isSelectingMultiple : clsP.get_isSelectingMultiple, gotCustom : clsP.get_gotCustom};
		this.methods = { shiftIndex : clsP.shiftIndex, emit : clsP.emit, get_hasNoProf : clsP.get_hasNoProf, get_weaponOptionValue : clsP.get_weaponOptionValue, watch_weaponOptionValue : clsP.watch_weaponOptionValue, get_gotVariant : clsP.get_gotVariant, get_weaponVar : clsP.get_weaponVar, get_gotCustomMultiCheck : clsP.get_gotCustomMultiCheck, get_showSelectMultipleFirst : clsP.get_showSelectMultipleFirst, get_multiSelectedOptionLabel : clsP.get_multiSelectedOptionLabel, get_isSelectingMultiple : clsP.get_isSelectingMultiple, get_gotCustom : clsP.get_gotCustom, onProfSelectChange : clsP.onProfSelectChange, requestCurWidget : clsP.requestCurWidget};
		this.props = { rangedProfs : { required : true, type : Array}, profMask : { required : false, "default" : 0, type : Number}, weaponAssign : { required : false}, curWidgetRequest : { required : true}, meleeProfs : { required : true, type : Array}, entry : { required : true}, customProfs : { required : false, type : Array}, section : { required : true, type : String}, index : { required : true, type : Number}, isVisibleWidget : { required : true, type : Function}, weapon : { required : true, type : Object}};
...
troshx_sos_vue_uifields_ArrayOf.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,pushEntry: function() {
		...
		if(this.selectedIndex < -1) {
			...
			arr.splice(this.selectedIndex,0,valueToUse);
		}
	}
	...
	,popEntry: function() {
		if(this.selectedIndex < 0) {
			...
			this.current.splice(this.selectedIndex,1);
		}
	}
	,get_current: function() {
		return this.obj[this.prop];
	...
	,get_arrayLength: function() {
		return this.current.length;
	}
	,watch_arrayLength: function(newVal,oldVal) {
	...
	,_Init: function() {
		...
		this.components = this.Components();
		this.template = this.Template();
		this.computed = { current : clsP.get_current, typeMap : clsP.get_typeMap, arrayLength : clsP.get_arrayLength};
		this.methods = { getKey : clsP.getKey, clearSelect : clsP.clearSelect, pushEntry : clsP.pushEntry, watch_maxLength : clsP.watch_maxLength, popEntry : clsP.popEntry, get_current : clsP.get_current, get_typeMap : clsP.get_typeMap, get_arrayLength : clsP.get_arrayLength, watch_arrayLength : clsP.watch_arrayLength};
		...
		this.watch = { maxLength : clsP.watch_maxLength, arrayLength : clsP.watch_arrayLength};
	}
...
var troshx_sos_vue_uifields_UIMixin = function() {
	haxevx_vuex_core_VComponent.call(this);
};
$hxClasses["troshx.sos.vue.uifields.UIMixin"] = troshx_sos_vue_uifields_UIMixin;
troshx_sos_vue_uifields_UIMixin.__name__ = ["troshx","sos","vue","uifields","UIMixin"];
troshx_sos_vue_uifields_UIMixin.getInstance = function() {
	if(troshx_sos_vue_uifields_UIMixin.INSTANCE != null) {
		return troshx_sos_vue_uifields_UIMixin.INSTANCE;
	} else {
		return troshx_sos_vue_uifields_UIMixin.INSTANCE = new troshx_sos_vue_uifields_UIMixin();
	}
};
troshx_sos_vue_uifields_UIMixin.__super__ = haxevx_vuex_core_VComponent;
troshx_sos_vue_uifields_UIMixin.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	Components: function() {
		return troshx_sos_vue_uifields_UI.getComponents();
	}
	,_Init: function() {
		var cls = troshx_sos_vue_uifields_UIMixin;
		var clsP = cls.prototype;
		this.components = this.Components();
	}
	,__class__: troshx_sos_vue_uifields_UIMixin
});
...
troshx_sos_vue_widgets_GingkoTreeBrowser.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,loadDomain: function(domainId) {
		...
		this.isLoading = true;
		this.loadedDomain = "";
		var http = new haxe_Http(troshx_sos_vue_Globals.CURL_DOMAIN + "/curlgink.php").setParameter("id",domainId);
...
troshx_sos_vue_widgets_InputName.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,get_label: function() {
		var lbl = this.item.get_label();
		if(this.weaponHeldOff != null) {
			return this.weapon.getLabelHeld(this.weaponHeldOff);
		} else {
			return lbl;
		}
	}
	...
	,watch_item: function(newVal) {
		var inputElem = this.$el;
		inputElem.value = newVal.get_label();
	}
	,setValidNameOfInput: function(inputElement) {
		var tarName = StringTools.trim(inputElement.value);
		var updated = false;
	...
	,_Init: function() {
		...
		this.mounted = clsP.Mounted;
		this.template = this.Template();
		this.computed = { label : clsP.get_label, weapon : clsP.get_weapon};
		this.methods = { emit : clsP.emit, get_label : clsP.get_label, get_weapon : clsP.get_weapon, watch_item : clsP.watch_item, setValidNameOfInput : clsP.setValidNameOfInput};
		this.props = { weaponHeldOff : { required : false, "default" : null, type : Boolean}, item : { type : Object}};
		this.watch = { item : clsP.watch_item};
	}
...
troshx_sos_vue_widgets_InputNameQty.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,watch_itemQty: function(newVal) {
		var inputElem = this.$el;
		inputElem.value = newVal.item.get_label();
	}
	,setValidNameOfInput: function(inputElement) {
		var tarName = StringTools.trim(inputElement.value);
		var updated = false;
	...
	,_Init: function() {
		...
		this.methods = { emit : clsP.emit, get_label : clsP.get_label, watch_itemQty : clsP.watch_itemQty, setValidNameOfInput : clsP.setValidNameOfInput};
		this.props = { customValidateName : { type : Function}, itemQty : { type : Object}};
		this.watch = { itemQty : clsP.watch_itemQty};
	}
	,__class__: troshx_sos_vue_widgets_InputNameQty
});
var troshx_sos_vue_widgets_MeleeVariantMixin = function() {
	haxevx_vuex_core_VComponent.call(this);
};
$hxClasses["troshx.sos.vue.widgets.MeleeVariantMixin"] = troshx_sos_vue_widgets_MeleeVariantMixin;
troshx_sos_vue_widgets_MeleeVariantMixin.__name__ = ["troshx","sos","vue","widgets","MeleeVariantMixin"];
troshx_sos_vue_widgets_MeleeVariantMixin.getInstance = function() {
	if(troshx_sos_vue_widgets_MeleeVariantMixin.INSTANCE != null) {
		return troshx_sos_vue_widgets_MeleeVariantMixin.INSTANCE;
	} else {
		return troshx_sos_vue_widgets_MeleeVariantMixin.INSTANCE = new troshx_sos_vue_widgets_MeleeVariantMixin();
	}
};
troshx_sos_vue_widgets_MeleeVariantMixin.inlineGotVariant = function(weaponAssign) {
	var result = false;
	if(weaponAssign != null) {
		var a = weaponAssign.weapon.variant != null;
		var b = weaponAssign.holding1H;
...
troshx_sos_vue_widgets_MeleeVariantMixin.__super__ = haxevx_vuex_core_VComponent;
troshx_sos_vue_widgets_MeleeVariantMixin.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	get_gotVariant: function() {
		var weaponAssign = this.weaponAssign;
		var result = false;
		if(weaponAssign != null) {
	...
	,_Init: function() {
		var cls = troshx_sos_vue_widgets_MeleeVariantMixin;
		var clsP = cls.prototype;
		this.computed = { gotVariant : clsP.get_gotVariant};
		this.methods = { get_gotVariant : clsP.get_gotVariant};
		this.props = { weaponAssign : { required : false}};
	}
	,__class__: troshx_sos_vue_widgets_MeleeVariantMixin
});
...
troshx_sos_vue_widgets_SelectHeld.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,get_twoHanded: function() {
		var stdH = (this.item.flags & 1) != 0;
		var b = this.gotVariant && (this.weaponAssign.holding1H && this.weaponAssign.weapon.variant != null);
		if(b) {
			return false;
		} else {
			return stdH;
	...
	,_Init: function() {
		...
		var clsP = cls.prototype;
		this.template = this.Template();
		this.computed = { gotVariant : clsP.get_gotVariant, item : clsP.get_item, twoHanded : clsP.get_twoHanded};
		this.methods = { get_gotVariant : clsP.get_gotVariant, get_item : clsP.get_item, get_twoHanded : clsP.get_twoHanded, holdItemHandler : clsP.holdItemHandler, shiftIndex : clsP.shiftIndex, emit : clsP.emit};
		this.props = { inventory : { type : Object}, weaponAssign : { required : false}, entry : { }};
	}
...
var troshx_sos_vue_widgets_WMeleeAtk = function() {
	haxevx_vuex_core_VComponent.call(this);
	this.mixins = [troshx_sos_vue_input_MixinInput.getInstance(),troshx_sos_vue_widgets_MeleeVariantMixin.getInstance()];
};
...
troshx_sos_vue_widgets_WMeleeAtk.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,_Init: function() {
		...
		this.props = { item : { required : true, type : Object}, entryIndex : { required : false, type : Number}, weaponAssign : { required : false}, parentAttr : { required : false}, entry : { required : true}, thrusting : { required : true, type : Boolean}};
	}
...
troshx_sos_vue_widgets_WProf.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,confirm: function() {
		...
		if(this.ranged) {
			...
			weap.variant = null;
			this.weaponAssign.holding1H = false;
	...
	,_Init: function() {
		...
		this.props = { item : { required : true, type : Object}, rangedProfs : { required : true, type : Array}, entryIndex : { required : false, type : Number}, weaponAssign : { required : true}, parentAttr : { required : false}, curWidgetRequest : { required : false}, meleeProfs : { required : true, type : Array}, customWidth : { required : false, "default" : false, type : Boolean}, entry : { required : true}, customProfs : { required : false, type : Array}};
	}
...
var troshx_sos_vue_widgets_WTags = function() {
	haxevx_vuex_core_VComponent.call(this);
	this.mixins = [troshx_sos_vue_input_MixinInput.getInstance(),troshx_sos_vue_uifields_UIMixin.getInstance(),troshx_sos_vue_widgets_MeleeVariantMixin.getInstance()];
};
...
troshx_sos_vue_widgets_WTags.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,Data: function() {
		return { meleeSpecialCache : null, missileSpecialCache : null, armorSpecialCache : null, armorBurdinadinCache : null, armorSpecialHitModifierCache : null, customise : null, customMeleeCache : null, customiseArmor : null, restoreOriginal : false, armorWornWith : null, armorCustomiseNotesCache : null};
	}
	,Mounted: function() {
	...
	,get_weaponSpec: function() {
		var weaponVar = this.gotVariant ? this.weaponAssign.weapon.variant : null;
		if(weaponVar != null) {
			return weaponVar;
		} else {
			return this.weapon;
	...
	,onArmorCustomiseNotesCheck: function(cb) {
		var armor = this.armor;
		if(cb.checked) {
			if(!Object.prototype.hasOwnProperty.call(armor.customise,"notes")) {
				Vue.set(armor.customise,"notes",[]);
			} else {
				armor.customise.notes = armor.customise.notes != null ? armor.customise.notes : this.armorCustomiseNotesCache != null ? this.armorCustomiseNotesCache : [];
			}
		} else {
			this.armorCustomiseNotesCache = armor.customise.notes;
			armor.customise.notes = null;
		}
	}
	,onArmorBurdinadinCheck: function(cb) {
		var armor = this.armor;
		if(cb.checked) {
			armor.burdinadin = armor.burdinadin != null ? armor.burdinadin : this.armorBurdinadinCache != null ? this.armorBurdinadinCache : new troshx_sos_core_BurdinadinArmor();
		} else {
			this.armorBurdinadinCache = armor.burdinadin;
			armor.burdinadin = null;
		}
	}
	...
	,onWeapSpecialCheck: function(cb,ranged) {
		...
		if(cb.checked) {
			if(ranged) {
				...
				weapon.meleeSpecial = weapon.meleeSpecial != null ? weapon.meleeSpecial : this.meleeSpecialCache != null ? this.meleeSpecialCache : new troshx_sos_core_MeleeSpecial();
			}
		} else if(ranged) {
...
troshx_util_TROSAI.getAtLeastXSuccessesProb = function(numDiceToRoll,tn,x) {
	...
	while(x <= numDiceToRoll) {
		...
		if(x > 0) {
			...
			var v2 = val2;
			while(--v2 > 1) val2 *= v2;
			accum1 = accum2 / (accum3 * val2);
		} else {
			accum1 = 1;
		}
		accum += accum1 * Math.pow(prob,x) * Math.pow(1 - prob,numDiceToRoll - x);
		++x;
	}
	if(accum > 1) {
		accum = 1;
...
troshx_util_TROSAI.getChanceToSucceedContest = function(numDice,tn,againstNumDice,againstTN,rs,requireAtLeast1TS) {
	...
	var accum = 0;
	var start = requireAtLeast1TS && rs < 1 ? 1 : rs;
	var i = start;
	var p = 0;
	while(i <= numDice) {
		var k = i - rs;
		...
		while(k >= 0) {
			...
			var p4 = p1 * Math.pow(prob,i) * Math.pow(1 - prob,numDice - i);
			...
			p += p4 * (p5 * Math.pow(prob1,k) * Math.pow(1 - prob1,againstNumDice - k));
			--k;
		}
		++i;
	}
...
troshx_util_TROSAI.getTabulatedRollData = function(numDice,tn) {
	...
	while(i >= 0) {
		...
		var v6 = v * Math.pow(prob,i) * Math.pow(1 - prob,numDice - i);
		accum += v6;
		arr.push({ x : i, gte : accum, eq : v6});
		--i;
	}
	arr.reverse();
	var _g1 = 0;
...
troshx_sos_vue_CharGen.MAX_SAVED_CHARS = 8;
troshx_sos_vue_CharGen.SAVED_CHARS_KEY = "generatedCharacters";
troshx_sos_vue_CharSheetVue.SESSION_KEY = "lastSavedCharacter";
...
troshx_sos_vue_Globals.DOMAIN_RANGECALC = "sos-sample-characters";
...
troshx_sos_vue_RangeCalculatorData.COVER_NONE = 0;
troshx_sos_vue_RangeCalculatorData.COVER_HALF = 2;
troshx_sos_vue_RangeCalculatorData.COVER_34 = 3;
troshx_sos_vue_RangeCalculatorData.COVER_MURDERHOLE = 4;
troshx_sos_vue_RangeCalculatorData.BASE_RS = 4;

...
troshx_sos_vue_combat_LayoutConstraints.applyDollView = function(layoutItems,names,tags,refWidth,refHeight) {
	...
	while(_g1 < _g) {
		...
		if(tag == "part" || tag == "swing" || name == "enemyHandLeft" || name == "enemyHandRight" || name == "enemyStatus") {
			item.pin(troshx_util_layout_PointScaleConstraint.createRelative(0.5,0.5).scaleMaxRelative(1.5,1.5).scaleMinRelative(0.5,0.5)).aspect(troshx_util_layout_AspectConstraint.createRelative(1,1).enablePreflight());
		}
		switch(name) {
		case "advManuever1":case "advManuever2":case "advManuever3":case "advManuever4":
			item.pin(troshx_util_layout_PointScaleConstraint.createRelative(0.5,0.5).scaleMinRelative(1,0).scaleMaxRelative(1.5,1.5)).pivot(troshx_util_layout_PointScaleConstraint.createRelative(1,0.5).scaleMinRelative(0.5,0.5).scaleMaxRelative(1.5,1.5)).aspect(troshx_util_layout_AspectConstraint.createRelative(1,0.5));
			break;
		case "initRange":
			item.pin(troshx_util_layout_PointScaleConstraint.createRelative(0.5,0.5).scaleMinRelative(1,0).scaleMaxRelative(1,1000)).pivot(troshx_util_layout_PointScaleConstraint.createRelative(0.5,1)).aspect(troshx_util_layout_AspectConstraint.createRelative(1,1).enablePreflight());
			break;
		default:
		}
	}

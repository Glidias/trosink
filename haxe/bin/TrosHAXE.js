...
var troshx_sos_core_Inventory = function() {
	this.strappedExtraItemOff = null;
	this.strappedExtraItem = null;
	this.miscItemHand = null;
	this.miscItemOffHand = null;
	this.shield = null;
	this.shieldOffHand = null;
	this.strappedExtraWeaponsOff = null;
	this.strappedExtraWeapons = null;
...
troshx_sos_core_Inventory.getEmptyReadyAssign = function(type) {
	if(type == "weapon") {
		return { weapon : new troshx_sos_core_Weapon(), held : 0, unheld : 0, unheldRemark : ""};
	} else if(type == "shield") {
		return { shield : new troshx_sos_core_Shield(), held : 0, unheld : 0, unheldRemark : ""};
	} else {
		return { item : new troshx_sos_core_Item(), held : 0, unheld : 0, unheldRemark : ""};
	}
};
troshx_sos_core_Inventory.prototype = {
	_shiftItem: function(item,preferedUnheld,qty) {
		if(qty == null) {
			qty = 1;
		}
		if(qty <= 0) {
		...
		if(preferedUnheld > 0) {
			if(preferedUnheld != 4) {
				if(preferedUnheld == 1) {
					this.packed.add(new troshx_sos_core_ItemQty(item,qty));
				} else if(preferedUnheld == 2) {
					this.dropped.add(new troshx_sos_core_ItemQty(item,qty));
				} else {
	...
	,dropEquipedShield: function(alreadyEquiped,doDestroy) {
		...
		if(!doDestroy) {
			this._shiftItem(alreadyEquiped.shield,2);
		}
	}
	,dropMiscItem: function(alreadyEquiped,doDestroy) {
		...
		this.equipedNonMeleeItems.splice(this.equipedNonMeleeItems.indexOf(alreadyEquiped),1);
		if(!doDestroy) {
			this._shiftItem(alreadyEquiped.item,2);
		}
	}
	,dropEquipedWeapon: function(alreadyEquiped,doDestroy) {
		...
		this.weapons.splice(this.weapons.indexOf(alreadyEquiped),1);
		if(!doDestroy) {
			this._shiftItem(alreadyEquiped.weapon,2);
		}
	}
	,dropWornArmor: function(armor,doDestroy) {
		...
		if(!doDestroy) {
			this._shiftItem(armor,2);
		}
	}
	,packEquipedShield: function(alreadyEquiped) {
		this.shields.splice(this.shields.indexOf(alreadyEquiped),1);
		this._shiftItem(alreadyEquiped.shield,1);
	}
	,packMiscItem: function(alreadyEquiped) {
		this.equipedNonMeleeItems.splice(this.equipedNonMeleeItems.indexOf(alreadyEquiped),1);
		this._shiftItem(alreadyEquiped.item,1);
	}
	,packEquipedWeapon: function(alreadyEquiped) {
		this.weapons.splice(this.weapons.indexOf(alreadyEquiped),1);
		this._shiftItem(alreadyEquiped.weapon,1);
	}
	,packWornArmor: function(armor) {
		this.wornArmor.splice(this.wornArmor.indexOf(armor),1);
		this._shiftItem(armor,1);
	}
	...
	,equipItem: function(item,unheldRemark) {
		...
		if(js_Boot.__instanceof(item,troshx_sos_core_Weapon)) {
			readyAssign = { weapon : js_Boot.__instanceof(item,troshx_sos_core_Weapon) ? item : null, held : 0, unheld : 4, unheldRemark : unheldRemark};
			this.weapons.push(readyAssign);
		} else if(js_Boot.__instanceof(item,troshx_sos_core_Shield)) {
			readyAssign = { shield : js_Boot.__instanceof(item,troshx_sos_core_Shield) ? item : null, held : 0, unheld : 4, unheldRemark : unheldRemark};
			this.shields.push(readyAssign);
		} else if(js_Boot.__instanceof(item,troshx_sos_core_Armor)) {
			this.wornArmor.push(js_Boot.__instanceof(item,troshx_sos_core_Armor) ? item : null);
		} else {
			readyAssign = { item : item, held : 0, unheld : 4, unheldRemark : unheldRemark};
...
troshx_sos_vue_CharSheetVue.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,executeEquipEntry: function(equipEntry,typeId) {
		if(equipEntry.isValid()) {
			...
			equipEntry.reset(troshx_sos_core_Inventory.getEmptyReadyAssign(typeId));
			return true;
		}
		return false;
	}
	,executeArmorEntry: function() {
		if(this.armorEntry.isValid()) {
			this["char"].inventory.wornArmor.push(this.armorEntry.e);
	...
	,on_packedEntryGotFocus: function(newValue,oldValue) {
		if(!newValue) {
			this.executeQtyEntry(this.packedEntry,this["char"].inventory.packed);
		}
	}
	,on_droppedEntryGotFocus: function(newValue,oldValue) {
		if(!newValue) {
			this.executeQtyEntry(this.droppedEntry,this["char"].inventory.dropped);
		}
	}
	...
	,_Init: function() {
		...
		this.computed = { isValidDroppedEntry : clsP.get_isValidDroppedEntry, isValidPackedEntry : clsP.get_isValidPackedEntry, droppedEntryGotFocus : clsP.get_droppedEntryGotFocus, packedEntryGotFocus : clsP.get_packedEntryGotFocus, miscEntryGotFocus : clsP.get_miscEntryGotFocus, weaponEntryGotFocus : clsP.get_weaponEntryGotFocus, shieldEntryGotFocus : clsP.get_shieldEntryGotFocus, armorEntryGotFocus : clsP.get_armorEntryGotFocus};
		this.methods = { loadSheet : clsP.loadSheet, saveSheet : clsP.saveSheet, getValidName : clsP.getValidName, setValidNameOfInput : clsP.setValidNameOfInput, focusOutRowField : clsP.focusOutRowField, focusInRowField : clsP.focusInRowField, executeQtyEntry : clsP.executeQtyEntry, executeEquipEntry : clsP.executeEquipEntry, executeArmorEntry : clsP.executeArmorEntry, holdItemHandler : clsP.holdItemHandler, get_isValidDroppedEntry : clsP.get_isValidDroppedEntry, get_isValidPackedEntry : clsP.get_isValidPackedEntry, get_droppedEntryGotFocus : clsP.get_droppedEntryGotFocus, get_packedEntryGotFocus : clsP.get_packedEntryGotFocus, get_miscEntryGotFocus : clsP.get_miscEntryGotFocus, get_weaponEntryGotFocus : clsP.get_weaponEntryGotFocus, get_shieldEntryGotFocus : clsP.get_shieldEntryGotFocus, get_armorEntryGotFocus : clsP.get_armorEntryGotFocus, on_packedEntryGotFocus : clsP.on_packedEntryGotFocus, on_droppedEntryGotFocus : clsP.on_droppedEntryGotFocus, on_shieldEntryGotFocus : clsP.on_shieldEntryGotFocus, on_weaponEntryGotFocus : clsP.on_weaponEntryGotFocus, on_miscEntryGotFocus : clsP.on_miscEntryGotFocus, on_armorEntryGotFocus : clsP.on_armorEntryGotFocus};
		this.watch = { packedEntryGotFocus : clsP.on_packedEntryGotFocus, droppedEntryGotFocus : clsP.on_droppedEntryGotFocus, shieldEntryGotFocus : clsP.on_shieldEntryGotFocus, weaponEntryGotFocus : clsP.on_weaponEntryGotFocus, miscEntryGotFocus : clsP.on_miscEntryGotFocus, armorEntryGotFocus : clsP.on_armorEntryGotFocus};
	}
...
var troshx_sos_vue_CharSheetVueData = function() {
	...
	this.armorEntry = new troshx_sos_vue_ArmorEntry();
...
var troshx_sos_vue_ArmorEntry = function() {
	this.focusedFlags = 0;
	this.e = new troshx_sos_core_Armor();
};
$hxClasses["troshx.sos.vue.ArmorEntry"] = troshx_sos_vue_ArmorEntry;
troshx_sos_vue_ArmorEntry.__name__ = ["troshx","sos","vue","ArmorEntry"];
troshx_sos_vue_ArmorEntry.__interfaces__ = [troshx_sos_vue_IFocusFlags,troshx_ds_IValidable];
troshx_sos_vue_ArmorEntry.prototype = {
	reset: function() {
		this.e = new troshx_sos_core_Armor();
	}
	,isValid: function() {
	...
	,__class__: troshx_sos_vue_ArmorEntry
};
...
troshx_sos_vue_RowReadyEntry.prototype = {
	...
	,setupValidable: function() {
		if(Object.prototype.hasOwnProperty.call(this.e,"weapon")) {
			...
			var obj2 = Reflect.field(this.e,"item");
			this.itemToValidate = js_Boot.__instanceof(obj2,troshx_sos_core_Item) ? obj2 : null;
		} else {
	...
	,isValid: function() {
		...
		if(this.itemToValidate.name != null) {
			return this.itemToValidate.name != "";
		} else {
			return false;
		}
	}

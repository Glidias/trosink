...
troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline = function(availableCP,tn,threshold,againstRoll,againstTN,useAllCP) {
	...
	while(_g1 < _g) {
		...
		accum = troshx_tros_ai_TROSAiBot.precisionPerc((successProbabilitWith1BS + troshx_util_TROSAI.getChanceToSucceedContest(c,tn,againstRoll,againstTN,0)) * 0.5);
		if(accum >= threshold) {
...
troshx_tros_ai_TROSAiBot.getComboExchangeBudgetingWithInitiative = function(combo,cp,cp2,threatManuever) {
	...
	case 2:
		if(troshx_tros_ai_TROSAiBot.getAForcefulInitiativeAttack(troshx_tros_ai_TROSAiBot.P_THRESHOLD_FAVORABLE,cp,cp2,dtn,true,false,0)) {
			var mean = cp * .5 | 0;
			if(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP <= mean) troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP = mean + 1;
			troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[0] = troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
			troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[1] = cp2;
			stipulateRemaining = 0;
			cp -= troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
			if(cp > 0 && troshx_tros_ai_TROSAiBot.getFBAttack(true,cp,stipulateRemaining,dtn,false,3)) {

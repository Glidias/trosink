			...
			}
			{
				...
				case 4:
					...
					while(_g1_head != null) {
						...
						Reflect.setField(funcDep.instance,funcArg.name,funcArg.opt?dat_gui_DatUtil.parseOptStringParam(funcArg.value,haxe_rtti_CTypeTools.toString(funcArg.t),classe):null);
						count++;
					}
...
troshx_util_AIManueverChoice.prototype = {
	...
	,setAttack: function(manuever,manueverCP,manueverTN,targetZone,offhand) {
	...
	,setDefend: function(manuever,manueverCP,manueverTN,offhand) {
		if(offhand == null) offhand = false;
		this.manuever = manuever;
		this.manueverCP = manueverCP;
		this.targetZone = 0;
...
troshx_tros_ai_TROSAiBot.setTypicalAVAILCostsForTesting = function() {
	...
	troshx_tros_ai_TROSAiBot.AVAIL_disarm = 3;
};
...
troshx_tros_ai_TROSAiBot.getRegularAttack = function(availableCP,roll,againstRoll,againstTN) {
	...
	var dmg;
	if(weapon != null && againstRoll >= 0) dmg = weapon.damage; else dmg = 0;
	var dmgT;
	if(weapon != null && againstRoll >= 0) dmgT = weapon.damage2; else dmgT = 0;
	var probabilityToHitSlash;
	if(againstRoll >= 0) probabilityToHitSlash = troshx_util_TROSAI.getChanceToSucceedContest(roll,tn,againstRoll,againstTN,1); else probabilityToHitSlash = 0;
	var probabilityToHitThrust;
	if(againstRoll >= 0) probabilityToHitThrust = troshx_util_TROSAI.getChanceToSucceedContest(roll,tn2,againstRoll,againstTN,1); else probabilityToHitThrust = 0;
	var aggrCur = -999;
...
troshx_tros_ai_TROSAiBot.getRegularDefense = function(availableCP,roll,enforceMustRegainInitiative) {
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_partialevasion > 0) {
		...
		if(cost + (enforceMustRegainInitiative?2:0) < availableCP) {
			aggr = 0.4 * (roll - cost - (enforceMustRegainInitiative?2:0));
			if(aggr >= aggrCur) {
				if(aggr != aggrCur) troshx_tros_ai_TROSAiBot.B_CANDIDATE_COUNT = 0;
...
troshx_tros_ai_TROSAiBot.checkCostViability = function(availableCP,tn,threshold,againstRoll,againstTN,useAllCP) {
	...
	while(_g1 < _g) {
		...
		if(accum >= threshold) {
			if(troshx_tros_ai_TROSAiBot.B_BS_REQUIRED == 1) troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY = accum; else troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY = troshx_util_TROSAI.getChanceToSucceedContest(c,tn,againstRoll,againstTN,1,true);
			return c;
		}
	}
	return 0;
...
troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline = function(availableCP,tn,threshold,againstRoll,againstTN,useAllCP) {
	...
	var accum;
	var min1;
	if(useAllCP) min1 = availableCP; else min1 = 1;
	var _g1 = min1;
	var _g = availableCP + 1;
	while(_g1 < _g) {
		var c = _g1++;
		var successProbabilitWith1BS = troshx_util_TROSAI.getChanceToSucceedContest(c,tn,againstRoll,againstTN,1,true);
		accum = troshx_tros_ai_TROSAiBot.precisionPerc((successProbabilitWith1BS + troshx_util_TROSAI.getChanceToSucceedContest(c,tn,againstRoll,againstTN,0,false)) * 0.5);
		if(accum >= threshold) {
			troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY = successProbabilitWith1BS;
...
troshx_tros_ai_TROSAiBot.getTheSuitableAttack = function(manuever,tn,tarZone,threshold,availableCP,againstRoll,againstTN,favorable,useAllCP) {
	...
	if(cpToUse > 0) {
		troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack(manuever,cpToUse,tn,tarZone,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
		return true;
	}
	return false;
};
troshx_tros_ai_TROSAiBot.getTheSuitableDefense = function(manuever,tn,threshold,availableCP,againstRoll,againstTN,favorable,useAllCP) {
	...
	if(favorable) cpToUse = troshx_tros_ai_TROSAiBot.checkCostViability(availableCP,tn,threshold,againstRoll,againstTN,useAllCP); else cpToUse = troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline(availableCP,tn,threshold,againstRoll,againstTN,useAllCP);
	if(cpToUse > 0) {
		troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend(manuever,cpToUse,tn,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
		return true;
	}
	return false;
};
...
troshx_tros_ai_TROSAiBot.getAForcefulInitiativeAttack = function(threshold,availableCP,againstCP,againstTN,favorable,useAllCP,offset) {
	...
	if(againstCP == null) againstCP = 0;
	troshx_tros_ai_TROSAiBot.B_BS_REQUIRED = troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DEFAULT;
	var manuever = troshx_tros_ai_TROSAiBot.getRegularAttack(availableCP,0,againstCP,againstTN);
	if(manuever != null) {
		availableCP -= troshx_tros_ai_TROSAiBot.getCostOfManuever(manuever);
		var tn = troshx_tros_ai_TROSAiBot.getATNOfManuever(manuever);
		var tarZone = troshx_tros_ai_TROSAiBot.getRegularTargetZone(manuever,tn,availableCP);
		if(tarZone != 0) return troshx_tros_ai_TROSAiBot.getTheForcefulInitiativeAttack(manuever,tn,tarZone,threshold,availableCP,againstCP,againstTN,favorable,useAllCP,offset);
	}
	return false;
};
...
troshx_tros_ai_TROSAiBot.getFavorableAttack = function(availableCP,againstRoll,againstTN,heuristic,flags) {
	if(flags == null) flags = 0;
	if(heuristic == null) heuristic = true;
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return troshx_tros_ai_TROSAiBot.getFBAttack(true,availableCP,againstRoll,againstTN,heuristic,flags);
};
troshx_tros_ai_TROSAiBot.getBorderlineAttack = function(availableCP,againstRoll,againstTN,heuristic,flags) {
	if(flags == null) flags = 0;
	if(heuristic == null) heuristic = true;
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return troshx_tros_ai_TROSAiBot.getFBAttack(false,availableCP,againstRoll,againstTN,heuristic,flags);
};
			...
			while(_g1 < _g) {
				...
				if(cpToUse > 0) {
					...
					if(aggr <= curAggr) {
						...
						troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack("bash",cpToUse,tn,i,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
		...
		if(cp > 0) {
			...
			while(_g11 < _g2) {
				...
				if(cpToUse > 0) {
					...
					if(aggr <= curAggr) {
						if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = 0;
						troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack("spike",cpToUse,tn,i1,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
						troshx_tros_ai_TROSAiBot.addPossibleRegularManueverChoice(troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT);
						troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_PROBABILITIES[troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT] = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
						troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT++;
						curAggr = aggr;
		...
		if(cp > 0) {
			...
			while(_g12 < _g3) {
				...
				if(cpToUse > 0) {
					...
					if(aggr <= curAggr) {
						if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = 0;
						troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack("cut",cpToUse,tn,i2,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
						troshx_tros_ai_TROSAiBot.addPossibleRegularManueverChoice(troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT);
						troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_PROBABILITIES[troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT] = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
						troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT++;
						curAggr = aggr;
		...
		if(cp > 0) {
			...
			while(_g13 < _g4) {
				...
				if(cpToUse > 0) {
					...
					if(aggr <= curAggr) {
						if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = 0;
						troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack("thrust",cpToUse,tn,i3,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
						troshx_tros_ai_TROSAiBot.addPossibleRegularManueverChoice(troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT);
						troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_PROBABILITIES[troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT] = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
						troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT++;
						curAggr = aggr;
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_block > 0) {
		...
		if(cp > 0) {
			...
			if(cpToUse > 0) {
				...
				if(aggr <= curAggr) {
					if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = 0;
					troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend("block",cpToUse,tn,true);
					troshx_tros_ai_TROSAiBot.addPossibleRegularManueverChoice(troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT);
					troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_PROBABILITIES[troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT] = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
					troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT++;
				}
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_parry > 0) {
		...
		if(cp > 0) {
			...
			if(cpToUse > 0) {
				...
				if(aggr <= curAggr) {
					if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = 0;
					troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend("parry",cpToUse,tn,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
					troshx_tros_ai_TROSAiBot.addPossibleRegularManueverChoice(troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT);
					troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_PROBABILITIES[troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT] = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
					troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT++;
				}
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_partialevasion > 0) {
		...
		if(cp > 0) {
			...
			if(cpToUse > 0) {
				...
				if(aggr <= curAggr) {
					if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = 0;
					troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend("partialevasion",cpToUse,tn,false);
					troshx_tros_ai_TROSAiBot.addPossibleRegularManueverChoice(troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT);
					troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_PROBABILITIES[troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT] = troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY;
					troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT++;
				}
...
troshx_tros_ai_TROSAiBot.getFleeOrDefend = function(favorable,availableCP,againstRoll,againstTN,heuristic,flags,customThreshold,secondExchange) {
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_fullevasion > 0) {
		...
		if(favorable) {
			...
			if(spend > 0) {
				troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend("fullevasion",spend,4,false);
				return true;
			}
		} else {
...
troshx_tros_ai_TROSAiBot.getBlockOpenAndStrike = function(availableCP,againstCP,againstRoll,againstTN) {
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return false;
};
troshx_tros_ai_TROSAiBot.getRotaOrCounter = function(availableCP,againstCP,againstRoll,againstTN) {
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return false;
};
troshx_tros_ai_TROSAiBot.getDisarm = function(favorable,availableCP,againstRoll,againstTN,flags,customThreshold,preferedRS,defensive) {
	...
	var threshold;
	if(customThreshold != 0) threshold = customThreshold; else if(favorable) threshold = troshx_tros_ai_TROSAiBot.P_THRESHOLD_FAVORABLE; else threshold = troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE;
	if(troshx_tros_ai_TROSAiBot.AVAIL_disarm > 0) {
		availableCP -= troshx_tros_ai_TROSAiBot.getCostOfManuever("disarm");
		if(availableCP > 0) {
			troshx_tros_ai_TROSAiBot.B_BS_REQUIRED = preferedRS;
			var tn;
			if(defensive) tn = troshx_tros_ai_TROSAiBot.getDTNOfManuever("disarm"); else tn = troshx_tros_ai_TROSAiBot.getATNOfManuever("disarm");
			if(tn == 0 || tn == 888) return false;
			var costing;
			if(favorable) costing = troshx_tros_ai_TROSAiBot.checkCostViability(availableCP,tn,threshold,againstRoll,againstTN,(flags & 2) != 0); else costing = troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline(availableCP,troshx_tros_ai_TROSAiBot.getDTNOfManuever("disarm"),threshold,againstRoll,againstTN,(flags & 2) != 0);
			troshx_tros_ai_TROSAiBot.B_BS_REQUIRED = troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DEFAULT;
			if(costing > 0) {
				if(defensive) troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend("disarm",costing,tn,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND); else troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack("disarm",costing,tn,0,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
				return true;
			}
		}
	}
...
troshx_tros_ai_TROSAiBot.getHook = function(availableCP,againstCP,againstRoll,againstTN) {
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return false;
};
troshx_tros_ai_TROSAiBot.getBeat = function(availableCP,againstCP,againstRoll,againstTN) {
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return false;
};
troshx_tros_ai_TROSAiBot.getBindStrike = function(availableCP,againstCP,againstRoll,againstTN) {
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	return false;
};
...
troshx_tros_ai_TROSAiBot.getComboExchangeBudgetingWithInitiative = function(combo,cp,cp2,threatManuever) {
	...
	case 1:
		...
		if(troshx_tros_ai_TROSAiBot.getFavorableAttack(Math.floor(cp * .5),stipulateRemaining,dtn,false,1) && cp - troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP >= Math.ceil(cp * .5) || troshx_tros_ai_TROSAiBot.getBorderlineAttack(Math.floor(cp * .5),stipulateRemaining,dtn,false,1)) {
			cp -= troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
			...
			troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[2] = troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
			troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[3] = stipulateRemaining;
			if(troshx_tros_ai_TROSAiBot.getBorderlineAttack(cp,troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[1],dtn,false,2)) return troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET;
		}
		break;
	case 2:
		if(troshx_tros_ai_TROSAiBot.getAForcefulInitiativeAttack(troshx_tros_ai_TROSAiBot.P_THRESHOLD_FAVORABLE,cp,cp2,dtn,true,false,0)) {
			...
			var consideredCP = troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
			troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DEFAULT = 0;
			troshx_tros_ai_TROSAiBot.getFBAttack(true,cp,cp2 * .5 | 0,dtn,false,1);
			if(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP > consideredCP) consideredCP = troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
			...
			cp -= consideredCP;
			if(cp > 2 && troshx_tros_ai_TROSAiBot.getFBAttack(true,cp,stipulateRemaining,dtn,false,2,0.5)) {
				troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[2] = cp;
				troshx_tros_ai_TROSAiBot.B_COMBO_EXCHANGE_BUDGET[3] = stipulateRemaining;
...
troshx_tros_ai_TROSAiBot.getComboAction = function(combo,cp,cp2,threatManuever,hasInitiative,secondExchange) {
	...
	case 1:
		return troshx_tros_ai_TROSAiBot.getFavorableAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags) || troshx_tros_ai_TROSAiBot.getBorderlineAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags);
	case 2:
		if(!secondExchange) return troshx_tros_ai_TROSAiBot.getAForcefulInitiativeAttack(troshx_tros_ai_TROSAiBot.P_THRESHOLD_FAVORABLE,cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,true,0); else return troshx_tros_ai_TROSAiBot.getFavorableAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags) || troshx_tros_ai_TROSAiBot.getBorderlineAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags);
		break;
	case 3:
		if(!secondExchange) {
		} else return troshx_tros_ai_TROSAiBot.getFavorableAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags) || troshx_tros_ai_TROSAiBot.getBorderlineAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags);
		break;
	case 4:
		if(!secondExchange) {
		} else return troshx_tros_ai_TROSAiBot.getFavorableAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags) || troshx_tros_ai_TROSAiBot.getBorderlineAttack(cp,cp2,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),true,flags);
	...
	case -1:
		if(threatManuever != null) {
			if(!secondExchange) {
				if(troshx_tros_ai_TROSAiBot.getBorderlineAttack(cp,cp2 - threatManuever.manueverCP,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),false,1)) {
					cp -= troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP;
	...
	case -2:
		...
		break;
	case -4:
		if(threatManuever != null) return troshx_tros_ai_TROSAiBot.getDisarm(true,cp,threatManuever.manueverCP,threatManuever.manueverTN,0,0,2,true);
		break;
	case -3:
		break;
	case -5:
	...
	while(i < 10) {
		...
		if(budget != null) {
			if(troshx_tros_ai_TROSAiBot.getComboAction(i,budget[0],budget[1],threatManuever,true,false)) {
				...
				if(troshx_tros_ai_TROSAiBot.getComboAction(i,budget[2],budget[3],threatManuever,true,true)) {
					probabilityCheck += troshx_tros_ai_TROSAiBot.B_VIABLE_PROBABILITY_GET;
...
troshx_tros_Weapon.__rtti = "<class path=\"troshx.tros.Weapon\" params=\"\">\n\t<ATTR_BASE_NONE public=\"1\" get=\"inline\" set=\"null\" expr=\"-1\" line=\"39\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>-1</e></m></meta>\n\t</ATTR_BASE_NONE>\n\t<ATTR_BASE_STRENGTH public=\"1\" get=\"inline\" set=\"null\" expr=\"0\" line=\"40\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</ATTR_BASE_STRENGTH>\n\t<createDyn public=\"1\" set=\"method\" line=\"80\" static=\"1\"><f a=\"name:profGroups:properties\">\n\t<c path=\"String\"/>\n\t<c path=\"Array\"><c path=\"String\"/></c>\n\t<d/>\n\t<c path=\"troshx.tros.Weapon\"/>\n</f></createDyn>\n\t<atn public=\"1\"><x path=\"Int\"/></atn>\n\t<atn2 public=\"1\"><x path=\"Int\"/></atn2>\n\t<dtn public=\"1\"><x path=\"Int\"/></dtn>\n\t<dntT public=\"1\"><x path=\"Int\"/></dntT>\n\t<dtn2 public=\"1\"><x path=\"Int\"/></dtn2>\n\t<damage public=\"1\"><x path=\"Int\"/></damage>\n\t<damage2 public=\"1\"><x path=\"Int\"/></damage2>\n\t<damage3 public=\"1\"><x path=\"Int\"/></damage3>\n\t<shield public=\"1\"><x path=\"Bool\"/></shield>\n\t<profeciencies public=\"1\"><c path=\"Array\"><c path=\"String\"/></c></profeciencies>\n\t<name public=\"1\"><c path=\"String\"/></name>\n\t<drawCutModifier public=\"1\"><x path=\"Int\"/></drawCutModifier>\n\t<attrBaseIndex public=\"1\"><x path=\"Int\"/></attrBaseIndex>\n\t<twoHanded public=\"1\"><x path=\"Bool\"/></twoHanded>\n\t<rangedWeapon public=\"1\"><x path=\"Bool\"/></rangedWeapon>\n\t<cpPenalty public=\"1\"><x path=\"Float\"/></cpPenalty>\n\t<movePenalty public=\"1\"><x path=\"Float\"/></movePenalty>\n\t<shieldLimit public=\"1\"><x path=\"Int\"/></shieldLimit>\n\t<blunt public=\"1\"><x path=\"Bool\"/></blunt>\n\t<range public=\"1\"><x path=\"Int\"/></range>\n\t<getDamageTo public=\"1\" set=\"method\" line=\"42\"><f a=\"body:manuever:targetZone:margin:strength\">\n\t<c path=\"troshx.BodyChar\"/>\n\t<c path=\"troshx.tros.Manuever\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></getDamageTo>\n\t<new public=\"1\" set=\"method\" line=\"58\"><f a=\"name:profGroups\">\n\t<c path=\"String\"/>\n\t<c path=\"Array\"><c path=\"String\"/></c>\n\t<x path=\"Void\"/>\n</f></new>\n\t<meta>\n\t\t<m n=\":directlyUsed\"/>\n\t\t<m n=\":expose\"/>\n\t\t<m n=\":rtti\"/>\n\t</meta>\n</class>";
troshx_tros_Weapon.ATTR_BASE_NONE = -1;
troshx_tros_Weapon.ATTR_BASE_STRENGTH = 0;
troshx_tros_WeaponSheet.LIST = [troshx_tros_Weapon.createDyn("Short Sword",["cutthrust","swordshield"],{ 'range' : 1, 'atn' : 7, 'atn2' : 5, 'dtn' : 7, 'damage' : -1, 'damage2' : 1}),troshx_tros_Weapon.createDyn("Gladius",["swordshield"],{ 'range' : 1, 'atn' : 6, 'atn2' : 6, 'dtn' : 7, 'damage' : 0, 'damage2' : 1, 'drawCutModifier' : 0}),troshx_tros_Weapon.createDyn("Arming Sword",["swordshield","cutthrust"],{ 'range' : 2, 'atn' : 6, 'atn2' : 7, 'dtn' : 6, 'damage' : 1, 'damage2' : 0, 'drawCutModifier' : 0}),troshx_tros_Weapon.createDyn("Rapier",["rapier","caserapiers"],{ 'range' : 3, 'atn' : 7, 'atn2' : 5, 'dtn' : 8, 'dtnT' : 6, 'damage' : -3, 'damage2' : 2, 'drawCutModifier' : 1}),troshx_tros_Weapon.createDyn("Hand Shield",["swordshield","massweaponshield"],{ 'shield' : true, 'dtn' : 7, 'dtn2' : 9}),troshx_tros_Weapon.createDyn("Small Shield",["swordshield","massweaponshield"],{ 'shield' : true, 'dtn' : 6, 'dtn2' : 8}),troshx_tros_Weapon.createDyn("Medium Shield",["swordshield","massweaponshield"],{ 'shield' : true, 'dtn' : 5, 'dtn2' : 7, 'cpPenalty' : 0.5, 'movePenalty' : 0.5}),troshx_tros_Weapon.createDyn("Large Shield",["swordshield","massweaponshield"],{ 'shield' : true, 'dtn' : 5, 'dtn2' : 6, 'cpPenalty' : 0.5, 'movePenalty' : 1})];
troshx_tros_WeaponSheet.HASH = troshx_tros_WeaponSheet.createHashLookupViaName(troshx_tros_WeaponSheet.LIST);
troshx_util_AIManueverChoice.__meta__ = { fields : { manuever : { inspect : null}, manueverCP : { inspect : null}, manueverTN : { inspect : null}, targetZone : { inspect : null}, manueverType : { inspect : [{ display : "selector"}], choices : ["TYPE"]}, offhand : { inspect : null}, againstID : { inspect : null}}};
troshx_util_AIManueverChoice.__rtti = "<class path=\"troshx.util.AIManueverChoice\" params=\"\" module=\"troshx.util.TROSAI\">\n\t<TYPE_ATTACKING public=\"1\" get=\"inline\" set=\"null\" expr=\"2\" line=\"234\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>2</e></m></meta>\n\t</TYPE_ATTACKING>\n\t<TYPE_DEFENDING public=\"1\" get=\"inline\" set=\"null\" expr=\"1\" line=\"235\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>1</e></m></meta>\n\t</TYPE_DEFENDING>\n\t<manuever public=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</manuever>\n\t<manueverCP public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</manueverCP>\n\t<manueverTN public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</manueverTN>\n\t<targetZone public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</targetZone>\n\t<manueverType public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\"inspect\"><e>{display:\"selector\"}</e></m>\n\t\t\t<m n=\"choices\"><e>\"TYPE\"</e></m>\n\t\t</meta>\n\t</manueverType>\n\t<offhand public=\"1\">\n\t\t<x path=\"Bool\"/>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</offhand>\n\t<againstID public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</againstID>\n\t<nothing public=\"1\" get=\"inline\" set=\"null\" line=\"242\"><f a=\"\"><x path=\"Void\"/></f></nothing>\n\t<copyTo public=\"1\" get=\"inline\" set=\"null\" line=\"251\">\n\t\t<f a=\"newChoice:?newAgainstID\" v=\":-1\">\n\t\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Void\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{newAgainstID:-1}</e></m></meta>\n\t</copyTo>\n\t<setAttack public=\"1\" set=\"method\" line=\"262\">\n\t\t<f a=\"manuever:manueverCP:manueverTN:targetZone:?offhand\" v=\"::::false\">\n\t\t\t<c path=\"String\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Void\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{offhand:false}</e></m></meta>\n\t</setAttack>\n\t<setDefend public=\"1\" set=\"method\" line=\"271\">\n\t\t<f a=\"manuever:manueverCP:manueverTN:?offhand\" v=\":::false\">\n\t\t\t<c path=\"String\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Void\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{offhand:false}</e></m></meta>\n\t</setDefend>\n\t<new public=\"1\" set=\"method\" line=\"237\"><f a=\"\"><x path=\"Void\"/></f></new>\n\t<meta>\n\t\t<m n=\":directlyUsed\"/>\n\t\t<m n=\":rtti\"/>\n\t\t<m n=\":expose\"/>\n\t</meta>\n</class>";
troshx_util_AIManueverChoice.TYPE_ATTACKING = 2;
troshx_util_AIManueverChoice.TYPE_DEFENDING = 1;
troshx_tros_ai_TROSAiBot.__meta__ = { statics : { AVAIL_bash : { inspect : [{ min : 0}], bind : null}, AVAIL_spike : { inspect : [{ min : 0}], bind : null}, AVAIL_cut : { inspect : [{ min : 0}], bind : null}, AVAIL_thrust : { inspect : [{ min : 0}], bind : null}, AVAIL_beat : { inspect : [{ min : 0}], bind : null}, AVAIL_bindstrike : { inspect : [{ min : 0}], bind : null}, AVAIL_block : { inspect : [{ min : 0}], bind : null}, AVAIL_parry : { inspect : [{ min : 0}], bind : null}, AVAIL_duckweave : { inspect : [{ min : 0}], bind : null}, AVAIL_partialevasion : { inspect : [{ min : 0}], bind : null}, AVAIL_fullevasion : { inspect : [{ min : 0}], bind : null}, AVAIL_blockopenstrike : { inspect : [{ min : 0}], bind : null}, AVAIL_counter : { inspect : [{ min : 0}], bind : null}, AVAIL_rota : { inspect : [{ min : 0}], bind : null}, AVAIL_expulsion : { inspect : [{ min : 0}], bind : null}, AVAIL_disarm : { inspect : [{ min : 0}], bind : null}, AVAIL_StealInitiative : { inspect : [{ min : 0}], bind : null}, B_EQUIP : { settable : null}, B_IS_OFFHAND : { settable : null}, D_EQUIP : { settable : null}, CURRENT_OPPONENT : { inject : null}, P_THRESHOLD_FAVORABLE : { inspect : [{ min : 0, max : 1, display : "range", step : 0.01}]}, P_THRESHOLD_BORDERLINE : { inspect : [{ min : 0, max : 1, display : "range", step : 0.01}]}, P_RECKLESS : { inspect : [{ min : 0, max : 1, display : "range", step : 0.01}]}, getRegularAttack : { 'return' : ["B_CANDIDATES","B_CANDIDATE_COUNT"], inspect : [[{ inspect : { min : 0}},{ inspect : { min : 0}},{ inspect : { min : -1}},{ inspect : { min : 1}}]]}, getRegularDefense : { 'return' : ["B_CANDIDATES","B_CANDIDATE_COUNT"], inspect : [[{ inspect : { min : 0}},{ inspect : { min : 0}}]]}, checkCostViability : { 'return' : ["B_VIABLE_PROBABILITY"]}, checkCostViabilityBorderline : { 'return' : ["B_VIABLE_PROBABILITY"]}, checkCostAntiFavorability : { 'return' : ["B_VIABLE_PROBABILITY_GET"]}, checkCostAntiBorderline : { 'return' : ["B_VIABLE_PROBABILITY_GET"]}, getASuitableAttack : { 'return' : ["MANUEVER_CHOICE","B_VIABLE_PROBABILITY"], inspect : [[{ inspect : { min : 0, step : 0.01, max : 1, display : "range"}},{ inspect : { min : 0}},{ inspect : { min : 0}},{ inspect : { min : 1}}]]}, getAForcefulInitiativeAttack : { 'return' : ["MANUEVER_CHOICE","B_VIABLE_PROBABILITY_GET"], inspect : [[{ inspect : { min : 0, step : 0.01, max : 1, display : "range"}},{ inspect : { min : 0}},{ inspect : { min : 0}},{ inspect : { min : 1}}]]}, getASuitableDefense : { 'return' : ["MANUEVER_CHOICE","B_VIABLE_PROBABILITY"], inspect : [[{ inspect : { min : 0, step : 0.01, max : 1, display : "range"}},{ inspect : { min : 0}},{ inspect : { min : 0}},{ inspect : { min : 1}}]]}, getFavorableAttack : { inspect : [[{ inspect : { min : 0}},{ inspect : { min : 0}},{ inspect : { min : 1}},{ inspect : null},{ inspect : null, bitmask : "FLAG"}]], 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE","B_VIABLE_HEURISTIC"]}, getBorderlineAttack : { inspect : [[{ inspect : { min : 0}},{ inspect : { min : 0}},{ inspect : { min : 1}},{ inspect : null},{ inspect : null, bitmask : "FLAG"}]], 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE","B_VIABLE_HEURISTIC"]}, getFavorableDefense : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE","B_VIABLE_HEURISTIC"]}, getBorderlineDefense : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE","B_VIABLE_HEURISTIC"]}, getFleeOrDefend : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE","B_VIABLE_HEURISTIC"]}, getBlockOpenAndStrike : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"]}, getRotaOrCounter : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"]}, getDisarm : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"]}, getHook : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"]}, getBeat : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"]}, getBindStrike : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"]}, getPredictedOpponentDTN : { inspect : null}, getPredictedOpponentATN : { inspect : null}, getComboExchangeBudgetingWithInitiative : { inspect : [[{ inspect : { display : "selector"}, choices : "COMBO"},{ inspect : { min : 0}},{ inspect : { min : 0}}]]}, getComboAction : { 'return' : ["B_VIABLE_PROBABILITY_GET","MANUEVER_CHOICE"], inspect : [[{ inspect : { display : "selector"}, choices : "COMBO"},{ inspect : { min : 0}},{ inspect : { min : 0}}]]}, setBestComboActionWithInitiativePlan : { 'return' : ["MANUEVER_COMBO_SET","MANUEVER_CHOICE_SET"], inspect : [[{ inspect : { min : 0}},{ inspect : { min : 0}}]]}, setBestComboActionWithoutInitiativePlan : { 'return' : ["MANUEVER_COMBO_SET","MANUEVER_CHOICE_SET"], inspect : [[{ inspect : { min : 0}},{ inspect : { min : 0}}]]}}, fields : { body : { inject : null}, cp : { bind : ["_cp"]}, equipMasterhand : { bind : ["_equipMasterhand"]}, equipOffhand : { bind : ["_equipOffhand"]}, mobility : { bind : ["_mobility"]}, id : { bind : ["_id"]}, initiative : { bind : ["_fight_initiative"]}, stance : { bind : ["_fight_stance"]}, manueverUsingHands : { bind : ["_manueverUsingHands"]}}};
troshx_tros_ai_TROSAiBot.__rtti = "<class path=\"troshx.tros.ai.TROSAiBot\" params=\"\">\n\t<AVAIL_bash public=\"1\" expr=\"0\" line=\"61\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_bash>\n\t<AVAIL_spike public=\"1\" expr=\"0\" line=\"62\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_spike>\n\t<AVAIL_cut public=\"1\" expr=\"0\" line=\"63\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_cut>\n\t<AVAIL_thrust public=\"1\" expr=\"0\" line=\"64\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_thrust>\n\t<AVAIL_beat public=\"1\" expr=\"0\" line=\"65\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_beat>\n\t<AVAIL_bindstrike public=\"1\" expr=\"0\" line=\"66\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_bindstrike>\n\t<AVAIL_block public=\"1\" expr=\"0\" line=\"69\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_block>\n\t<AVAIL_parry public=\"1\" expr=\"0\" line=\"70\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_parry>\n\t<AVAIL_duckweave public=\"1\" expr=\"0\" line=\"71\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_duckweave>\n\t<AVAIL_partialevasion public=\"1\" expr=\"0\" line=\"72\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_partialevasion>\n\t<AVAIL_fullevasion public=\"1\" expr=\"0\" line=\"73\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_fullevasion>\n\t<AVAIL_blockopenstrike public=\"1\" expr=\"0\" line=\"74\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_blockopenstrike>\n\t<AVAIL_counter public=\"1\" expr=\"0\" line=\"75\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_counter>\n\t<AVAIL_rota public=\"1\" expr=\"0\" line=\"76\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_rota>\n\t<AVAIL_expulsion public=\"1\" expr=\"0\" line=\"77\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_expulsion>\n\t<AVAIL_disarm public=\"1\" expr=\"0\" line=\"78\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_disarm>\n\t<AVAIL_StealInitiative public=\"1\" expr=\"5\" line=\"79\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>5</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0}</e></m>\n\t\t\t<m n=\"bind\"/>\n\t\t</meta>\n\t</AVAIL_StealInitiative>\n\t<setTypicalAVAILCostsForTesting set=\"method\" line=\"81\" static=\"1\"><f a=\"\"><x path=\"Void\"/></f></setTypicalAVAILCostsForTesting>\n\t<getCostOfAVAIL get=\"inline\" set=\"null\" line=\"101\" static=\"1\"><f a=\"avail\">\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></getCostOfAVAIL>\n\t<getCostOfManuever set=\"method\" line=\"105\" static=\"1\"><f a=\"manuever\">\n\t<c path=\"String\"/>\n\t<x path=\"Int\"/>\n</f></getCostOfManuever>\n\t<B_EQUIP expr=\"&quot;&quot;\" line=\"133\" static=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>\"\"</e></m>\n\t\t\t<m n=\"settable\"/>\n\t\t</meta>\n\t</B_EQUIP>\n\t<B_IS_OFFHAND expr=\"false\" line=\"134\" static=\"1\">\n\t\t<x path=\"Bool\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>false</e></m>\n\t\t\t<m n=\"settable\"/>\n\t\t</meta>\n\t</B_IS_OFFHAND>\n\t<D_EQUIP expr=\"&quot;&quot;\" line=\"135\" static=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>\"\"</e></m>\n\t\t\t<m n=\"settable\"/>\n\t\t</meta>\n\t</D_EQUIP>\n\t<B_COMBO_EXCHANGE_BUDGET expr=\"&apos;???&apos;\" line=\"138\" static=\"1\">\n\t\t<x path=\"haxe.ds.Vector\"><x path=\"Int\"/></x>\n\t\t<meta><m n=\":value\"><e>'???'</e></m></meta>\n\t</B_COMBO_EXCHANGE_BUDGET>\n\t<BUDGET_EXCHANGE_1 get=\"inline\" set=\"null\" expr=\"0\" line=\"139\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</BUDGET_EXCHANGE_1>\n\t<BUDGET_EXCHANGE_1_ENEMY get=\"inline\" set=\"null\" expr=\"1\" line=\"140\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>1</e></m></meta>\n\t</BUDGET_EXCHANGE_1_ENEMY>\n\t<BUDGET_EXCHANGE_2 get=\"inline\" set=\"null\" expr=\"2\" line=\"141\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>2</e></m></meta>\n\t</BUDGET_EXCHANGE_2>\n\t<BUDGET_EXCHANGE_2_ENEMY get=\"inline\" set=\"null\" expr=\"3\" line=\"142\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>3</e></m></meta>\n\t</BUDGET_EXCHANGE_2_ENEMY>\n\t<CURRENT_OPPONENT static=\"1\">\n\t\t<c path=\"troshx.tros.ai.TROSAiBot\"/>\n\t\t<meta><m n=\"inject\"/></meta>\n\t</CURRENT_OPPONENT>\n\t<COMBO_PureMeanStrikes get=\"inline\" set=\"null\" expr=\"1\" line=\"151\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>1</e></m></meta>\n\t</COMBO_PureMeanStrikes>\n\t<COMBO_HeavyFirstStrikes get=\"inline\" set=\"null\" expr=\"2\" line=\"152\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>2</e></m></meta>\n\t</COMBO_HeavyFirstStrikes>\n\t<COMBO_AlphaDisarm get=\"inline\" set=\"null\" expr=\"3\" line=\"153\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>3</e></m></meta>\n\t</COMBO_AlphaDisarm>\n\t<COMBO_AlphaHookStrike get=\"inline\" set=\"null\" expr=\"4\" line=\"154\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>4</e></m></meta>\n\t</COMBO_AlphaHookStrike>\n\t<COMBO_AlphaStrike get=\"inline\" set=\"null\" expr=\"5\" line=\"155\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>5</e></m></meta>\n\t</COMBO_AlphaStrike>\n\t<COMBO_BeatStrike get=\"inline\" set=\"null\" expr=\"6\" line=\"156\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>6</e></m></meta>\n\t</COMBO_BeatStrike>\n\t<COMBO_BindAndStrike get=\"inline\" set=\"null\" expr=\"7\" line=\"157\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>7</e></m></meta>\n\t</COMBO_BindAndStrike>\n\t<COMBO_FeintStrike get=\"inline\" set=\"null\" expr=\"8\" line=\"158\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>8</e></m></meta>\n\t</COMBO_FeintStrike>\n\t<COMBO_DoubleAttack get=\"inline\" set=\"null\" expr=\"9\" line=\"159\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>9</e></m></meta>\n\t</COMBO_DoubleAttack>\n\t<COMBO_SimulatenousBlockStrike get=\"inline\" set=\"null\" expr=\"10\" line=\"160\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>10</e></m></meta>\n\t</COMBO_SimulatenousBlockStrike>\n\t<COMBOS_LEN_INITIATIVE get=\"inline\" set=\"null\" expr=\"10\" line=\"161\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>10</e></m></meta>\n\t</COMBOS_LEN_INITIATIVE>\n\t<COMBO_DefensiveFirst get=\"inline\" set=\"null\" expr=\"-1\" line=\"164\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>-1</e></m></meta>\n\t</COMBO_DefensiveFirst>\n\t<COMBO_DefensiveBorderline get=\"inline\" set=\"null\" expr=\"-2\" line=\"165\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>-2</e></m></meta>\n\t</COMBO_DefensiveBorderline>\n\t<COMBO_AlphaInitiativeStealer get=\"inline\" set=\"null\" expr=\"-3\" line=\"166\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>-3</e></m></meta>\n\t</COMBO_AlphaInitiativeStealer>\n\t<COMBO_AlphaDisarmDef get=\"inline\" set=\"null\" expr=\"-4\" line=\"167\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>-4</e></m></meta>\n\t</COMBO_AlphaDisarmDef>\n\t<COMBO_SimulatenousBlockStrikeStealer get=\"inline\" set=\"null\" expr=\"-5\" line=\"168\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>-5</e></m></meta>\n\t</COMBO_SimulatenousBlockStrikeStealer>\n\t<COMBOS_LEN_NO_INITAITIVE get=\"inline\" set=\"null\" expr=\"5\" line=\"169\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>5</e></m></meta>\n\t</COMBOS_LEN_NO_INITAITIVE>\n\t<P_THRESHOLD_BEYOND_FAVORABLE public=\"1\" expr=\"0.9\" line=\"171\" static=\"1\">\n\t\t<x path=\"Float\"/>\n\t\t<meta><m n=\":value\"><e>0.9</e></m></meta>\n\t</P_THRESHOLD_BEYOND_FAVORABLE>\n\t<P_THRESHOLD_FAVORABLE public=\"1\" expr=\"0.75\" line=\"172\" static=\"1\">\n\t\t<x path=\"Float\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0.75</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0,max:1,display:\"range\",step:0.01}</e></m>\n\t\t</meta>\n\t</P_THRESHOLD_FAVORABLE>\n\t<P_THRESHOLD_BORDERLINE public=\"1\" expr=\"0.5\" line=\"173\" static=\"1\">\n\t\t<x path=\"Float\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0.5</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0,max:1,display:\"range\",step:0.01}</e></m>\n\t\t</meta>\n\t</P_THRESHOLD_BORDERLINE>\n\t<P_RECKLESS public=\"1\" expr=\"0.2\" line=\"174\" static=\"1\">\n\t\t<x path=\"Float\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0.2</e></m>\n\t\t\t<m n=\"inspect\"><e>{min:0,max:1,display:\"range\",step:0.01}</e></m>\n\t\t</meta>\n\t</P_RECKLESS>\n\t<B_CANDIDATES expr=\"[]\" line=\"177\" static=\"1\">\n\t\t<c path=\"Array\"><c path=\"String\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</B_CANDIDATES>\n\t<B_CANDIDATE_COUNT expr=\"0\" line=\"178\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</B_CANDIDATE_COUNT>\n\t<B_BS_REQUIRED expr=\"1\" line=\"179\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>1</e></m></meta>\n\t</B_BS_REQUIRED>\n\t<B_BS_REQUIRED_DEFAULT expr=\"1\" line=\"180\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>1</e></m></meta>\n\t</B_BS_REQUIRED_DEFAULT>\n\t<B_BS_REQUIRED_LOCK expr=\"false\" line=\"181\" static=\"1\">\n\t\t<x path=\"Bool\"/>\n\t\t<meta><m n=\":value\"><e>false</e></m></meta>\n\t</B_BS_REQUIRED_LOCK>\n\t<B_VIABLE_PROBABILITY static=\"1\"><x path=\"Float\"/></B_VIABLE_PROBABILITY>\n\t<B_VIABLE_PROBABILITY_GET static=\"1\"><x path=\"Float\"/></B_VIABLE_PROBABILITY_GET>\n\t<B_VIABLE_HEURISTIC static=\"1\"><x path=\"Bool\"/></B_VIABLE_HEURISTIC>\n\t<B_MANUEVER_CHOICES expr=\"[]\" line=\"187\" static=\"1\">\n\t\t<c path=\"Array\"><c path=\"troshx.util.AIManueverChoice\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</B_MANUEVER_CHOICES>\n\t<B_MANUEVER_CHOICE_PROBABILITIES expr=\"[]\" line=\"188\" static=\"1\">\n\t\t<c path=\"Array\"><x path=\"Float\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</B_MANUEVER_CHOICE_PROBABILITIES>\n\t<B_MANUEVER_CHOICE_COUNT expr=\"0\" line=\"189\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</B_MANUEVER_CHOICE_COUNT>\n\t<B_COMBO_CANDIDATES expr=\"[]\" line=\"191\" static=\"1\">\n\t\t<c path=\"Array\"><x path=\"Int\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</B_COMBO_CANDIDATES>\n\t<B_COMBO_CANDIDATE_MANUEVER expr=\"[]\" line=\"192\" static=\"1\">\n\t\t<c path=\"Array\"><c path=\"troshx.util.AIManueverChoice\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</B_COMBO_CANDIDATE_MANUEVER>\n\t<B_COMBO_CANDIDATE_COUNT expr=\"0\" line=\"193\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</B_COMBO_CANDIDATE_COUNT>\n\t<IMPOSSIBLE_TN get=\"inline\" set=\"null\" expr=\"888\" line=\"195\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>888</e></m></meta>\n\t</IMPOSSIBLE_TN>\n\t<getRegularAttack public=\"1\" set=\"method\" line=\"207\" static=\"1\">\n\t\t<f a=\"availableCP:?roll:?againstRoll:?againstTN\" v=\":0:-1:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<c path=\"String\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{againstTN:1,againstRoll:-1,roll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_CANDIDATES\"</e>\n\t\t\t\t<e>\"B_CANDIDATE_COUNT\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0}},{inspect:{min:0}},{inspect:{min:-1}},{inspect:{min:1}}]</e></m>\n\t\t</meta>\n\t</getRegularAttack>\n\t<getRegularDefense public=\"1\" set=\"method\" line=\"306\" static=\"1\">\n\t\t<f a=\"availableCP:?roll:?enforceMustRegainInitiative\" v=\":0:false\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<c path=\"String\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{enforceMustRegainInitiative:false,roll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_CANDIDATES\"</e>\n\t\t\t\t<e>\"B_CANDIDATE_COUNT\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0}},{inspect:{min:0}}]</e></m>\n\t\t</meta>\n\t</getRegularDefense>\n\t<getATNOfManuever set=\"method\" line=\"369\" static=\"1\"><f a=\"manuever\">\n\t<c path=\"String\"/>\n\t<x path=\"Int\"/>\n</f></getATNOfManuever>\n\t<getDTNOfManuever set=\"method\" line=\"387\" static=\"1\"><f a=\"manuever\">\n\t<c path=\"String\"/>\n\t<x path=\"Int\"/>\n</f></getDTNOfManuever>\n\t<getTargetZoneAverageAVOffset public=\"1\" set=\"method\" line=\"405\" static=\"1\"><f a=\"tarZone:weaponName\">\n\t<x path=\"Int\"/>\n\t<c path=\"String\"/>\n\t<x path=\"Int\"/>\n</f></getTargetZoneAverageAVOffset>\n\t<getRegularTargetZone public=\"1\" set=\"method\" line=\"410\" static=\"1\"><f a=\"manuever:atn:cp\">\n\t<c path=\"String\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></getRegularTargetZone>\n\t<MANUEVER_CHOICE expr=\"&apos;???&apos;\" line=\"443\" static=\"1\">\n\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t<meta><m n=\":value\"><e>'???'</e></m></meta>\n\t</MANUEVER_CHOICE>\n\t<MANUEVER_CHOICE_1 expr=\"&apos;???&apos;\" line=\"444\" static=\"1\">\n\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t<meta><m n=\":value\"><e>'???'</e></m></meta>\n\t</MANUEVER_CHOICE_1>\n\t<MANUEVER_CHOICE_SET expr=\"&apos;???&apos;\" line=\"445\" static=\"1\">\n\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t<meta><m n=\":value\"><e>'???'</e></m></meta>\n\t</MANUEVER_CHOICE_SET>\n\t<MANUEVER_COMBO_SET expr=\"0\" line=\"446\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</MANUEVER_COMBO_SET>\n\t<checkCostViability set=\"method\" line=\"449\" static=\"1\">\n\t\t<f a=\"availableCP:tn:threshold:againstRoll:?againstTN:?useAllCP\" v=\"::::1:false\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{useAllCP:false,againstTN:1}</e></m>\n\t\t\t<m n=\"return\"><e>\"B_VIABLE_PROBABILITY\"</e></m>\n\t\t</meta>\n\t</checkCostViability>\n\t<precisionPerc get=\"inline\" set=\"null\" line=\"464\" static=\"1\"><f a=\"val\">\n\t<x path=\"Float\"/>\n\t<x path=\"Float\"/>\n</f></precisionPerc>\n\t<checkCostViabilityBorderline set=\"method\" line=\"469\" static=\"1\">\n\t\t<f a=\"availableCP:tn:threshold:againstRoll:?againstTN:?useAllCP\" v=\"::::1:false\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{useAllCP:false,againstTN:1}</e></m>\n\t\t\t<m n=\"return\"><e>\"B_VIABLE_PROBABILITY\"</e></m>\n\t\t</meta>\n\t</checkCostViabilityBorderline>\n\t<checkCostAntiFavorability set=\"method\" line=\"489\" static=\"1\">\n\t\t<f a=\"availableCP:tn:threshold:againstCP:?againstTN:?useAllCP:?offset:?requiredBS\" v=\"::::1:false:0:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{requiredBS:1,offset:0,useAllCP:false,againstTN:1}</e></m>\n\t\t\t<m n=\"return\"><e>\"B_VIABLE_PROBABILITY_GET\"</e></m>\n\t\t</meta>\n\t</checkCostAntiFavorability>\n\t<checkCostAntiBorderline set=\"method\" line=\"526\" static=\"1\">\n\t\t<f a=\"availableCP:tn:threshold:againstCP:?againstTN:?useAllCP:?offset\" v=\"::::1:false:0\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{offset:0,useAllCP:false,againstTN:1}</e></m>\n\t\t\t<m n=\"return\"><e>\"B_VIABLE_PROBABILITY_GET\"</e></m>\n\t\t</meta>\n\t</checkCostAntiBorderline>\n\t<getCostOfAvail get=\"inline\" set=\"null\" line=\"560\" static=\"1\"><f a=\"avail\">\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></getCostOfAvail>\n\t<getTheSuitableAttack set=\"method\" line=\"564\" static=\"1\">\n\t\t<f a=\"manuever:tn:tarZone:threshold:availableCP:?againstRoll:?againstTN:?favorable:?useAllCP\" v=\":::::0:1:true:false\">\n\t\t\t<c path=\"String\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{useAllCP:false,favorable:true,againstTN:1,againstRoll:0}</e></m></meta>\n\t</getTheSuitableAttack>\n\t<getTheSuitableDefense set=\"method\" line=\"574\" static=\"1\">\n\t\t<f a=\"manuever:tn:threshold:availableCP:?againstRoll:?againstTN:?favorable:?useAllCP\" v=\"::::0:1:true:false\">\n\t\t\t<c path=\"String\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{useAllCP:false,favorable:true,againstTN:1,againstRoll:0}</e></m></meta>\n\t</getTheSuitableDefense>\n\t<getTheForcefulInitiativeAttack set=\"method\" line=\"583\" static=\"1\">\n\t\t<f a=\"manuever:tn:tarZone:threshold:availableCP:?againstCP:?againstTN:?favorable:?useAllCP:?offset\" v=\":::::0:1:true:false:0\">\n\t\t\t<c path=\"String\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{offset:0,useAllCP:false,favorable:true,againstTN:1,againstCP:0}</e></m></meta>\n\t</getTheForcefulInitiativeAttack>\n\t<getASuitableAttack public=\"1\" set=\"method\" line=\"594\" static=\"1\">\n\t\t<f a=\"threshold:availableCP:?againstRoll:?againstTN:?favorable:?useAllCP\" v=\"::0:1:true:false\">\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{useAllCP:false,favorable:true,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0,step:0.01,max:1,display:\"range\"}},{inspect:{min:0}},{inspect:{min:0}},{inspect:{min:1}}]</e></m>\n\t\t</meta>\n\t</getASuitableAttack>\n\t<getAForcefulInitiativeAttack public=\"1\" set=\"method\" line=\"612\" static=\"1\">\n\t\t<f a=\"threshold:availableCP:?againstCP:?againstTN:?favorable:?useAllCP:?offset\" v=\"::0:1:true:false:0\">\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{offset:0,useAllCP:false,favorable:true,againstTN:1,againstCP:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0,step:0.01,max:1,display:\"range\"}},{inspect:{min:0}},{inspect:{min:0}},{inspect:{min:1}}]</e></m>\n\t\t</meta>\n\t</getAForcefulInitiativeAttack>\n\t<getASuitableDefense public=\"1\" set=\"method\" line=\"630\" static=\"1\">\n\t\t<f a=\"threshold:availableCP:?againstRoll:?againstTN:?mustRegainInitiative:?favorable:?useAllCP\" v=\"::0:1:false:true:false\">\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{useAllCP:false,favorable:true,mustRegainInitiative:false,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0,step:0.01,max:1,display:\"range\"}},{inspect:{min:0}},{inspect:{min:0}},{inspect:{min:1}}]</e></m>\n\t\t</meta>\n\t</getASuitableDefense>\n\t<FLAG_GET_CHEAPEST public=\"1\" get=\"inline\" set=\"null\" expr=\"1\" line=\"641\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>1</e></m></meta>\n\t</FLAG_GET_CHEAPEST>\n\t<FLAG_USE_ALL_CP public=\"1\" get=\"inline\" set=\"null\" expr=\"2\" line=\"642\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>2</e></m></meta>\n\t</FLAG_USE_ALL_CP>\n\t<FLAG_BORDERLINE_DEF_SAFETY public=\"1\" get=\"inline\" set=\"null\" expr=\"4\" line=\"643\" static=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>4</e></m></meta>\n\t</FLAG_BORDERLINE_DEF_SAFETY>\n\t<getFavorableAttack public=\"1\" get=\"inline\" set=\"null\" line=\"647\" static=\"1\">\n\t\t<f a=\"availableCP:?againstRoll:?againstTN:?heuristic:?flags\" v=\":0:1:true:0\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0}},{inspect:{min:0}},{inspect:{min:1}},{inspect:null},{inspect:null,bitmask:\"FLAG\"}]</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_HEURISTIC\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getFavorableAttack>\n\t<getBorderlineAttack public=\"1\" set=\"method\" line=\"655\" static=\"1\">\n\t\t<f a=\"availableCP:?againstRoll:?againstTN:?heuristic:?flags\" v=\":0:1:true:0\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0}},{inspect:{min:0}},{inspect:{min:1}},{inspect:null},{inspect:null,bitmask:\"FLAG\"}]</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_HEURISTIC\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getBorderlineAttack>\n\t<getFBAttack set=\"method\" line=\"660\" static=\"1\">\n\t\t<f a=\"favorable:availableCP:?againstRoll:?againstTN:?heuristic:?flags:?customThreshold\" v=\"::0:1:true:0:0\">\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{customThreshold:0,flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m></meta>\n\t</getFBAttack>\n\t<getFBDefense set=\"method\" line=\"782\" static=\"1\">\n\t\t<f a=\"favorable:availableCP:?againstRoll:?againstTN:?heuristic:?flags:?customThreshold\" v=\"::0:1:true:0:0\">\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{customThreshold:0,flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m></meta>\n\t</getFBDefense>\n\t<getFavorableDefense public=\"1\" get=\"inline\" set=\"null\" line=\"921\" static=\"1\">\n\t\t<f a=\"availableCP:?againstRoll:?againstTN:?heuristic:?flags\" v=\":0:1:true:0\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_HEURISTIC\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getFavorableDefense>\n\t<getBorderlineDefense public=\"1\" get=\"inline\" set=\"null\" line=\"926\" static=\"1\">\n\t\t<f a=\"availableCP:?againstRoll:?againstTN:?heuristic:?flags\" v=\":0:1:true:0\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_HEURISTIC\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getBorderlineDefense>\n\t<getFleeOrDefend public=\"1\" set=\"method\" line=\"931\" static=\"1\">\n\t\t<f a=\"favorable:availableCP:?againstRoll:?againstTN:?heuristic:?flags:?customThreshold:?secondExchange\" v=\"::0:1:true:0:0:false\">\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{secondExchange:false,customThreshold:0,flags:0,heuristic:true,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t\t<e>\"B_VIABLE_HEURISTIC\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getFleeOrDefend>\n\t<addPossibleRegularManueverChoice get=\"inline\" set=\"null\" line=\"955\" static=\"1\"><f a=\"index\">\n\t<x path=\"Int\"/>\n\t<x path=\"Void\"/>\n</f></addPossibleRegularManueverChoice>\n\t<getBlockOpenAndStrike public=\"1\" set=\"method\" line=\"961\" static=\"1\">\n\t\t<f a=\"availableCP:againstCP:?againstRoll:?againstTN\" v=\"::0:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getBlockOpenAndStrike>\n\t<getRotaOrCounter public=\"1\" set=\"method\" line=\"966\" static=\"1\">\n\t\t<f a=\"availableCP:againstCP:?againstRoll:?againstTN\" v=\"::0:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getRotaOrCounter>\n\t<getDisarm public=\"1\" set=\"method\" line=\"970\" static=\"1\">\n\t\t<f a=\"favorable:availableCP:?againstRoll:?againstTN:?flags:?customThreshold:?preferedRS:?defensive\" v=\"::0:1:0:0:1:false\">\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Float\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{defensive:false,preferedRS:1,customThreshold:0,flags:0,againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getDisarm>\n\t<getHook public=\"1\" set=\"method\" line=\"997\" static=\"1\">\n\t\t<f a=\"availableCP:againstCP:?againstRoll:?againstTN\" v=\"::0:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getHook>\n\t<getBeat public=\"1\" set=\"method\" line=\"1001\" static=\"1\">\n\t\t<f a=\"availableCP:againstCP:?againstRoll:?againstTN\" v=\"::0:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getBeat>\n\t<getBindStrike public=\"1\" set=\"method\" line=\"1005\" static=\"1\">\n\t\t<f a=\"availableCP:againstCP:?againstRoll:?againstTN\" v=\"::0:1\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{againstTN:1,againstRoll:0}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t</meta>\n\t</getBindStrike>\n\t<getPredictedOpponentDTN public=\"1\" get=\"inline\" set=\"null\" line=\"1010\" static=\"1\">\n\t\t<f a=\"\"><x path=\"Int\"/></f>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</getPredictedOpponentDTN>\n\t<getPredictedOpponentATN public=\"1\" get=\"inline\" set=\"null\" line=\"1015\" static=\"1\">\n\t\t<f a=\"\"><x path=\"Int\"/></f>\n\t\t<meta><m n=\"inspect\"/></meta>\n\t</getPredictedOpponentATN>\n\t<fluctuateLower get=\"inline\" set=\"null\" line=\"1077\" static=\"1\"><f a=\"val:lowerBy\">\n\t<x path=\"Float\"/>\n\t<x path=\"Float\"/>\n\t<x path=\"Float\"/>\n</f></fluctuateLower>\n\t<getComboExchangeBudgetingWithInitiative set=\"method\" line=\"1086\" static=\"1\">\n\t\t<f a=\"combo:cp:cp2:threatManuever\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t\t<x path=\"haxe.ds.Vector\"><x path=\"Int\"/></x>\n\t\t</f>\n\t\t<meta><m n=\"inspect\"><e>[{inspect:{display:\"selector\"},choices:\"COMBO\"},{inspect:{min:0}},{inspect:{min:0}}]</e></m></meta>\n\t</getComboExchangeBudgetingWithInitiative>\n\t<getComboAction set=\"method\" line=\"1157\" static=\"1\">\n\t\t<f a=\"combo:cp:cp2:?threatManuever:?hasInitiative:?secondExchange\" v=\":::null:true:false\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>{secondExchange:false,hasInitiative:true,threatManuever:null}</e></m>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"B_VIABLE_PROBABILITY_GET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{display:\"selector\"},choices:\"COMBO\"},{inspect:{min:0}},{inspect:{min:0}}]</e></m>\n\t\t</meta>\n\t</getComboAction>\n\t<addPossibleComboManueverChoice get=\"inline\" set=\"null\" line=\"1304\" static=\"1\"><f a=\"index:theChoice\">\n\t<x path=\"Int\"/>\n\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t<x path=\"Void\"/>\n</f></addPossibleComboManueverChoice>\n\t<setBestComboActionWithInitiativePlan set=\"method\" line=\"1320\" static=\"1\">\n\t\t<f a=\"cp:cp2:threatManuever\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"MANUEVER_COMBO_SET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE_SET\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0}},{inspect:{min:0}}]</e></m>\n\t\t</meta>\n\t</setBestComboActionWithInitiativePlan>\n\t<setBestComboActionWithoutInitiativePlan public=\"1\" set=\"method\" line=\"1392\" static=\"1\">\n\t\t<f a=\"cp:cp2:threatManuever\">\n\t\t\t<x path=\"Int\"/>\n\t\t\t<x path=\"Int\"/>\n\t\t\t<c path=\"troshx.util.AIManueverChoice\"/>\n\t\t\t<x path=\"Bool\"/>\n\t\t</f>\n\t\t<meta>\n\t\t\t<m n=\"return\">\n\t\t\t\t<e>\"MANUEVER_COMBO_SET\"</e>\n\t\t\t\t<e>\"MANUEVER_CHOICE_SET\"</e>\n\t\t\t</m>\n\t\t\t<m n=\"inspect\"><e>[{inspect:{min:0}},{inspect:{min:0}}]</e></m>\n\t\t</meta>\n\t</setBestComboActionWithoutInitiativePlan>\n\t<opponents expr=\"[]\" line=\"19\">\n\t\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</opponents>\n\t<opponentLen expr=\"0\" line=\"20\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</opponentLen>\n\t<cpBudget expr=\"[]\" line=\"21\">\n\t\t<c path=\"Array\"><x path=\"Int\"/></c>\n\t\t<meta><m n=\":value\"><e>[]</e></m></meta>\n\t</cpBudget>\n\t<plannedCombos expr=\"[0,0,0,0]\" line=\"23\">\n\t\t<c path=\"Array\"><x path=\"Int\"/></c>\n\t\t<meta><m n=\":value\"><e>[0,0,0,0]</e></m></meta>\n\t</plannedCombos>\n\t<currentExchange expr=\"0\" line=\"25\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</currentExchange>\n\t<body public=\"1\">\n\t\t<c path=\"troshx.BodyChar\"/>\n\t\t<meta><m n=\"inject\"/></meta>\n\t</body>\n\t<cp public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"bind\"><e>\"_cp\"</e></m></meta>\n\t</cp>\n\t<equipMasterhand public=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta><m n=\"bind\"><e>\"_equipMasterhand\"</e></m></meta>\n\t</equipMasterhand>\n\t<equipOffhand public=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta><m n=\"bind\"><e>\"_equipOffhand\"</e></m></meta>\n\t</equipOffhand>\n\t<mobility public=\"1\" expr=\"6\" line=\"33\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>6</e></m>\n\t\t\t<m n=\"bind\"><e>\"_mobility\"</e></m>\n\t\t</meta>\n\t</mobility>\n\t<id public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"bind\"><e>\"_id\"</e></m></meta>\n\t</id>\n\t<initiative public=\"1\">\n\t\t<x path=\"Bool\"/>\n\t\t<meta><m n=\"bind\"><e>\"_fight_initiative\"</e></m></meta>\n\t</initiative>\n\t<stance public=\"1\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\"bind\"><e>\"_fight_stance\"</e></m></meta>\n\t</stance>\n\t<manueverUsingHands public=\"1\" expr=\"0\" line=\"39\">\n\t\t<x path=\"Int\"/>\n\t\t<meta>\n\t\t\t<m n=\":value\"><e>0</e></m>\n\t\t\t<m n=\"bind\"><e>\"_manueverUsingHands\"</e></m>\n\t\t</meta>\n\t</manueverUsingHands>\n\t<handsUsedUp expr=\"0\" line=\"42\">\n\t\t<x path=\"Int\"/>\n\t\t<meta><m n=\":value\"><e>0</e></m></meta>\n\t</handsUsedUp>\n\t<decidedStance public=\"1\"><x path=\"Int\"/></decidedStance>\n\t<decidedOrientation public=\"1\"><x path=\"Int\"/></decidedOrientation>\n\t<decidedManuevers public=\"1\" expr=\"[&apos;???&apos;,&apos;???&apos;,&apos;???&apos;,&apos;???&apos;]\" line=\"46\">\n\t\t<c path=\"Array\"><c path=\"troshx.util.AIManueverChoice\"/></c>\n\t\t<meta><m n=\":value\"><e>['???','???','???','???']</e></m></meta>\n\t</decidedManuevers>\n\t<getDecidedManueverForSlot public=\"1\" set=\"method\" line=\"50\"><f a=\"slot\">\n\t<x path=\"Int\"/>\n\t<c path=\"String\"/>\n</f></getDecidedManueverForSlot>\n\t<getDecidedManueverCP public=\"1\" set=\"method\" line=\"53\"><f a=\"slot\">\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></getDecidedManueverCP>\n\t<getDecidedManueverTargetZone public=\"1\" set=\"method\" line=\"56\"><f a=\"slot\">\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></getDecidedManueverTargetZone>\n\t<getPredictedDTN set=\"method\" line=\"1019\"><f a=\"\"><x path=\"Int\"/></f></getPredictedDTN>\n\t<getPredictedATN set=\"method\" line=\"1039\"><f a=\"\"><x path=\"Int\"/></f></getPredictedATN>\n\t<getHighestATNOrDTN set=\"method\" line=\"1067\"><f a=\"\"><x path=\"Int\"/></f></getHighestATNOrDTN>\n\t<getDTNBetterMargin get=\"inline\" set=\"null\" line=\"1073\"><f a=\"\"><x path=\"Int\"/></f></getDTNBetterMargin>\n\t<newExchange public=\"1\" set=\"method\" line=\"1446\">\n\t\t<f a=\"?newRound\" v=\"false\">\n\t\t\t<x path=\"Bool\"/>\n\t\t\t<x path=\"Void\"/>\n\t\t</f>\n\t\t<meta><m n=\":value\"><e>{newRound:false}</e></m></meta>\n\t</newExchange>\n\t<decideStance public=\"1\" set=\"method\" line=\"1464\"><f a=\"enemies:target:targetedBy\">\n\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t<c path=\"troshx.tros.ai.TROSAiBot\"/>\n\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t<x path=\"Void\"/>\n</f></decideStance>\n\t<decideOrientation public=\"1\" set=\"method\" line=\"1468\"><f a=\"enemies:target:targetedBy\">\n\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t<c path=\"troshx.tros.ai.TROSAiBot\"/>\n\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t<x path=\"Void\"/>\n</f></decideOrientation>\n\t<decideTarget public=\"1\" set=\"method\" line=\"1472\"><f a=\"enemies\">\n\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t<x path=\"Void\"/>\n</f></decideTarget>\n\t<preDeclareManuevers public=\"1\" set=\"method\" line=\"1477\"><f a=\"target:targetedBy\">\n\t<c path=\"troshx.tros.ai.TROSAiBot\"/>\n\t<c path=\"Array\"><c path=\"troshx.tros.ai.TROSAiBot\"/></c>\n\t<x path=\"Void\"/>\n</f></preDeclareManuevers>\n\t<declareManuevers public=\"1\" set=\"method\" line=\"1506\"><f a=\"\"><x path=\"Void\"/></f></declareManuevers>\n\t<declareManueverAgainstOpponent public=\"1\" set=\"method\" line=\"1518\"><f a=\"index\">\n\t<x path=\"Int\"/>\n\t<x path=\"Bool\"/>\n</f></declareManueverAgainstOpponent>\n\t<new public=\"1\" set=\"method\" line=\"1438\"><f a=\"\"><x path=\"Void\"/></f></new>\n\t<meta>\n\t\t<m n=\":directlyUsed\"/>\n\t\t<m n=\":expose\"/>\n\t\t<m n=\":rtti\"/>\n\t</meta>\n</class>";
...
troshx_tros_ai_TROSAiBot.COMBOS_LEN_INITIATIVE = 10;
...
troshx_tros_ai_TROSAiBot.P_THRESHOLD_BEYOND_FAVORABLE = 0.9;
...
troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DEFAULT = 1;
troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_LOCK = false;

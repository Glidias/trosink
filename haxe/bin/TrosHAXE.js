...
troshx_components_FightState.isAttackingChoice = function(choice) {
	return choice.manuever != null && choice.manuever.type == 2;
};
...
troshx_components_FightState.prototype = {
	...
	,getInitiativeTowards: function(fightState) {
		if(this.orientation == 0) if(this.initiative) {
			if(fightState.initiative) return 1; else return 2;
		} else return 0; else if(this.orientation != 1) {
			if(this.orientation != fightState.orientation) if(this.orientation > fightState.orientation) return 2; else return 0; else if(this.orientation == 2) {
				if(fightState.orientation != 2) throw new js__$Boot_HaxeError("Equal assertion cautious failed:" + fightState.orientation);
				if(this.initiative != fightState.initiative) return 10; else return -10;
			} else if(this.orientation == 3) {
				if(fightState.orientation != 3) throw new js__$Boot_HaxeError("Equal assertion aggressive failed:" + fightState.orientation);
				return 1;
			} else throw new js__$Boot_HaxeError("Missed out this case?? Orientation: " + this.orientation);
		} else return 0;
	}
	,manuevers: null
	,enemyManuevers: null
	...
	,syncStepWith: function(fight) {
		if(fight.s >= this.s) this.s = fight.s; else fight.s = this.s;
	}
	,isSyncedWith: function(fight) {
		return this.s == fight.s && this.e == fight.e;
	}
	...
	,checkContestAgainstDefense: function(defManueverChoice) {
		var primary = this.manuevers[0];
		var secondary;
		if(this.manuevers.length > 1) secondary = this.manuevers[1]; else secondary = null;
		return troshx_components_FightState.isAttackingChoice(primary) && primary.to == defManueverChoice.from || secondary != null && troshx_components_FightState.isAttackingChoice(secondary) && secondary.to == defManueverChoice.from;
	}
	,getPrimaryManuever: function() {
		return this.manuevers[0];
	}
	,getSecondaryManuever: function() {
		if(this.manuevers.length > 1) return this.manuevers[1]; else return null;
	}
	,getPrimaryEnemyManuever: function() {
		if(this.enemyManuevers.length > 0) return this.enemyManuevers[0]; else return null;
	}
...
troshx_core_BodyChar.prototype = {
	getAllWoundLocations: function() {
		...
		while( $it0.hasNext() ) {
			...
			if(Object.prototype.hasOwnProperty.call(this.partsCut,f3)) woundLocation.cut = troshx_core_BodyChar.getCleanArrayOfWound(Reflect.field(this.partsCut,f3));
			if(Object.prototype.hasOwnProperty.call(this.partsPuncture,f3)) woundLocation.puncture = troshx_core_BodyChar.getCleanArrayOfWound(Reflect.field(this.partsPuncture,f3));
			if(Object.prototype.hasOwnProperty.call(this.partsBludgeon,f3)) woundLocation.bludgeon = troshx_core_BodyChar.getCleanArrayOfWound(Reflect.field(this.partsBludgeon,f3));
...
troshx_tros_ai_TROSAiBot.getDesperateFlee = function(availableCP,secondExchange) {
	if(troshx_tros_ai_TROSAiBot.AVAIL_fullevasion > 0) {
		...
		if(troshx_core_GameRules.FLEE_CAP == 0 || troshx_core_GameRules.FLEE_CAP == 1 && secondExchange) cpToUseForFleeing = availableCP; else if(availableCP > 6) cpToUseForFleeing = 6; else cpToUseForFleeing = availableCP;
...
troshx_tros_ai_TROSAiBot.getCPMaxForFleeing = function(availableCP,secondExchange) {
	if(troshx_core_GameRules.FLEE_CAP == 0 || troshx_core_GameRules.FLEE_CAP == 1 && secondExchange) return availableCP; else if(availableCP > 6) return 6; else return availableCP;
};
...
troshx_tros_ai_TROSAiBot.prototype = {
	...
	,getOpponentIndex: function(oppId) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			if(this.opponents[i].id == oppId) return i;
		}
		return -1;
	}

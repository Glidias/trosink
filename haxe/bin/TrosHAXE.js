...
troshx_tros_ai_TROSAiBot.getRegularAdvantageMove = function(availableCP,roll,againstRoll,againstTN) {
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_beat > 0 && tn != 888) {
		...
		if(cost < availableCP) {
			...
			if(gotEnemyRoll) aggr = Math.round(troshx_util_TROSAI.getChanceToSucceedContest(roll - cost,tn1,againstRoll,againstTN,1) * 1000); else aggr = tnP * (roll - cost);
...
troshx_tros_ai_TROSAiBot.getATNOfManuever = function(manuever) {
	var weapon = troshx_tros_WeaponSheet.getWeaponByName(troshx_tros_ai_TROSAiBot.B_EQUIP);
	var weaponOff;
	if(troshx_tros_ai_TROSAiBot.B_IS_OFFHAND) weaponOff = weapon; else weaponOff = troshx_tros_WeaponSheet.getWeaponByName(troshx_tros_ai_TROSAiBot.D_EQUIP);
	var tn;
	if(weapon != null) tn = weapon.atn; else tn = 888;
	var tn2;
	if(weapon != null) tn2 = weapon.atn2; else tn2 = 888;
	var tnOff;
	if(weaponOff != null) {
		if(weaponOff.shield) tnOff = weaponOff.dtn; else tnOff = weaponOff.atn;
	} else tnOff = 888;
	var cost;
	var aggr;
	switch(manuever) {
...
troshx_tros_ai_TROSAiBot.getFleeOrDefend = function(favorable,availableCP,againstRoll,againstTN,heuristic,flags,customThreshold,secondExchange) {
	...
	if(resultFirst && (flags & 1) != 0) return resultFirst;
	...
	var resultSecond = troshx_tros_ai_TROSAiBot.getFBDefense(favorable,availableCP,againstRoll,againstTN,heuristic,flags);
	if(resultSecond) {
		if(!resultFirst || troshx_util_TROSAI.getChanceToSucceedContest(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP,troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverTN,againstRoll,againstTN,1,true) >= fleeLikelihood) return resultSecond; else return troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
	} else return resultFirst && troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
};
troshx_tros_ai_TROSAiBot.minimalDefenseFirst = function(availableCP,againstRoll,againstTN,enemyCPInReserve) {
	if(enemyCPInReserve == null) enemyCPInReserve = 0;
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	var cp;
	if(troshx_tros_ai_TROSAiBot.getFlee(false,availableCP,enemyCPInReserve,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedATN(),false,1,0,true)) {
		...
		if(troshx_tros_ai_TROSAiBot.getFBDefense(false,cp,againstRoll,againstTN,true,6,0.0001)) return true;
	}
	return false;
};
...
troshx_tros_ai_TROSAiBot.lastDitchFleeOrDefend = function(availableCP,threatManuever,secondExchange,enemyCPInReserve) {
	...
	if(secondExchange == null) secondExchange = false;
	var cp;
	var againstRoll = threatManuever.manueverCP;
	var againstTN = threatManuever.manueverTN;
	var isDamagingThreat = troshx_tros_ManueverSheet.isDamagingManuever(threatManuever.manuever);
	if(enemyCPInReserve < 0) enemyCPInReserve = 0;
	if(!secondExchange) {
		if(isDamagingThreat && troshx_tros_ai_TROSAiBot.AVAIL_fullevasion > 0) {
			if(troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCP,againstRoll,againstTN,true,secondExchange?2:0,0) || troshx_tros_ai_TROSAiBot.getDesperateFleeOrDefend(availableCP,againstRoll,againstTN,true,secondExchange?2:0,0,secondExchange,0,0,isDamagingThreat)) {
				if(!(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manuever == "fullevasion")) {
					cp = availableCP - troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.getManueverCPSpent();
					var cpToUseForMinimalFlee;
					if(enemyCPInReserve >= troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DMG_DEFAULT && (cpToUseForMinimalFlee = troshx_tros_ai_TROSAiBot.checkCostViabilityWithBs(cp,4,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,enemyCPInReserve,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedATN(),false,1)) == 0) {
						if(troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCP,againstRoll,againstTN,true,4,0.0001)) {
							if((cpToUseForMinimalFlee = troshx_tros_ai_TROSAiBot.checkCostViabilityWithBs(availableCP - troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.getManueverCPSpent(),4,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,enemyCPInReserve,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedATN(),false,troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DMG_DEFAULT)) > 0) {
								console.log("minimal defense first with flee successful");
								return true;
							}
						}
					} else {
						console.log("defending with armor ok");
						return true;
					}
				} else {
					console.log("fleeing should be ok..but forced to..");
					return true;
				}
			} else return false;
		} else if(isDamagingThreat && troshx_tros_ai_TROSAiBot.minimalDefenseFirst(availableCP,againstRoll,againstTN,enemyCPInReserve)) return true;
	} else {
	}
	console.log("bopian flee or defend");
	return troshx_tros_ai_TROSAiBot.getDesperateFleeOrDefend(availableCP,againstRoll,againstTN,true,secondExchange?2:0,0,secondExchange,enemyCPInReserve,0,isDamagingThreat);
};
troshx_tros_ai_TROSAiBot.getDesperateFleeOrDefend = function(availableCP,againstRoll,againstTN,heuristic,flags,customThreshold,secondExchange,enemyCPInReserve,fleeVsDefendMarginMax,useArmor) {
	if(useArmor == null) useArmor = true;
	if(fleeVsDefendMarginMax == null) fleeVsDefendMarginMax = 0;
	if(enemyCPInReserve == null) enemyCPInReserve = 0;
	if(secondExchange == null) secondExchange = false;
	if(customThreshold == null) customThreshold = 0;
	if(flags == null) flags = 0;
	...
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	var resultFirst = troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
	if(resultFirst && (flags & 1) != 0) return true;
	var fleeLikelihood = troshx_util_TROSAI.getChanceToSucceedContest(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP,troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverTN,againstRoll,againstTN,1,true);
	var availableCPForRegularDef = availableCP;
	if(enemyCPInReserve > 0) {
		var reduc;
		if((reduc = troshx_tros_ai_TROSAiBot.checkCostViabilityWithBs(availableCP,4,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,enemyCPInReserve,troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedATN(),false,troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DMG_DEFAULT)) > 0) availableCPForRegularDef -= reduc; else if(resultFirst) return true;
	}
	var dmgBarrier;
	if(useArmor) dmgBarrier = troshx_tros_ai_TROSAiBot.B_BS_REQUIRED_DMG_DEFAULT; else dmgBarrier = 1;
	var singleExchangeOverwrite = false;
	if(singleExchangeOverwrite) fleeVsDefendMarginMax = 0.1;
	var resultSecond;
	if(!useArmor) resultSecond = troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCPForRegularDef,againstRoll,againstTN,heuristic,flags,customThreshold); else if(singleExchangeOverwrite) resultSecond = troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCPForRegularDef,againstRoll,againstTN,true,2,0.0001); else resultSecond = troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCPForRegularDef,againstRoll,againstTN,true,flags | 4,0.0001);
	if(!resultSecond && !resultFirst) {
		if(troshx_tros_ai_TROSAiBot.getFBDefense(false,availableCP,againstRoll,againstTN,true,flags | 4,0.0001)) return true;
		var def = troshx_tros_ai_TROSAiBot.getRegularDefense(availableCP,0,false);
		if(def != null) {
			availableCP -= troshx_tros_ai_TROSAiBot.getCostOfManuever(def);
			if(availableCP > 0) {
				troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend(def,(flags & 2) != 0?availableCP:Std["int"](Math.ceil(availableCP * .5)),troshx_tros_ai_TROSAiBot.getDTNOfManuever(def),troshx_tros_ai_TROSAiBot.getCostOfManuever(def),troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
				return true;
			}
		} else return false;
	}
	var defLikelihood = troshx_util_TROSAI.getChanceToSucceedContest(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverCP,troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manueverTN,againstRoll,againstTN,1,true);
	if(troshx_tros_ai_TROSAiBot.WISH_TO_REGAIN_STANCE && resultFirst && fleeLikelihood > defLikelihood) return troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
	if(resultSecond) {
		if(!resultFirst || !troshx_tros_ai_TROSAiBot.weightedChoiceBetween2(fleeLikelihood,defLikelihood,fleeVsDefendMarginMax)) return resultSecond; else return troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
	} else return resultFirst && troshx_tros_ai_TROSAiBot.getDesperateFlee(availableCP,secondExchange);
};
...
troshx_tros_ai_TROSAiBot.weightedChoiceBetween2 = function(trueConditionProbability,falseConditionProbability,maxMarginOfDifference) {
	if(maxMarginOfDifference != 0 && (trueConditionProbability > falseConditionProbability?trueConditionProbability - falseConditionProbability:falseConditionProbability - trueConditionProbability) > maxMarginOfDifference) return trueConditionProbability >= falseConditionProbability; else return Math.random() * (trueConditionProbability + falseConditionProbability) <= trueConditionProbability;
};
troshx_tros_ai_TROSAiBot.absDiffF = function(val,val2) {
	if(val > val2) return val - val2; else return val2 - val;
};
troshx_tros_ai_TROSAiBot.getDesperateFlee = function(availableCP,secondExchange) {
	if(troshx_tros_ai_TROSAiBot.AVAIL_fullevasion > 0) {
		...
		troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setDefend("fullevasion",cpToUseForFleeing,troshx_tros_ai_TROSAiBot.getDTNOfManuever("fullevasion"),troshx_tros_ai_TROSAiBot.AVAIL_fullevasion - 1,false);
		return true;
	}
	return false;
};
...
troshx_tros_ai_TROSAiBot.getCPMaxForFleeing = function(availableCP,secondExchange) {
	if(troshx_GameRules.FLEE_CAP == 0 || troshx_GameRules.FLEE_CAP == 1 && secondExchange) return availableCP; else if(availableCP > 6) return 6; else return availableCP;
};
troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao = function(cp,cp2,threatManuever,secondExchange) {
	...
	return troshx_tros_ai_TROSAiBot.getSpecialDefWithReturnAttack(cp,cp2,threatManuever.manueverCP,threatManuever.manueverTN,0,secondExchange) || troshx_tros_ai_TROSAiBot.lastDitchFleeOrDefend(cp,threatManuever,secondExchange,cp2 - threatManuever.manueverCP);
};
troshx_tros_ai_TROSAiBot.tryBestPossibleAttackBoliao = function(cp,cp2,threatManuever,secondExchange) {
	...
	if(atk != null) {
		var cost;
		cp -= cost = troshx_tros_ai_TROSAiBot.getCostOfManuever(atk);
		if(cp > 0) {
			var atn = troshx_tros_ai_TROSAiBot.getATNOfManuever(atk);
			var cpToUse = troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline(cp,cp2,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,secondExchange?cp2:Std["int"](Math.ceil(cp2 * .5)),troshx_tros_ai_TROSAiBot.CURRENT_OPPONENT.getPredictedDTN(),secondExchange);
			troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.setAttack(atk,secondExchange?cp:cpToUse > 0?cpToUse:troshx_tros_ai_TROSAiBot.CAN_DESPERATE_ALPHASTRIKE && Math.random() < troshx_tros_ai_TROSAiBot.DESPERATE_ALPHASTRIKE_CHANCE?cp:Std["int"](Math.ceil(cp * .5)),atn,troshx_tros_ai_TROSAiBot.getRegularTargetZone(atk,atn,cp),cost,troshx_tros_ai_TROSAiBot.B_IS_OFFHAND);
			return true;
		}
	}
	return false;
...
troshx_tros_ai_TROSAiBot.tryBestPossibleAttackOrDefBoliao = function(cp,cp2,threatManuever,secondExchange) {
	...
	if(secondExchange) {
		if(troshx_tros_ai_TROSAiBot.WISH_TO_REGAIN_STANCE) {
			if(troshx_tros_ai_TROSAiBot.tryBestPossibleDefWithInitiativeBoliao(cp,cp2,threatManuever,secondExchange)) return true;
		}
...
troshx_tros_ai_TROSAiBot.tryBestPossibleDefWithInitiativeBoliao = function(cp,cp2,threatManuever,secondExchange) {
	...
	if(troshx_tros_ai_TROSAiBot.getFleeOrDefend(false,cp,troshx_tros_ai_TROSAiBot.MOCK_RETURN_ATTACK.manueverCP,troshx_tros_ai_TROSAiBot.MOCK_RETURN_ATTACK.manueverTN,true,0,0,secondExchange)) {
		if(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.manuever == "fullevasion") result = true; else if(cp > 2 && troshx_tros_ai_TROSAiBot.getFBDefense(false,cp - 2,troshx_tros_ai_TROSAiBot.MOCK_RETURN_ATTACK.manueverCP,troshx_tros_ai_TROSAiBot.MOCK_RETURN_ATTACK.manueverTN,true,secondExchange?2:0,0)) {
			troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.waitForQuickDefense();
			result = true;
		}
	}
	return result;
...
troshx_tros_ai_TROSAiBot.getAdvantageManuever = function(manueverName,favorable,availableCP,againstRoll,againstTN,flags,customThreshold,preferedRS,defensive,alwaysUseOffhand) {
	if(alwaysUseOffhand == null) alwaysUseOffhand = false;
...
troshx_tros_ai_TROSAiBot.getSpecialDefWithReturnAttack = function(availableCP,againstCP,againstRoll,againstTN,customThreshold,secondExchange) {
	if(secondExchange == null) secondExchange = false;
	if(customThreshold == null) customThreshold = 0;
	var minCPRemainRequired;
	var cost;
	var cpToUse;
	var weapon = troshx_tros_WeaponSheet.getWeaponByName(troshx_tros_ai_TROSAiBot.B_EQUIP);
	var offhandWeap = troshx_tros_WeaponSheet.getWeaponByName(troshx_tros_ai_TROSAiBot.D_EQUIP);
	var dtn;
	if(weapon != null) dtn = weapon.dtn; else dtn = 0;
	var blockDTN;
	if(offhandWeap != null) blockDTN = offhandWeap.dtn; else blockDTN = 0;
	var atn;
	if(weapon != null) atn = weapon.atn; else atn = 0;
	var atn2;
	if(weapon != null) atn2 = weapon.atn2; else atn2 = 0;
	...
	var initialCount = 0;
	var threshold;
	if(customThreshold != 0) threshold = customThreshold; else threshold = troshx_tros_ai_TROSAiBot.P_THRESHOLD_FAVORABLE;
	if(troshx_tros_ai_TROSAiBot.AVAIL_counter > 0) {
		...
		if(troshx_tros_ai_TROSAiBot.checkCostViability(againstRoll,againstTN,troshx_tros_ai_TROSAiBot.P_THRESHOLD_ANTI_FAVORABLE,0,1,false) > 0) {
			...
			if(secondExchange) cpToSave = 0; else cpToSave = troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline(pool,usingReturnStrikeTN,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,enemyCPLeft,enemyDTN,false);
			...
			troshx_tros_ai_TROSAiBot.B_BS_REQUIRED = 1;
			if(availableCP > 0) cpToUse = troshx_tros_ai_TROSAiBot.checkCostViability(availableCP,dtn,threshold,againstRoll,againstTN,true); else cpToUse = 0;
			if(cpToUse > 0) {
				if(aggr <= curAggr) {
					if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = initialCount;
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_blockopenstrike > 0 && blockDTN > 0) {
		...
		troshx_tros_ai_TROSAiBot.B_BS_REQUIRED = 1;
		if(atn2 < atn) usingReturnStrikeTN = atn2; else usingReturnStrikeTN = atn;
		if(secondExchange) cpToSave = 0; else cpToSave = troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline(pool,usingReturnStrikeTN,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,enemyCPLeft,enemyDTN,false);
		...
		if(availableCP > 0) cpToUse = troshx_tros_ai_TROSAiBot.checkCostViability(availableCP,blockDTN,threshold,againstRoll,againstTN,true); else cpToUse = 0;
		if(cpToUse > 0) {
			if(aggr <= curAggr) {
				if(aggr != curAggr) troshx_tros_ai_TROSAiBot.B_MANUEVER_CHOICE_COUNT = initialCount;
	...
	if(troshx_tros_ai_TROSAiBot.AVAIL_expulsion > 0) {
		...
		if(secondExchange) cpToSave = 0; else cpToSave = troshx_tros_ai_TROSAiBot.checkCostViabilityBorderline(availableCP,usingReturnStrikeTN,troshx_tros_ai_TROSAiBot.P_THRESHOLD_BORDERLINE,troshx_tros_ai_TROSAiBot.maxI(0,enemyCPLeft - troshx_tros_ai_TROSAiBot.B_BS_REQUIRED),enemyDTN,false);
		aggr = dtn + usingReturnStrikeTN + troshx_tros_ai_TROSAiBot.getAverageOpponentCounterAV() / 8;
		aggr = cpToSave + cost + aggr / 20;
...
troshx_tros_ai_TROSAiBot.maxI = function(val,val2) {
	if(val >= val2) return val; else return val2;
};
...
troshx_tros_ai_TROSAiBot.getBindStrike = function(favorable,availableCP,againstCP,againstRoll,againstTN,flags,customThreshold,preferedRS) {
	...
	if(againstTN == null) againstTN = 1;
	if(againstRoll == null) againstRoll = 0;
	if(troshx_tros_ai_TROSAiBot.AVAIL_bindstrike > 0) return troshx_tros_ai_TROSAiBot.getAdvantageManuever("bindstrike",favorable,availableCP,againstRoll,againstTN,flags,customThreshold,preferedRS,false,true); else return false;
};
...
troshx_tros_ai_TROSAiBot.getComboAction = function(combo,cp,cp2,threatManuever,hasInitiative,secondExchange) {
	...
	case -2:
		...
		break;
	case -3:
		if(threatManuever != null) return troshx_tros_ai_TROSAiBot.getSpecialDefWithReturnAttack(cp,cp2,threatManuever.manueverCP,threatManuever.manueverTN,0,secondExchange);
		break;
	case -5:
...
troshx_tros_ai_TROSAiBot.getComboActionTest = function(combo,cp,cp2,threatManuever,hasInitiative,secondExchange) {
	if(secondExchange == null) secondExchange = false;
	if(hasInitiative == null) hasInitiative = true;
	return troshx_tros_ai_TROSAiBot.getComboAction(combo,cp,cp2,threatManuever,hasInitiative,secondExchange) || (hasInitiative?troshx_tros_ai_TROSAiBot.tryBestPossibleAttackOrDefBoliao(cp,cp2,threatManuever,secondExchange):troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao(cp,cp2,threatManuever,secondExchange));
};
...
troshx_tros_ai_TROSAiBot.setBestComboActionWithInitiativePlanTest = function(cp,cp2,threatManuever) {
	if(troshx_tros_ai_TROSAiBot.setBestComboActionWithInitiativePlan(cp,cp2,threatManuever)) return true; else if(troshx_tros_ai_TROSAiBot.tryBestPossibleAttackOrDefBoliao(cp,cp2,threatManuever,false)) {
		troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.copyTo(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE_SET,null);
		return true;
	}
	return false;
};
...
troshx_tros_ai_TROSAiBot.setBestComboActionWithoutInitiativePlanTest = function(cp,cp2,threatManuever) {
	if(troshx_tros_ai_TROSAiBot.setBestComboActionWithoutInitiativePlan(cp,cp2,threatManuever)) return true; else if(troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao(cp,cp2,threatManuever,false)) {
		troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE.copyTo(troshx_tros_ai_TROSAiBot.MANUEVER_CHOICE_SET,null);
		return true;
	}
	return false;
...
troshx_tros_ai_TROSAiBot.prototype = {
	...
	,declareManueverAgainstOpponent: function(index) {
		...
		if(this.initiative) return troshx_tros_ai_TROSAiBot.tryBestPossibleAttackOrDefBoliao(this.cp,opponent.cp,threatManuever,this.currentExchange == 2); else return troshx_tros_ai_TROSAiBot.tryBestPossibleDefenseBoliao(this.cp,opponent.cp,threatManuever,this.currentExchange == 2);
		return false;
	}
...
troshx_tros_Manuever.DAMAGE_TYPE_NONE = 0;

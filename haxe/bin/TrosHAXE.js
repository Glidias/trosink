...
troshx_components_Bout.prototype = {
	...
	,setupAllNewSideLinks: function() {
		var n = null;
		var _g1 = 0;
		var _g = this.combatants.length;
		while(_g1 < _g) {
			var a = _g1++;
			var _g3 = a + 1;
			var _g2 = this.combatants.length;
			while(_g3 < _g2) {
				var b = _g3++;
				if(this.combatants[a].sideIndex != this.combatants[b].sideIndex) {
					if(n != null) {
						n = n.next = new troshx_components_FightLink(this.combatants[a],this.combatants[b]);
					} else {
						n = this.links = new troshx_components_FightLink(this.combatants[a],this.combatants[b]);
					}
				}
			}
		}
		this.linkUpdateCount++;
	}
	,pushNewFightNode: function(node) {
		var l = null;
		var gotUpdateLinks = false;
		if(node.targetLink == null) {
			var _g1 = 0;
			var _g = this.combatants.length;
			while(_g1 < _g) {
				var a = _g1++;
				var candidate = this.combatants[a];
				if(candidate.sideIndex != node.sideIndex) {
					l = new troshx_components_FightLink(node,candidate);
					node.targetLink = l;
					if(candidate.targetLink == null) {
		...
		this.combatants.push(node);
	}
	,clearLinksWithCondition: function(cond) {
		var n = this.links;
		var gotChange = false;
		var next;
		var last = null;
	...
	,findLinkedOpponents: function(a) {
		var arr = [];
		var n = this.links;
		while(n != null) {
			if(n.a == a || n.b == a) {
	...
	,countLinkedOpponents: function(a) {
		var count = 0;
		var n = this.links;
		while(n != null) {
			if(n.a == a || n.b == a) {
	...
	,removeLinksFromAgainst: function(from,against) {
		return this.clearLinksWithCondition(function(c) {
			if(c.a == from) {
				return against.indexOf(c.b) >= 0;
			} else if(c.b == from) {
				return against.indexOf(c.a) >= 0;
			} else {
				return false;
			}
		});
	}
	,purgeDeadLinks: function() {
		return this.clearLinksWithCondition(function(c) {
			if(!c.a.fight.dead) {
				return c.b.fight.dead;
			} else {
				return true;
			}
		});
...
troshx_components_FightNode.prototype = {
	getTargetOpponent: function() {
		if(this.targetLink != null) {
			if(this.targetLink.a != this) {
				return this.targetLink.a;
			} else {
				return this.targetLink.b;
			}
		} else {
			return null;
		}
...
var troshx_components_FightLink = function(a,b) {
	this.reach = 0;
	this.a = a;
	this.b = b;
	Object.defineProperty(this,"next",{ enumerable : false, iterable : false, writable : true, configurable : false});
	Object.defineProperty(this,"a",{ enumerable : false, iterable : false, writable : false, configurable : false});
	Object.defineProperty(this,"b",{ enumerable : false, iterable : false, writable : false, configurable : false});
};
$hxClasses["troshx.components.FightLink"] = troshx_components_FightLink;
troshx_components_FightLink.__name__ = ["troshx","components","FightLink"];
troshx_components_FightLink.prototype = {
	findNodeSide: function(node) {
		if(node == this.a) {
			return -1;
		} else if(node == this.b) {
			return 1;
	...
	,__class__: troshx_components_FightLink
};
...
var troshx_core_IBodyChar = function() { };
$hxClasses["troshx.core.IBodyChar"] = troshx_core_IBodyChar;
troshx_core_IBodyChar.__name__ = ["troshx","core","IBodyChar"];
troshx_core_IBodyChar.prototype = {
	__class__: troshx_core_IBodyChar
};
...
var troshx_core_ManueverSpec = function() {
	this.replyTo = null;
	this.activeEnemyItem = null;
	this.activeEnemyZone = -1;
	this.activeEnemyBody = null;
	this.usingLeftLimb = false;
	this.activeItem = null;
	this.typePreference = 0;
};
$hxClasses["troshx.core.ManueverSpec"] = troshx_core_ManueverSpec;
troshx_core_ManueverSpec.__name__ = ["troshx","core","ManueverSpec"];
troshx_core_ManueverSpec.prototype = {
	...
	,setNewEnemy: function(body) {
		this.activeEnemyZone = -1;
		this.activeEnemyBody = body;
		this.activeEnemyItem = null;
	}
	,__class__: troshx_core_ManueverSpec
};
...
troshx_sos_core_BodyChar.__interfaces__ = [troshx_core_IBodyChar];
...
troshx_sos_core_BodyChar.prototype = {
	...
	,getDescLabelTargetZone: function(zoneIndex,thrustLabel,swingLabel,descOverwites) {
		if(swingLabel == null) {
			swingLabel = "Swing to";
		}
		if(thrustLabel == null) {
			thrustLabel = "Thrust to";
		}
...
troshx_sos_core_Inventory.weaponFromAssign = function(w) {
...
troshx_sos_core_Inventory.prototype = {
	...
	,findOffHandAssign: function() {
		...
		while(_g12 < _g3) {
			var i2 = _g12++;
			if(this.equipedNonMeleeItems[i2].held == 2) {
				return this.equipedNonMeleeItems[i2];
			}
		}
		return null;
	}
	...
	,holdBackWeaponNormalise: function(masterWeaponAssign) {
		if(masterWeaponAssign == null) {
			masterWeaponAssign = this.getMasterWeaponAssign();
		}
		if(masterWeaponAssign == null) {
			return false;
		}
		if(masterWeaponAssign.holding1H && (masterWeaponAssign.weapon.flags & 1) != 0 && this.findOffHandAssign() == null) {
			masterWeaponAssign.holding1H = false;
			masterWeaponAssign.held = 3;
			return true;
		}
		return false;
	}
	...
	,_unholdAllItems: function(held,isForShield) {
		...
		while(_g1 < _g) {
			...
			if((w.weapon.flags & 2) == 0) {
				w.held &= ~held;
				if(held != 0 && (w.weapon.flags & 1) != 0 && w.held == 3) {
					w.held = 0;
				}
			}
		}
		...
		while(_g11 < _g2) {
			...
			if((s.shield.flags & 2) == 0 || isForShield) {
				s.held &= ~held;
				if(held != 0 && (s.shield.flags & 1) != 0 && s.held == 3) {
					s.held = 0;
				}
			}
		}
		...
		while(_g12 < _g3) {
			...
			if((t.item.flags & 2) == 0) {
				t.held &= ~held;
				if(held != 0 && (t.item.flags & 1) != 0 && t.held == 3) {
					t.held = 0;
				}
			}
		}
...
troshx_sos_core_Manuever.specificTargetZoneModeMask = function(val) {
	return -val;
};
troshx_sos_core_Manuever.hasInitiative = function(node,link) {
	return (node == link.a ? -1 : node == link.b ? 1 : 0) == link.initiative;
};
...
troshx_sos_core_Manuever.prototype = {
	...
	,_requisite: function(val,reqStuffs) {
		this.requisites = val;
		this.reqStuffs = reqStuffs;
		return this;
	}
	,getAvailability: function(bout,node,spec) {
		return true;
	}
	,validForTargetZone: function(spec) {
		if(this.targetZoneMode != 0) {
			if(this.targetZoneMode == 1) {
				return js_Boot.__instanceof(spec.activeEnemyItem,troshx_sos_core_Weapon);
			} else if(this.targetZoneMode == 2) {
				return js_Boot.__instanceof(spec.activeEnemyItem,troshx_sos_core_Shield);
			}
		}
		if(this.targetZoneMode == 0 || (this.targetZoneMode & 4) != 0) {
			if(this.attackTypes == 0 || spec.activeEnemyZone < 0) {
				return true;
			}
			var tzMouse = spec.activeEnemyBody.isThrusting(spec.activeEnemyZone) ? 2 : 1;
			var atks = this.attackTypes;
			if((atks & 12) != 0) {
				atks |= 2;
			}
	...
	,_targetZoneMode: function(val) {
		this.targetZoneMode = val;
		return this;
	}
	,_superior: function(val) {
		var manuever = val != null ? val : this.clone();
		manuever.isSuperior = true;
		return this;
	}
	,_superiorInit: function(func) {
		var manuever = this.clone();
		manuever.isSuperior = true;
		func(manuever);
		return this;
	}
	...
	,clonePropertiesFrom: function(manuever) {
		manuever.name = this.name;
		manuever.cost = this.cost;
		manuever.costFuncInputs = this.costFuncInputs;
		manuever.types = this.types;
		manuever.attackTypes = this.attackTypes;
		manuever.requisites = this.requisites;
		manuever.reqStuffs = this.reqStuffs;
		manuever.targetZoneMode = this.targetZoneMode;
		manuever.tags = this.tags;
		manuever.usingHands = this.usingHands;
		manuever.tn = this.tn;
		manuever.bs = this.bs;
		manuever.damage = this.damage;
		manuever.damageType = this.damageType;
		manuever.stun = this.stun;
		manuever.reach = this.reach;
	}
	...
	,isDeferedCost: function() {
		return this.costFuncInputs == -2;
	}
	,_costs: function(cost,costFuncInputs,costVaries) {
		if(costVaries == null) {
			costVaries = false;
		}
	...
	,getMaxInvest: function(bout,node,spec) {
		return -1;
	}
	,_damage: function(type,val,stun) {
		if(stun == null) {
			stun = 0;
		}
	...
	,hasOwnDamage: function() {
		return this.damageType >= 0;
	}
	,handleEvent: function(sheet,fightState,declare,event) {
		return false;
	}
	,getCost: function(bout,node,inputs) {
		return this.cost;
	}
	,resolve: function(bout,node,declare) {
	}
	,getTN: function(spec) {
		if(this.requisites != 0) {
			return this.getRequisiteTN(spec);
		} else {
			return this.tn;
		}
	}
	,getRequisiteTN: function(spec) {
		...
		var shield = js_Boot.__instanceof(activeItem,troshx_sos_core_Shield) ? activeItem : null;
		var weapon = js_Boot.__instanceof(activeItem,troshx_sos_core_Weapon) ? activeItem : null;
		var reqMilitaryArms = 0;
		reqMilitaryArms |= shield != null ? 2 : 0;
		reqMilitaryArms |= weapon != null ? 1 : 0;
		if((req & 8) != 0) {
			if(activeItem == null) {
				return -1;
			} else if(this.reqStuffs != null) {
				if(this.reqStuffs.indexOf(activeItem.name) < 0) {
					return -1;
				}
			} else if(req == 8 && reqMilitaryArms != 0) {
				return -1;
			}
		}
		req &= 7;
		if((req & 4) != 0) {
			if(activeItem != null && !this.itemAsUnarmedAllowed(activeItem)) {
				return -1;
			}
		}
		req &= 3;
		...
		if((req & 1) != 0) {
			if((types & 2) != 0) {
				if(this.reach < 0) {
					rtn = weapon.atnM;
				} else if(spec.activeEnemyBody.isThrusting(spec.activeEnemyZone)) {
					rtn = weapon.atnT;
				} else {
					rtn = weapon.atnS;
				}
	...
	,itemAsUnarmedAllowed: function(item) {
		return false;
	}
	,get_uid: function() {
		return this.id;
...
var troshx_sos_manuevers_ArmParry = function() {
	troshx_sos_core_Manuever.call(this,"armParry","Arm Parry");
	this._types(1)._requisite(4)._tags(64)._tn(7)._superiorInit(function(m) {
		m._tn(6);
	});
};
$hxClasses["troshx.sos.manuevers.ArmParry"] = troshx_sos_manuevers_ArmParry;
troshx_sos_manuevers_ArmParry.__name__ = ["troshx","sos","manuevers","ArmParry"];
troshx_sos_manuevers_ArmParry.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_ArmParry.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getTN: function(spec) {
		var tn = troshx_sos_core_Manuever.prototype.getTN.call(this,spec);
		if(tn < 0) {
			return tn;
		}
		var usingLeftLimb = spec.usingLeftLimb;
		if(spec.replyTo != null && ((spec.replyTo.targetZone == 4 || spec.replyTo.targetZone == 5 || spec.replyTo.targetZone == 14 || spec.replyTo.targetZone == 13) && spec.replyTo.targetZonePreferLeft == usingLeftLimb)) {
			++tn;
		}
		return tn;
	}
	,getAvailability: function(bout,node,spec) {
		var obj = spec.activeItem;
		var shield = js_Boot.__instanceof(obj,troshx_sos_core_Shield) ? obj : null;
		return shield == null;
	}
	,__class__: troshx_sos_manuevers_ArmParry
});
var troshx_sos_manuevers_Beat = function() {
	troshx_sos_core_Manuever.call(this,"beat","Beat");
	this._types(2)._requisite(1)._attackTypes(1)._targetZoneMode(3)._superior();
};
$hxClasses["troshx.sos.manuevers.Beat"] = troshx_sos_manuevers_Beat;
troshx_sos_manuevers_Beat.__name__ = ["troshx","sos","manuevers","Beat"];
troshx_sos_manuevers_Beat.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Beat.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var tempo = !bout.secondTempo;
		var round = this.isSuperior ? true : bout.roundCount < 2;
		if(tempo) {
			return round;
		} else {
			return false;
	...
	,__class__: troshx_sos_manuevers_Beat
});
var troshx_sos_manuevers_ShieldBeat = function() {
	troshx_sos_core_Manuever.call(this,"shieldBeat","Shield Beat");
	this._types(2)._requisite(2)._attackTypes(0)._targetZoneMode(3)._reach(3)._costs(2);
};
$hxClasses["troshx.sos.manuevers.ShieldBeat"] = troshx_sos_manuevers_ShieldBeat;
troshx_sos_manuevers_ShieldBeat.__name__ = ["troshx","sos","manuevers","ShieldBeat"];
troshx_sos_manuevers_ShieldBeat.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_ShieldBeat.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_ShieldBeat
});
var troshx_sos_manuevers_BlindToss = function() {
	troshx_sos_core_Manuever.call(this,"blindToss","Blind Toss");
	this._types(2)._requisite(8)._tn(5)._costs(0,0,true);
};
$hxClasses["troshx.sos.manuevers.BlindToss"] = troshx_sos_manuevers_BlindToss;
troshx_sos_manuevers_BlindToss.__name__ = ["troshx","sos","manuevers","BlindToss"];
troshx_sos_manuevers_BlindToss.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_BlindToss.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		return node.targetLink.reach <= 7;
	}
	,__class__: troshx_sos_manuevers_BlindToss
});
var troshx_sos_manuevers_Break = function() {
	troshx_sos_core_Manuever.call(this,"break","Break");
	this._types(2)._requisite(1)._attackTypes(1)._targetZoneMode(1)._costs(2)._superior();
};
$hxClasses["troshx.sos.manuevers.Break"] = troshx_sos_manuevers_Break;
troshx_sos_manuevers_Break.__name__ = ["troshx","sos","manuevers","Break"];
troshx_sos_manuevers_Break.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Break.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_Break
});
var troshx_sos_manuevers_CleavingBlow = function() {
	troshx_sos_core_Manuever.call(this,"cleavingBlow","Cleaving Blow");
	this._types(2)._requisite(1)._attackTypes(1)._tags(1)._costs(2,-2);
};
$hxClasses["troshx.sos.manuevers.CleavingBlow"] = troshx_sos_manuevers_CleavingBlow;
troshx_sos_manuevers_CleavingBlow.__name__ = ["troshx","sos","manuevers","CleavingBlow"];
troshx_sos_manuevers_CleavingBlow.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_CleavingBlow.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		var multipleOpp = bout.countLinkedOpponents(node) > 1;
		if(weapon.reach >= 5) {
			return multipleOpp;
		} else {
			return false;
	...
	,__class__: troshx_sos_manuevers_CleavingBlow
});
var troshx_sos_manuevers_Disarm = function() {
	troshx_sos_core_Manuever.call(this,"disarm","Disarm");
	this._types(2)._requisite(1)._costs(1)._attackTypes(1)._targetZoneMode(1)._superior();
};
$hxClasses["troshx.sos.manuevers.Disarm"] = troshx_sos_manuevers_Disarm;
troshx_sos_manuevers_Disarm.__name__ = ["troshx","sos","manuevers","Disarm"];
troshx_sos_manuevers_Disarm.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Disarm.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_Disarm
});
var troshx_sos_manuevers_DisarmUnarmedAtk = function() {
	troshx_sos_core_Manuever.call(this,"disarmUnarmedAtk","Disarm (Unarmed)");
	this._types(2)._requisite(4)._targetZoneMode(1)._costs(1)._reach(2)._tn(7)._superiorInit(function(m) {
		m._tn(6);
	});
};
$hxClasses["troshx.sos.manuevers.DisarmUnarmedAtk"] = troshx_sos_manuevers_DisarmUnarmedAtk;
troshx_sos_manuevers_DisarmUnarmedAtk.__name__ = ["troshx","sos","manuevers","DisarmUnarmedAtk"];
troshx_sos_manuevers_DisarmUnarmedAtk.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_DisarmUnarmedAtk.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_DisarmUnarmedAtk
});
var troshx_sos_manuevers_DisarmUnarmedDef = function() {
	troshx_sos_core_Manuever.call(this,"disarmUnarmedDef","Disarm (Unarmed)");
	this._types(1)._requisite(4)._tags(64)._costs(1)._reach(2)._tn(8)._superiorInit(function(m) {
		m._tn(7);
	});
};
$hxClasses["troshx.sos.manuevers.DisarmUnarmedDef"] = troshx_sos_manuevers_DisarmUnarmedDef;
troshx_sos_manuevers_DisarmUnarmedDef.__name__ = ["troshx","sos","manuevers","DisarmUnarmedDef"];
troshx_sos_manuevers_DisarmUnarmedDef.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_DisarmUnarmedDef.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_DisarmUnarmedDef
});
var troshx_sos_manuevers_DrawCut = function() {
	troshx_sos_core_Manuever.call(this,"drawCut","Deep Draw Cut");
	this._types(2)._requisite(1)._attackTypes(1);
};
$hxClasses["troshx.sos.manuevers.DrawCut"] = troshx_sos_manuevers_DrawCut;
troshx_sos_manuevers_DrawCut.__name__ = ["troshx","sos","manuevers","DrawCut"];
troshx_sos_manuevers_DrawCut.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_DrawCut.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		if(weapon.damageTypeS == 0) {
			if(weapon.meleeSpecial != null) {
				return weapon.meleeSpecial.draw > 0;
			} else {
				return false;
			}
		} else {
	...
	,__class__: troshx_sos_manuevers_DrawCut
});
var troshx_sos_manuevers_Feint = function() {
	troshx_sos_core_Manuever.call(this,"feint","Feint");
	this._types(2)._requisite(1)._attackTypes(3)._costs(2,-2)._superior();
};
$hxClasses["troshx.sos.manuevers.Feint"] = troshx_sos_manuevers_Feint;
troshx_sos_manuevers_Feint.__name__ = ["troshx","sos","manuevers","Feint"];
troshx_sos_manuevers_Feint.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Feint.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getSecondary: function() {
		if(this._shieldFeint != null) {
			return this._shieldFeint;
		} else {
			return this._shieldFeint = new troshx_sos_manuevers_ShieldFeint();
		}
	}
	,getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		var fluidThrusts = (weapon.meleeFlags & 16) != 0;
		if(!spec.activeEnemyBody.isThrusting(spec.activeEnemyZone) && fluidThrusts) {
			return false;
		}
		return true;
	}
	,__class__: troshx_sos_manuevers_Feint
});
var troshx_sos_manuevers_Flee = function() {
	troshx_sos_core_Manuever.call(this,"flee","Flee");
	this._types(1)._tags(128)._tn(5);
};
$hxClasses["troshx.sos.manuevers.Flee"] = troshx_sos_manuevers_Flee;
troshx_sos_manuevers_Flee.__name__ = ["troshx","sos","manuevers","Flee"];
troshx_sos_manuevers_Flee.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Flee.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var noAttacksNow = node.fight.attackManuevers.length == 0;
		var firstRound = bout.roundCount <= 1;
		var orientDef = node.fight.orientation == 3;
		if(!node.fight.lastAttacking && noAttacksNow) {
			if(!(!firstRound)) {
				return orientDef;
			} else {
				return true;
	...
	,__class__: troshx_sos_manuevers_Flee
});
var troshx_sos_manuevers_HastyVoid = function() {
	troshx_sos_core_Manuever.call(this,"hastyVoid","Hasty Void");
	this._types(1)._tags(128)._tn(7)._bs(2);
};
$hxClasses["troshx.sos.manuevers.HastyVoid"] = troshx_sos_manuevers_HastyVoid;
troshx_sos_manuevers_HastyVoid.__name__ = ["troshx","sos","manuevers","HastyVoid"];
troshx_sos_manuevers_HastyVoid.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_HastyVoid.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_HastyVoid
});
var troshx_sos_manuevers_Hew = function() {
	troshx_sos_core_Manuever.call(this,"hew","Hew");
	this._types(2)._requisite(1)._attackTypes(1)._targetZoneMode(2)._costs(1)._superior();
};
$hxClasses["troshx.sos.manuevers.Hew"] = troshx_sos_manuevers_Hew;
troshx_sos_manuevers_Hew.__name__ = ["troshx","sos","manuevers","Hew"];
troshx_sos_manuevers_Hew.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Hew.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_Hew
});
var troshx_sos_manuevers_Hook = function() {
	troshx_sos_core_Manuever.call(this,"hook","Hook");
	this._types(2)._requisite(1)._attackTypes(3)._targetZoneMode(6)._costs(1)._superior();
};
$hxClasses["troshx.sos.manuevers.Hook"] = troshx_sos_manuevers_Hook;
troshx_sos_manuevers_Hook.__name__ = ["troshx","sos","manuevers","Hook"];
troshx_sos_manuevers_Hook.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Hook.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		return (weapon.meleeFlags & 512) != 0;
	}
	,__class__: troshx_sos_manuevers_Hook
});
var troshx_sos_manuevers_JointThrust = function() {
	troshx_sos_core_Manuever.call(this,"jointThrust","Joint Thrust");
	this._types(2)._requisite(1)._attackTypes(2)._costs(0,0,true);
};
$hxClasses["troshx.sos.manuevers.JointThrust"] = troshx_sos_manuevers_JointThrust;
troshx_sos_manuevers_JointThrust.__name__ = ["troshx","sos","manuevers","JointThrust"];
troshx_sos_manuevers_JointThrust.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_JointThrust.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		return weapon.damageTypeT == 1;
	}
	,__class__: troshx_sos_manuevers_JointThrust
});
var troshx_sos_manuevers_MeleeShoot = function() {
	troshx_sos_core_Manuever.call(this,"meleeShoot","Melee Shoot");
	this._types(2)._requisite(1)._attackTypes(4)._ranged();
};
$hxClasses["troshx.sos.manuevers.MeleeShoot"] = troshx_sos_manuevers_MeleeShoot;
troshx_sos_manuevers_MeleeShoot.__name__ = ["troshx","sos","manuevers","MeleeShoot"];
troshx_sos_manuevers_MeleeShoot.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_MeleeShoot.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		return true;
	}
	,__class__: troshx_sos_manuevers_MeleeShoot
});
var troshx_sos_manuevers_MobileVoid = function() {
	troshx_sos_core_Manuever.call(this,"mobileVoid","Mobile Void");
	this._types(1)._tags(128)._tn(8)._bs(1);
};
$hxClasses["troshx.sos.manuevers.MobileVoid"] = troshx_sos_manuevers_MobileVoid;
troshx_sos_manuevers_MobileVoid.__name__ = ["troshx","sos","manuevers","MobileVoid"];
troshx_sos_manuevers_MobileVoid.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_MobileVoid.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_MobileVoid
});
var troshx_sos_manuevers_NetItems = function() { };
$hxClasses["troshx.sos.manuevers.NetItems"] = troshx_sos_manuevers_NetItems;
troshx_sos_manuevers_NetItems.__name__ = ["troshx","sos","manuevers","NetItems"];
var troshx_sos_manuevers_NetFeint = function() {
	troshx_sos_core_Manuever.call(this,"netFeint","Net Feint");
	this._types(2)._requisite(1,troshx_sos_manuevers_NetItems.STUFF)._costs(1,-2)._ranged();
};
$hxClasses["troshx.sos.manuevers.NetFeint"] = troshx_sos_manuevers_NetFeint;
troshx_sos_manuevers_NetFeint.__name__ = ["troshx","sos","manuevers","NetFeint"];
troshx_sos_manuevers_NetFeint.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_NetFeint.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon;
		var useRight = spec.usingLeftLimb;
		if(useRight) {
			weapon = node.charSheet.inventory.getMasterWeapon();
		} else {
			weapon = node.charSheet.inventory.getOffhandWeapon();
		}
		if(weapon != null) {
			if(!weapon.ranged) {
				return !weapon.isAmmo;
			} else {
				return false;
			}
		} else {
	...
	,__class__: troshx_sos_manuevers_NetFeint
});
var troshx_sos_manuevers_NetToss = function() {
	troshx_sos_core_Manuever.call(this,"netToss","Net Toss");
	this._types(2)._requisite(9)._ranged();
};
$hxClasses["troshx.sos.manuevers.NetToss"] = troshx_sos_manuevers_NetToss;
troshx_sos_manuevers_NetToss.__name__ = ["troshx","sos","manuevers","NetToss"];
troshx_sos_manuevers_NetToss.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_NetToss.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_NetToss
});
var troshx_sos_manuevers_PugilisiticAttack = function(id,name) {
	troshx_sos_core_Manuever.call(this,id,name);
	this._types(2)._requisite(4);
};
$hxClasses["troshx.sos.manuevers.PugilisiticAttack"] = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_PugilisiticAttack.__name__ = ["troshx","sos","manuevers","PugilisiticAttack"];
troshx_sos_manuevers_PugilisiticAttack.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_PugilisiticAttack.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_PugilisiticAttack
});
var troshx_sos_manuevers_HookPunch = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"hookPunch","Hook Punch");
	this._reach(1)._tn(6)._attackTypes(1)._damage(3,1)._superiorInit(function(m) {
		m._damage(3,2,2);
	});
};
$hxClasses["troshx.sos.manuevers.HookPunch"] = troshx_sos_manuevers_HookPunch;
troshx_sos_manuevers_HookPunch.__name__ = ["troshx","sos","manuevers","HookPunch"];
troshx_sos_manuevers_HookPunch.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_HookPunch.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_HookPunch
});
var troshx_sos_manuevers_StraightPunch = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"straightPunch","Straight Punch");
	this._reach(2)._tn(6)._attackTypes(2)._damage(3,2)._superiorInit(function(m) {
		m._damage(3,3,1);
	});
};
$hxClasses["troshx.sos.manuevers.StraightPunch"] = troshx_sos_manuevers_StraightPunch;
troshx_sos_manuevers_StraightPunch.__name__ = ["troshx","sos","manuevers","StraightPunch"];
troshx_sos_manuevers_StraightPunch.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_StraightPunch.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_StraightPunch
});
var troshx_sos_manuevers_Knee = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"knee","Knee");
	this._reach(1)._tn(7)._attackTypes(2)._damage(3,1)._superiorInit(function(m) {
		m._damage(3,3);
	});
};
$hxClasses["troshx.sos.manuevers.Knee"] = troshx_sos_manuevers_Knee;
troshx_sos_manuevers_Knee.__name__ = ["troshx","sos","manuevers","Knee"];
troshx_sos_manuevers_Knee.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_Knee.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_Knee
});
var troshx_sos_manuevers_HeadButt = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"headbutt","Headbutt");
	this._reach(1)._tn(6)._attackTypes(2)._targetZoneMode(-2560)._damage(3,1)._costs(1);
};
$hxClasses["troshx.sos.manuevers.HeadButt"] = troshx_sos_manuevers_HeadButt;
troshx_sos_manuevers_HeadButt.__name__ = ["troshx","sos","manuevers","HeadButt"];
troshx_sos_manuevers_HeadButt.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_HeadButt.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_HeadButt
});
var troshx_sos_manuevers_Elbow = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"elbow","Elbow");
	this._reach(1)._tn(7)._attackTypes(3)._damage(3,0)._superiorInit(function(m) {
		m._damage(3,2);
	});
};
$hxClasses["troshx.sos.manuevers.Elbow"] = troshx_sos_manuevers_Elbow;
troshx_sos_manuevers_Elbow.__name__ = ["troshx","sos","manuevers","Elbow"];
troshx_sos_manuevers_Elbow.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_Elbow.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_Elbow
});
var troshx_sos_manuevers_Kick = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"kick","Kick");
	this._reach(3)._tn(7)._attackTypes(3)._damage(3,0)._superiorInit(function(m) {
		m._damage(3,2);
	});
};
$hxClasses["troshx.sos.manuevers.Kick"] = troshx_sos_manuevers_Kick;
troshx_sos_manuevers_Kick.__name__ = ["troshx","sos","manuevers","Kick"];
troshx_sos_manuevers_Kick.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_Kick.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_Kick
});
...
troshx_sos_manuevers_Trip.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_Trip.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	__class__: troshx_sos_manuevers_Trip
});
var troshx_sos_manuevers_OneTwoPunch = function() {
	troshx_sos_manuevers_PugilisiticAttack.call(this,"oneTwoPunch","One-Two Punch");
	this._costs(2,-2);
};
$hxClasses["troshx.sos.manuevers.OneTwoPunch"] = troshx_sos_manuevers_OneTwoPunch;
troshx_sos_manuevers_OneTwoPunch.__name__ = ["troshx","sos","manuevers","OneTwoPunch"];
troshx_sos_manuevers_OneTwoPunch.__super__ = troshx_sos_manuevers_PugilisiticAttack;
troshx_sos_manuevers_OneTwoPunch.prototype = $extend(troshx_sos_manuevers_PugilisiticAttack.prototype,{
	getAvailability: function(bout,node,spec) {
		...
		if(masterHandItem != null && (masterHandItem.flags & 1) != 0) {
			return false;
		}
		return true;
	}
	,__class__: troshx_sos_manuevers_OneTwoPunch
});
var troshx_sos_manuevers_PushCut = function() {
	troshx_sos_core_Manuever.call(this,"pushCut","Push Cut");
	this._types(2)._requisite(1)._attackTypes(2)._costs(1)._superior();
};
$hxClasses["troshx.sos.manuevers.PushCut"] = troshx_sos_manuevers_PushCut;
troshx_sos_manuevers_PushCut.__name__ = ["troshx","sos","manuevers","PushCut"];
troshx_sos_manuevers_PushCut.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_PushCut.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		if(weapon.atnS > 0) {
			return weapon.damageTypeS == 0;
		} else {
			return false;
		}
	}
	,__class__: troshx_sos_manuevers_PushCut
});
var troshx_sos_manuevers_Riposte = function() {
	troshx_sos_core_Manuever.call(this,"riposte","Riposte");
	this._types(1)._requisite(1)._tags(576)._costs(2)._superior();
};
$hxClasses["troshx.sos.manuevers.Riposte"] = troshx_sos_manuevers_Riposte;
troshx_sos_manuevers_Riposte.__name__ = ["troshx","sos","manuevers","Riposte"];
troshx_sos_manuevers_Riposte.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_Riposte.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var weapon = spec.activeItem;
		return weapon.atnS > 0;
	}
	,__class__: troshx_sos_manuevers_Riposte
});
var troshx_sos_manuevers_ShieldBash = function() {
	troshx_sos_core_Manuever.call(this,"shieldBash","Shield Bash");
	this._types(2)._requisite(2)._reach(2)._attackTypes(2)._superior();
};
$hxClasses["troshx.sos.manuevers.ShieldBash"] = troshx_sos_manuevers_ShieldBash;
troshx_sos_manuevers_ShieldBash.__name__ = ["troshx","sos","manuevers","ShieldBash"];
troshx_sos_manuevers_ShieldBash.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_ShieldBash.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getSecondary: function() {
		if(this._bash != null) {
			return this._bash;
		} else {
			return this._bash = new troshx_sos_manuevers_ShieldBash();
		}
	}
	,__class__: troshx_sos_manuevers_ShieldBash
});
var troshx_sos_manuevers_ShieldBind = function() {
	troshx_sos_core_Manuever.call(this,"shieldBind","Shield Bind");
	this._types(1)._requisite(2)._tags(768)._costs(1)._superior();
};
$hxClasses["troshx.sos.manuevers.ShieldBind"] = troshx_sos_manuevers_ShieldBind;
troshx_sos_manuevers_ShieldBind.__name__ = ["troshx","sos","manuevers","ShieldBind"];
troshx_sos_manuevers_ShieldBind.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_ShieldBind.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getAvailability: function(bout,node,spec) {
		var obj = spec.activeEnemyItem;
		var weapon = js_Boot.__instanceof(obj,troshx_sos_core_Weapon) ? obj : null;
		if(weapon == null) {
			return false;
		}
		if(!weapon.ranged && !weapon.isAmmo) {
			return weapon.range >= 4;
		} else {
			return false;
		}
	}
	,__class__: troshx_sos_manuevers_ShieldBind
});
var troshx_sos_manuevers_ShieldFeint = function() {
	troshx_sos_core_Manuever.call(this,"shieldFeint","Shield Feint");
	this._types(2)._requisite(3)._costs(1,-2);
};
$hxClasses["troshx.sos.manuevers.ShieldFeint"] = troshx_sos_manuevers_ShieldFeint;
troshx_sos_manuevers_ShieldFeint.__name__ = ["troshx","sos","manuevers","ShieldFeint"];
troshx_sos_manuevers_ShieldFeint.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_ShieldFeint.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getTN: function(spec) {
		return troshx_sos_core_Manuever.prototype.getTN.call(this,spec);
	}
	,getSecondary: function() {
		if(this._bash != null) {
			return this._bash;
	...
	,getAvailability: function(bout,node,spec) {
		var shield = node.charSheet.inventory.findHeldShield();
		if(shield == null) {
			return false;
		}
		var weapon = node.charSheet.inventory.getMasterWeapon();
		if(weapon == null) {
			node.charSheet.inventory.getOffhandWeapon();
		}
		var fluidThrusts = (weapon.meleeFlags & 16) != 0;
		if(!spec.activeEnemyBody.isThrusting(spec.activeEnemyZone) && fluidThrusts) {
			return false;
...
var troshx_sos_manuevers_TotalBlock = function() {
	troshx_sos_core_Manuever.call(this,"totalBlock","Total Block");
	this._types(1)._requisite(2)._tags(256)._costs(0,-1);
};
$hxClasses["troshx.sos.manuevers.TotalBlock"] = troshx_sos_manuevers_TotalBlock;
troshx_sos_manuevers_TotalBlock.__name__ = ["troshx","sos","manuevers","TotalBlock"];
troshx_sos_manuevers_TotalBlock.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_TotalBlock.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	getCost: function(bout,node,inputs) {
		var _this = node.charSheet;
		var row = _this.get_encumbranceLvlRow();
		return Math.floor(row.cpMult * _this.clampIntZero(_this.getModifiedValue(13,(_this.schoolLevel >= 1 ? _this.schoolLevel : 0) + _this.clampIntZero(_this.getModifiedValue(8,(_this.get_AGI() + _this.get_WIT()) / 2 | 0))) * (_this.prone ? 0.5 : 1) + row.cp + troshx_sos_sheets_FatiqueTable.getTable()[_this.get_fatiqueLevel()].cp) * 0.5);
	}
	,getAvailability: function(bout,node,spec) {
		var firstMove = !bout.secondTempo;
		var firstRound = bout.roundCount <= 1;
		var orientDef = node.fight.orientation == 3;
		var noAttacksNow = node.fight.attackManuevers.length == 0;
		var noDefsNow = node.fight.defensiveManevers.length == 0;
		var _this = node.charSheet;
		var row = _this.get_encumbranceLvlRow();
		var atLeast1CP = Math.floor(row.cpMult * _this.clampIntZero(_this.getModifiedValue(13,(_this.schoolLevel >= 1 ? _this.schoolLevel : 0) + _this.clampIntZero(_this.getModifiedValue(8,(_this.get_AGI() + _this.get_WIT()) / 2 | 0))) * (_this.prone ? 0.5 : 1) + row.cp + troshx_sos_sheets_FatiqueTable.getTable()[_this.get_fatiqueLevel()].cp) * 0.5) >= 1;
		if(noAttacksNow && noDefsNow && firstMove && (!firstRound || orientDef)) {
			return atLeast1CP;
		} else {
			return false;
		}
	}
	,__class__: troshx_sos_manuevers_TotalBlock
});
var troshx_sos_manuevers_WeaponThrow = function() {
	troshx_sos_core_Manuever.call(this,"weaponThrow","Weapon Throw");
	this._types(2)._requisite(1)._attackTypes(8)._costs(1)._ranged();
};
$hxClasses["troshx.sos.manuevers.WeaponThrow"] = troshx_sos_manuevers_WeaponThrow;
troshx_sos_manuevers_WeaponThrow.__name__ = ["troshx","sos","manuevers","WeaponThrow"];
troshx_sos_manuevers_WeaponThrow.__super__ = troshx_sos_core_Manuever;
troshx_sos_manuevers_WeaponThrow.prototype = $extend(troshx_sos_core_Manuever.prototype,{
	__class__: troshx_sos_manuevers_WeaponThrow
});
...
var troshx_sos_vue_combat_CombatViewModel = function(boutModel) {
	this.lastToggleMasterSlot = true;
	...
	this.playerManueverSpec = new troshx_core_ManueverSpec();
	this.browseManueverIndex = 0;
	this.browseAttackManueverList3i = [false,false,true,true,false,false];
	this.browseAttackManueverList3 = [1,3,1,2,0,7];
	...
	this.basicShieldBash = this.manueverRepo["shieldBash"];
	this.basicMeleeShoot = this.manueverRepo["meleeShoot"];
	this.basicWeaponThrow = this.manueverRepo["weaponThrow"];
	...
	this.advShieldAttackArr = ["","","","shieldFeint"].map(function(p2) {
		return _e2[p2];
	});
	var _e3 = this.manueverRepo;
	this.advAntiWeapWithWeaponArr = ["disarm","beat","break",""].map(function(p3) {
		return _e3[p3];
	});
	var _e4 = this.manueverRepo;
	this.advAntiShieldWithWeaponArr = ["hew","beat","hook",""].map(function(p4) {
		return _e4[p4];
	});
	var _e5 = this.manueverRepo;
	this.advAntiHandUnarmedArr = ["disarmUnarmedAtk"].map(function(p5) {
		return _e5[p5];
	});
	var _e6 = this.manueverRepo;
	this.advAntiHandWithShieldArr = ["shieldBeat"].map(function(p6) {
		return _e6[p6];
	});
	...
	this.advPuglismHandlessArr = ["headbutt","elbow","knee","trip"].map(function(p7) {
		return _e7[p7];
	});
	var _e8 = this.manueverRepo;
	this.advPuglismThrustSwingArr = ["oneTwoPunch","elbow","kick","trip"].map(function(p8) {
		return _e8[p8];
	});
	...
	this.advRangedArr = ["","","blindToss","weaponThrow"].map(function(p9) {
		return _e9[p9];
	});
	...
	this.advVoidArr = ["hastyVoid","mobileVoid","","flee"].map(function(p10) {
		return _e10[p10];
	});
	var _e11 = this.manueverRepo;
	this.advParryArr = ["disarmUnarmedDef","armParry","riposte",""].map(function(p11) {
		return _e11[p11];
	});
	var _e12 = this.manueverRepo;
	this.advBlockArr = ["","shieldBind","","totalBlock"].map(function(p12) {
		return _e12[p12];
	});
...
troshx_sos_vue_combat_CombatViewModel.prototype = {
	...
	,cycleAttackManueverMode: function(leftLimb) {
		if(leftLimb == null) {
			leftLimb = false;
		}
		var tarArr = this.browseAttackManueverList3;
		if(leftLimb) {
			this.browseManueverIndex--;
			if(this.browseManueverIndex < 0) {
				this.browseManueverIndex = tarArr.length - 1;
			}
		} else {
			this.browseManueverIndex++;
			if(this.browseManueverIndex >= tarArr.length) {
				this.browseManueverIndex = 0;
			}
		}
		this.playerManueverSpec.usingLeftLimb = this.browseAttackManueverList3i[this.browseManueverIndex];
	}
	,getTarArr: function() {
		return this.browseAttackManueverList3;
	}
	,getBrowseAttackMode: function() {
		return this.browseAttackManueverList3[this.browseManueverIndex];
	}
	,getBrowseAttackModeLabel: function(alsoLeft) {
		var tarArr = this.browseAttackManueverList3;
		var val = tarArr[this.browseManueverIndex];
		switch(val) {
		case 1:case 3:
			return "Weapon";
		case 2:
	...
	,isBrowseHighlightLeft: function() {
		var tarArr = this.browseAttackManueverList3;
		if(!(this.playerManueverSpec.usingLeftLimb && tarArr[this.browseManueverIndex] == 1)) {
			return (tarArr[this.browseManueverIndex] & 2) != 0;
		} else {
			return true;
		}
	}
	,isBrowseHighlightRight: function() {
		var tarArr = this.browseAttackManueverList3;
		return (tarArr[this.browseManueverIndex] & 1) != 0;
	}
	,getBasicTargetingLabels: function(usingNet) {
		var activeItem = this.playerManueverSpec.activeItem;
		var gotPlayer = (this.currentPlayerIndex >= 0 ? this.boutModel.bout.combatants[this.currentPlayerIndex] : null) != null;
		var b = this.browseAttackManueverList3[this.browseManueverIndex];
		if(gotPlayer) {
			var weap = js_Boot.__instanceof(activeItem,troshx_sos_core_Weapon) ? activeItem : null;
			var shield = js_Boot.__instanceof(activeItem,troshx_sos_core_Shield) ? activeItem : null;
			if(weap != null) {
				if(!(weap.ranged && !weap.isAmmo)) {
					return troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_MELEE;
				} else {
					var a = (weap.profs & 16) != 0;
					if(!(weap.ranged && a && !weap.isAmmo)) {
						return troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_SHOOT;
					} else {
						if(usingNet == null) {
							usingNet = this.isUsingNet();
						}
						if(usingNet) {
							return troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_THROW_NET;
						} else {
							return troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_THROW;
						}
					}
				}
			} else if(shield != null) {
				return troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_SHIELD;
			} else {
				return troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_UNARMED;
			}
		} else {
			switch(b) {
			case 1:case 3:
	...
	,getBasicManuever: function(usingNet) {
		var spec = this.playerManueverSpec;
		var activeItem = spec.activeItem;
		var gotPlayer = (this.currentPlayerIndex >= 0 ? this.boutModel.bout.combatants[this.currentPlayerIndex] : null) != null;
		var b = this.browseAttackManueverList3[this.browseManueverIndex];
		var enemyBody = spec.activeEnemyBody;
		var enemyZone = spec.activeEnemyZone;
		var f = this.focusedIndex;
		if(this._swingMap.h.hasOwnProperty(f) || this._partMap.h.hasOwnProperty(f) || f == this._enemyHandLeftIdx || f == this._enemyHandRightIdx) {
			if(gotPlayer) {
				var weap = js_Boot.__instanceof(activeItem,troshx_sos_core_Weapon) ? activeItem : null;
				var shield = js_Boot.__instanceof(activeItem,troshx_sos_core_Shield) ? activeItem : null;
				if(weap != null) {
					if(!(weap.ranged && !weap.isAmmo)) {
						if(enemyBody.isThrusting(spec.activeEnemyZone)) {
							return this.basicThrust;
						} else {
							return this.basicSwing;
						}
					} else {
						var a = (weap.profs & 16) != 0;
						if(!(weap.ranged && a && !weap.isAmmo)) {
							return this.basicMeleeShoot;
						} else {
							if(usingNet == null) {
								usingNet = this.isUsingNet();
							}
							if(usingNet) {
								return this.basicNetToss;
							} else {
								return this.basicWeaponThrow;
							}
						}
					}
				} else if(shield != null) {
					return this.basicShieldBash;
				} else if(enemyBody.isThrusting(enemyZone)) {
					return this.basicStraightPunch;
				} else {
					return this.basicHookPunch;
				}
			} else {
				switch(b) {
				case 1:case 3:
					if(enemyBody.isThrusting(enemyZone)) {
						return this.basicThrust;
					} else {
						return this.basicSwing;
					}
			...
			if(f == this.btnVoidInteract.index) {
				return this.basicVoid;
			} else if(f == this.btnParryInteract.index) {
				return this.basicParry;
			} else if(f == this.btnBlockInteract.index) {
	...
	,updateSwingThrustAvails: function() {
		var activeItem = this.playerManueverSpec.activeItem;
		var gotPlayer = (this.currentPlayerIndex >= 0 ? this.boutModel.bout.combatants[this.currentPlayerIndex] : null) != null;
		if(gotPlayer) {
			var weap = js_Boot.__instanceof(activeItem,troshx_sos_core_Weapon) ? activeItem : null;
			...
			if(weap != null) {
				if(!(weap.ranged && !weap.isAmmo)) {
					this.thrustAvailabilityMask = this.defaultThrustAvailMask;
					this.swingAvailabilityMask = this.defaultSwingAvailMask;
				} else {
					var a = (weap.profs & 16) != 0;
					if(!(weap.ranged && a && !weap.isAmmo)) {
						this.thrustAvailabilityMask = this.defaultThrustAvailMask;
	...
	,onThrustAvailabilityChange: function() {
		...
		if(this._partMap.h.hasOwnProperty(i)) {
			if((this.thrustAvailabilityMask & 1 << this._partMap.h[i]) == 0) {
				this.focusedIndex = -1;
			}
		}
	}
	,onSwingAvailabilityChange: function() {
		...
		if(this._swingMap.h.hasOwnProperty(i)) {
			if((this.swingAvailabilityMask & 1 << this._swingMap.h[i]) == 0) {
				this.focusedIndex = -1;
			}
		}
	}
	,togglePlayerMasterhandSlot: function() {
		...
		var weapon = weaponAssign.holding1H && weaponAssign.weapon.variant != null ? weaponAssign.weapon.variant : weaponAssign.weapon;
		this.playerManueverSpec.usingLeftLimb = false;
		if(weaponAssign != null) {
			if(this.playerManueverSpec.activeItem != null && weapon == this.playerManueverSpec.activeItem) {
				var tmp;
				var tmp1;
				if(weaponAssign.held == 3) {
					if((weapon.flags & 1) != 0) {
						var a = weapon.meleeFlags;
						var b = weapon.flags;
						var c = !weapon.ranged && !weapon.isAmmo;
						if(c && (a & 128) != 0) {
							tmp1 = (b & 1) != 0;
						} else {
							tmp1 = false;
				...
				if(tmp) {
					...
					weaponAssign.held = 1;
					this.playerManueverSpec.activeItem = weapon;
				} else {
					this.playerManueverSpec.activeItem = null;
		...
		var item = pl.charSheet.inventory.findMasterHandItem();
		if(this.playerManueverSpec.activeItem != null && item == this.playerManueverSpec.activeItem) {
			this.playerManueverSpec.activeItem = null;
		} else {
			this.playerManueverSpec.activeItem = item;
		}
	}
	,togglePlayerOffhandSlot: function() {
		var pl = this.currentPlayerIndex >= 0 ? this.boutModel.bout.combatants[this.currentPlayerIndex] : null;
		this.lastToggleMasterSlot = false;
		var item = pl.charSheet.inventory.findOffHandItem();
		if(item == this.playerManueverSpec.activeItem) {
			this.playerManueverSpec.activeItem = null;
			this.playerManueverSpec.usingLeftLimb = false;
		} else {
			this.playerManueverSpec.activeItem = item;
			this.playerManueverSpec.usingLeftLimb = true;
		}
	}
	,activatePlayerItem: function(offhand) {
		if(offhand == null) {
			offhand = false;
		}
		var player = this.currentPlayerIndex >= 0 ? this.boutModel.bout.combatants[this.currentPlayerIndex] : null;
		var tmp = offhand ? player.charSheet.inventory.findOffHandItem() : player.charSheet.inventory.findMasterHandItem();
		this.playerManueverSpec.activeItem = tmp;
	}
	...
	,setFocusedIndex: function(val) {
		...
		this.playerManueverSpec.activeEnemyZone = this.getTargetZoneIndexFromFocIndex(val);
		this.showFocusedTag = val >= 0;
	}
	,setAdvFocusedIndex: function(val) {
		this.advFocusedIndex = val;
	}
	,resetAdvFocusedIndex: function() {
		this.advFocusedIndex = -1;
	}
	,getFocusManueverAtkType: function() {
		var i = this.focusedIndex;
		if(this._swingMap.h.hasOwnProperty(i)) {
			return 1;
		} else if(this._partMap.h.hasOwnProperty(i)) {
			return 2;
		} else if(this.focusedIndex == this._enemyHandLeftIdx) {
			return -2;
		} else if(this.focusedIndex == this._enemyHandRightIdx) {
			return -3;
		} else {
			return -1;
		}
	...
	,isUsingNet: function() {
		var obj = this.playerManueverSpec.activeItem;
		var weap = js_Boot.__instanceof(obj,troshx_sos_core_Weapon) ? obj : null;
		if(weap != null) {
			var a = (weap.profs & 16) != 0;
			var throwing = weap.ranged && a && !weap.isAmmo;
			if(throwing) {
				return troshx_sos_manuevers_NetItems.STUFF.indexOf(weap.name) >= 0;
			} else {
				return false;
			}
		}
	...
	,getTargetZoneIndexFromFocIndex: function(i) {
		if(this._swingMap.h.hasOwnProperty(i)) {
			return this.DOLL_SWING_Indices[this._swingMap.h[i]];
		} else if(this._partMap.h.hasOwnProperty(i)) {
			return this.DOLL_PART_Indices[this._partMap.h[i]];
		} else if(this.focusedIndex == this._enemyHandLeftIdx) {
			return -2;
		} else if(this.focusedIndex == this._enemyHandRightIdx) {
			return -3;
	...
	,getAdvancedManuevers: function(curPlayer,playerLeftItem,playerRightItem,curEnemy,enemyLeftItem,enemyRightItem,preferOneTwoPunch) {
		if(preferOneTwoPunch == null) {
			preferOneTwoPunch = false;
		}
		var manueverSpec = this.playerManueverSpec;
		...
		if(manueverSpec.activeEnemyBody == null) {
			return troshx_sos_vue_combat_CombatViewModel.EMPTY_ARR;
		}
		if(curEnemy != null) {
			if(this.playerManueverSpec.activeEnemyZone < 0 && this.playerManueverSpec.activeEnemyZone != -1) {
				if(this.playerManueverSpec.activeEnemyZone == -2) {
					if(enemyLeftItem == null) {
						enemyLeftItem = curEnemy.charSheet.inventory.findOffHandItem();
					}
					this.playerManueverSpec.activeEnemyItem = enemyLeftItem;
				} else if(this.playerManueverSpec.activeEnemyZone == -3) {
					if(enemyRightItem == null) {
						enemyRightItem = curEnemy.charSheet.inventory.findMasterHandItem();
					}
					this.playerManueverSpec.activeEnemyItem = enemyRightItem;
				} else {
					this.playerManueverSpec.activeEnemyItem = null;
				}
			} else {
				this.playerManueverSpec.activeEnemyItem = null;
			}
		...
		if(targetZone >= 0) {
			if(curPlayer == null) {
				var mode = this.browseAttackManueverList3[this.browseManueverIndex];
				if(mode == 1 || mode == 3) {
					if(manueverSpec.activeEnemyBody.isThrusting(targetZone)) {
						return this.advThrustArr;
					} else {
						return this.advSwingArr;
					}
				} else if(mode == 7 || mode == 0) {
					if(mode == 7) {
						return this.advPuglismThrustSwingArr;
					} else {
						return this.advPuglismHandlessArr;
					}
				} else if(mode == 2) {
					return this.advShieldAttackArr;
				}
			} else if(manueverSpec.activeItem == null) {
				if(preferOneTwoPunch) {
					return this.advPuglismThrustSwingArr;
				} else {
					return this.advPuglismHandlessArr;
				}
		...
		} else if(focusIndex == this._enemyHandLeftIdx || focusIndex == this._enemyHandRightIdx) {
			if(curPlayer == null) {
				var mode1 = this.browseAttackManueverList3[this.browseManueverIndex];
				if(mode1 == 1 || mode1 == 3) {
					if(js_Boot.__instanceof(manueverSpec.activeEnemyItem,troshx_sos_core_Shield)) {
						return this.advAntiShieldWithWeaponArr;
					} else {
						return this.advAntiWeapWithWeaponArr;
					}
				} else if(mode1 == 7 || mode1 == 0) {
					if(js_Boot.__instanceof(manueverSpec.activeEnemyItem,troshx_sos_core_Shield)) {
						return troshx_sos_vue_combat_CombatViewModel.EMPTY_ARR;
					} else {
						return this.advAntiHandUnarmedArr;
					}
				} else if(mode1 == 2) {
					if(js_Boot.__instanceof(manueverSpec.activeEnemyItem,troshx_sos_core_Shield)) {
						return troshx_sos_vue_combat_CombatViewModel.EMPTY_ARR;
					} else {
						return this.advAntiHandWithShieldArr;
					}
				}
			} else if(js_Boot.__instanceof(manueverSpec.activeItem,troshx_sos_core_Weapon)) {
				if(js_Boot.__instanceof(manueverSpec.activeEnemyItem,troshx_sos_core_Shield)) {
					return this.advAntiShieldWithWeaponArr;
				} else {
					return this.advAntiWeapWithWeaponArr;
				}
			} else if(js_Boot.__instanceof(manueverSpec.activeItem,troshx_sos_core_Shield)) {
				return this.advAntiHandWithShieldArr;
			}
			return this.advAntiHandUnarmedArr;
		} else if(focusIndex == this.btnBlockInteract.index || focusIndex == this.btnParryInteract.index || focusIndex == this.btnVoidInteract.index) {
			if(focusIndex == this.btnBlockInteract.index) {
				return this.advBlockArr;
			} else if(focusIndex == this.btnParryInteract.index) {
				return this.advParryArr;
			} else if(focusIndex == this.btnVoidInteract.index) {
				return this.advVoidArr;
			} else {
				return troshx_sos_vue_combat_CombatViewModel.EMPTY_ARR;
			}
		}
	...
	,getFocusedLabel: function(enemyLeftItem,enemyRightItem,lblDescs) {
		var i = this.focusedIndex;
		if(i < 0) {
			return "";
		}
		if(this._swingMap.h.hasOwnProperty(i)) {
			if(lblDescs != null) {
				return this._body.getDescLabelTargetZone(this.DOLL_SWING_Indices[this._swingMap.h[i]],lblDescs[0],lblDescs[1],lblDescs.length >= 3 ? lblDescs.slice(2) : null);
			} else {
				return this.getDollSwingDescAt(this._swingMap.h[i]);
			}
		} else if(this._partMap.h.hasOwnProperty(i)) {
			if(lblDescs != null) {
				return this._body.getDescLabelTargetZone(this.DOLL_PART_Indices[this._partMap.h[i]],lblDescs[0],lblDescs[1]);
			} else {
		...
		} else if(this.DOLL_GENERAL_FOCUS_DESC.h.hasOwnProperty(i)) {
			return this.DOLL_GENERAL_FOCUS_DESC.h[i];
		} else {
			return "todo/missing";
		}
	}
	...
	,getGeneralFocusDescAt: function(index) {
		return this.DOLL_GENERAL_FOCUS_DESC.h[index];
	}
	,isDefensiveFocusIndex: function(index) {
		if(!(index == this.btnBlockInteract.index || index == this.btnParryInteract.index)) {
			return index == this.btnVoidInteract.index;
		} else {
			return true;
		}
	}
	...
	,set_currentPlayerIndex: function(value) {
		this.playerManueverSpec.resetPlayer();
		this.lastToggleMasterSlot = true;
		return this.currentPlayerIndex = value;
	}
	,set_focusOpponentIndex: function(value) {
		this.playerManueverSpec.setNewEnemy(this.boutModel.bout.combatants[value].charSheet.body);
		return this.focusOpponentIndex = value;
	}
	...
	,getCurrentActiveOpponents: function() {
		var pl = this.currentPlayerIndex >= 0 ? this.boutModel.bout.combatants[this.currentPlayerIndex] : null;
		if(this.boutModel.bout == null) {
			return null;
		}
	...
	,handleDisabledMask: function(val,slugs) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			map.h[this._dollImageMapData.idIndices[slugs[i]]].disabled = (val & 1 << i) == 0;
		}
	}
	,setDisabledAll: function(slugs,disabled) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			map.h[this._dollImageMapData.idIndices[slugs[i]]].disabled = disabled;
		}
	}
	,onAdvAvailabilityChange: function(mask) {
		this.advInteract1.disabled = (mask & 17) != 0;
		this.advInteract2.disabled = (mask & 34) != 0;
		this.advInteract3.disabled = (mask & 68) != 0;
		this.advInteract4.disabled = (mask & 136) != 0;
	}
	,partIndexAvailable: function(index) {
		return (this.thrustAvailabilityMask & index << 1) != 0;
	}
	,swingArcAvailableBetween: function(slugs) {
		return (this.swingAvailabilityMask & (this._dollImageMapData.idIndices[slugs[0]] | this._dollImageMapData.idIndices[slugs[1]])) != 0;
	}
	...
	,setupDollInteraction: function(fullInteractList,imageMapData) {
		...
		(this.advInteract1 = this._interactionMaps[0].h[this._dollImageMapData.idIndices["advManuever1"]]).disabled = false;
		(this.advInteract2 = this._interactionMaps[0].h[this._dollImageMapData.idIndices["advManuever2"]]).disabled = false;
		(this.advInteract3 = this._interactionMaps[0].h[this._dollImageMapData.idIndices["advManuever3"]]).disabled = false;
		(this.advInteract4 = this._interactionMaps[0].h[this._dollImageMapData.idIndices["advManuever4"]]).disabled = false;
		...
		var this1 = { };
		var mapTempSwings = this1;
		var this2 = { };
		var mapTempThrusts = this2;
		var key;
		var _g11 = 0;
		...
		while(_g12 < _g3) {
			...
			mapTempThrusts[key] = i2;
			if(key == "LOWER_ARM") {
				mapTempThrusts["FOREARM"] = i2;
			} else if(key == "UPPER_LEG") {
				mapTempThrusts["THIGH"] = i2;
			} else if(key == "HEAD") {
				mapTempThrusts["FACE"] = i2;
			} else if(key == "LOWER_LEG") {
				mapTempThrusts["SHIN"] = i2;
			}
		}
		...
		while(_g13 < _g4) {
			...
			if(mapTempThrusts[key] != null) {
				this.DOLL_PART_Indices[i3] = mapTempThrusts[key];
				maskAccum |= 1 << i3;
			} else {
		...
		while(_g14 < _g5) {
			...
			} else if(mapTempSwings[key] != null) {
				this.DOLL_SWING_Indices[i4] = mapTempSwings[key];
			} else {
				this.DOLL_SWING_Indices[i4] = -1;
		...
		(this.btnBlockInteract = this._interactionMaps[0].h[this._dollImageMapData.idIndices["btnBlock"]]).disabled = true;
		this.btnVoidInteract = this._interactionMaps[0].h[this._dollImageMapData.idIndices["btnVoid"]];
		(this.btnParryInteract = this._interactionMaps[0].h[this._dollImageMapData.idIndices["btnParry"]]).disabled = true;
		this.DOLL_GENERAL_FOCUS_DESC.h[this.btnBlockInteract.index] = this.basicBlock.name;
		this.DOLL_GENERAL_FOCUS_DESC.h[this.btnVoidInteract.index] = this.basicVoid.name;
		this.DOLL_GENERAL_FOCUS_DESC.h[this.btnParryInteract.index] = this.basicParry.name;
	}
...
troshx_sos_vue_combat_HammerJSCombat.prototype = {
	...
	,defaultCallback: function(index,event) {
		...
		if(this.viewModel.actingState == 1) {
			if(event == 2) {
				...
				this.viewModel.advFocusedIndex = -1;
			} else if(event == 16384) {
				this.viewModel.setActingState(0);
				this.defaultAct = this.DEFAULT_ACT_HOVER;
				this.requiredActs = 0;
		...
		if(tag == "swing" || tag == "part" || name == "enemyHandLeft" || name == "enemyHandRight") {
			if((event & 266240) != 0 || index != this.viewModel.focusedIndex) {
				if(!this.viewModel.observeOpponent) {
					var _this = this.viewModel;
					_this.focusedIndex = index;
					_this.playerManueverSpec.activeEnemyZone = _this.getTargetZoneIndexFromFocIndex(index);
				...
				if(name == "enemyHandLeft" || name == "enemyHandRight") {
					if(!this.viewModel.advInteract1.disabled && this.viewModel.currentPlayerIndex >= 0) {
						this.viewModel.advFocusedIndex = 0;
					} else {
						return;
					}
				} else {
				...
				if(this.viewModel.currentPlayerIndex >= 0) {
					this.startDragCP(name,tag,this.viewModel.isFocusedEnemyLeftSide(),this.viewModel.isFocusedEnemyLower());
				}
			}
			return;
		}
		if(name == "incomingManuevers") {
			...
			if(name == "advManuever1" || name == "advManuever2" || name == "advManuever3" || name == "advManuever4") {
				if(event == 1) {
					if(this.viewModel.currentPlayerIndex >= 0) {
						this.viewModel.advFocusedIndex = Std.parseInt(HxOverrides.substr(name,name.length - 1,null)) - 1;
						this.startDragCP(name,tag,true,name == "advManuever3" || name == "advManuever4");
					}
				}
			} else if(name == "btnVoid" || name == "btnParry" || name == "btnBlock") {
				if((event & 1) != 0) {
					if(index != this.viewModel.focusedIndex) {
						var _this1 = this.viewModel;
						_this1.focusedIndex = index;
						_this1.playerManueverSpec.activeEnemyZone = _this1.getTargetZoneIndexFromFocIndex(index);
						_this1.showFocusedTag = index >= 0;
					} else if(this.viewModel.currentPlayerIndex >= 0) {
						this.viewModel.advFocusedIndex = -1;
						this.startDragCP(name,tag,name == "btnParry",true);
					}
				} else if((event & 49152) != 0) {
					this.viewModel.setActingState(0);
					this.defaultAct = this.DEFAULT_ACT_HOVER;
				...
				if((event & 4) != 0) {
					var _this2 = this.viewModel;
					if((_this2.currentPlayerIndex >= 0 ? _this2.boutModel.bout.combatants[_this2.currentPlayerIndex] : null) != null) {
						if(name != "handLeftAlt") {
							this.viewModel.togglePlayerMasterhandSlot();
						} else {
							this.viewModel.togglePlayerOffhandSlot();
						}
					} else {
						this.viewModel.cycleAttackManueverMode(name == "handLeftAlt");
					}
				}
			} else {
	...
	,startDragCP: function(name,tag,flip,flipY) {
		this.viewModel.draggedCP = 0;
		...
		this.viewModel.trayPosFlip = flip;
		this.viewModel.trayPosFlipY = flipY;
		...
		if(e.isFirst && e.type == "hammer.input") {
			...
			if(e.type == "hammer.input") {
				if(e.eventType == Hammer.INPUT_MOVE) {
					if((e.deltaX != 0 || e.deltaY != 0) && (act.mask & 274434) != 0) {
						if((mask & 2) != 0) {
							this.callback(act.index,2);
						}
						if((act.mask & 405504) != 0) {
							...
							if(act2 != null) {
								if((act2.mask & 8192) != 0 && act2.index == act.index) {
								...
								} else if((act2.mask & 266240) != 0 && act2.index != act.index) {
									...
									if((act.mask & 262144) != 0) {
										this.activeTouches.h[id] = act2;
										this.callback(act2.index,262144);
									}
								}
...
troshx_sos_vue_combat_UIInteraction.setupDollViewInteracts = function(layoutItems,names,tags) {
	...
	while(_g1 < _g) {
		...
		if(tag == "part" || tag == "swing" || name == "enemyHandLeft" || name == "enemyHandRight") {
			arr.push(new troshx_sos_vue_combat_UInteract(i,311297));
			continue;
		}
		switch(name) {
		case "advManuever1":case "advManuever2":case "advManuever3":case "advManuever4":
			arr.push(new troshx_sos_vue_combat_UInteract(i,49155));
			break;
		case "btnBlock":case "btnParry":case "btnVoid":
			item.hitPadding = 5;
			arr.push(new troshx_sos_vue_combat_UInteract(i,49155));
...
troshx_sos_vue_combat_components_DollView.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	...
	,quickStartDebug: function() {
		...
		var valData;
		var node;
		valData = troshx_sos_pregens_FightCharacters.get()[7];
		var valData1 = valData.label;
		var node1 = this.deserializeSheet(valData.savedData);
		var _this = this.viewModel;
		node = new troshx_components_FightNode(valData1,node1,0);
		var theIndex = boutModel.bout.combatants.length;
		boutModel.bout.pushNewFightNode(node);
		var _this1 = this.viewModel;
		_this1.playerManueverSpec.resetPlayer();
		_this1.lastToggleMasterSlot = true;
		_this1.currentPlayerIndex = theIndex;
		var _this2 = node.charSheet;
		var row = _this2.get_encumbranceLvlRow();
		var tmp = row.cpMult * _this2.clampIntZero(_this2.getModifiedValue(13,(_this2.schoolLevel >= 1 ? _this2.schoolLevel : 0) + _this2.clampIntZero(_this2.getModifiedValue(8,(_this2.get_AGI() + _this2.get_WIT()) / 2 | 0))) * (_this2.prone ? 0.5 : 1) + row.cp + troshx_sos_sheets_FatiqueTable.getTable()[_this2.get_fatiqueLevel()].cp);
		node.fight.cp = tmp;
		...
		this.viewModel.activatePlayerItem();
		valData = troshx_sos_pregens_FightCharacters.get()[7];
		var valData2 = valData.label;
		var node2 = this.deserializeSheet(valData.savedData);
		var _this3 = this.viewModel;
		node = new troshx_components_FightNode(valData2,node2,1);
		boutModel.bout.pushNewFightNode(node);
		var _this4 = this.viewModel;
		_this4.playerManueverSpec.setNewEnemy(_this4.boutModel.bout.combatants[theIndex].charSheet.body);
		_this4.focusOpponentIndex = theIndex;
		node.charSheet.inventory.refreshHalfArmorLabels();
		node.charSheet.inventory.cleanupShieldLabels();
		node.charSheet.inventory.weildMeleeEquip(node.charSheet.profsMelee);
		this.showPregens = 0;
	...
	,confirmPregens: function(val) {
		...
		if(showPregens == 1) {
			...
			var theIndex = boutModel.bout.combatants.length;
			boutModel.bout.pushNewFightNode(node);
			viewModel.playerManueverSpec.resetPlayer();
			viewModel.lastToggleMasterSlot = true;
			...
			var valData1 = val;
			var theIndex1 = boutModel.bout.combatants.length;
			node = new troshx_components_FightNode(valData1.label,this.deserializeSheet(valData1.savedData),1);
			boutModel.bout.pushNewFightNode(node);
			viewModel.playerManueverSpec.setNewEnemy(viewModel.boutModel.bout.combatants[theIndex1].charSheet.body);
			viewModel.focusOpponentIndex = theIndex1;
		}
		if(node != null) {
			node.charSheet.inventory.refreshHalfArmorLabels();
			node.charSheet.inventory.cleanupShieldLabels();
	...
	,get_rightItem: function() {
		...
		var weaponAssign = pl.charSheet.inventory.getMasterWeaponAssign();
		if(weaponAssign != null) {
			if(weaponAssign.holding1H && weaponAssign.weapon.variant != null) {
				return weaponAssign.weapon.variant;
			} else {
				return weaponAssign.weapon;
			}
		}
	...
	,get_rightItemAssign: function() {
		var _this = this.viewModel;
		var pl = _this.currentPlayerIndex >= 0 ? _this.boutModel.bout.combatants[_this.currentPlayerIndex] : null;
		return pl.charSheet.inventory.findMasterHandAssign();
	}
	,get_leftItemAssign: function() {
		var _this = this.viewModel;
		var pl = _this.currentPlayerIndex >= 0 ? _this.boutModel.bout.combatants[_this.currentPlayerIndex] : null;
		return pl.charSheet.inventory.findOffHandAssign();
	}
	,get_rightItemHighlighted: function() {
		var usingLeftLimb = this.viewModel.playerManueverSpec.usingLeftLimb;
		var activeItem = this.viewModel.playerManueverSpec.activeItem;
		var rightItem = this.rightItem;
		if(usingLeftLimb) {
			return false;
		} else {
			return rightItem == activeItem;
		}
	}
	,get_leftItemHighlighted: function() {
		var usingLeftLimb = this.viewModel.playerManueverSpec.usingLeftLimb;
		var activeItem = this.viewModel.playerManueverSpec.activeItem;
		var leftItem = this.leftItem;
		var rightItemAssign = this.rightItemAssign;
		if(usingLeftLimb) {
			return leftItem == activeItem;
		} else if(rightItemAssign != null) {
			return rightItemAssign.held == 3;
		} else {
			return false;
		}
	}
	...
	,get_styleObservePartProps: function() {
		var d = this.mapData;
		return { fillColor : "transparent", strokeColor : "rgba(0,0,0,1)", strokeWidth : 4 * (d.scaleX < d.scaleY ? d.scaleX : d.scaleY)};
	}
	,get_focusManueverAtkType: function() {
		return this.viewModel.getFocusManueverAtkType();
	}
	,get_advStyleProps: function() {
		if(this.focusManueverAtkType != 1) {
			return this.stylePartProps;
		} else {
			return this.styleSwingProps;
		}
	}
	,get_stylePartProps: function() {
		var d = this.mapData;
		return { fillColor : "rgba(0,200,255,0.3)", strokeColor : "rgba(0,200,255,0.3)", strokeWidth : 4 * (d.scaleX < d.scaleY ? d.scaleX : d.scaleY)};
	}
	,getArmorPartPropsOf: function(name,index) {
		...
		var d = this.mapData;
		var p = { index : d.idIndices[name], mapData : d};
	...
	,get_armorPartProps: function() {
		var arr = [];
		var slugs = this.viewModel.DOLL_PART_Slugs;
		var _g1 = 0;
		var _g = slugs.length;
		while(_g1 < _g) {
			var i = _g1++;
			arr[i] = this.getArmorPartPropsOf(slugs[i],i);
		}
		return arr;
	}
	,get_isUsingNet: function() {
		return this.viewModel.isUsingNet();
	}
	,get_styleSwingProps: function() {
		var d = this.mapData;
		return { fillColor : "rgba(0,255,255,0.3)", strokeColor : "rgba(0,255,255,0.3)", strokeWidth : 8 * (d.scaleX < d.scaleY ? d.scaleX : d.scaleY)};
	}
	,get_styleDefBtnProps: function() {
		var d = this.mapData;
		return { fillColor : "rgba(0,255,255,0.3)", strokeColor : "rgba(0,255,255,0.3)", strokeWidth : 4};
	}
	,get_stylePlHandBtnProps: function() {
		return { fillColor : "rgba(161,244,210,1)", strokeWidth : 0};
	}
	,get_basicTargetingLabels: function() {
		return this.viewModel.getBasicTargetingLabels();
	}
	...
	,get_currentOpponent: function() {
		var allOpponents = this.opponents;
		var currentPlayer = this.player;
		if(currentPlayer != null) {
			if(currentPlayer.targetLink != null) {
				if(currentPlayer.targetLink.a != currentPlayer) {
					return currentPlayer.targetLink.a;
				} else {
					return currentPlayer.targetLink.b;
				}
			} else {
				return null;
			}
		} else if(allOpponents != null) {
			return allOpponents[this.clampedOpponentIndex];
		} else {
			return null;
		}
	}
	,get_gotCurrentOpponent: function() {
		return this.currentOpponent != null;
	}
	,Components: function() {
		return { zone : new troshx_sos_vue_combat_components_ZoneItemView(), pregens : new troshx_sos_vue_pregen_PregenSelectView()};
	}
	,get_canOneTwoPunch: function() {
		var a = this.boutModel.bout;
		var b = this.player;
		if(b != null) {
			return this.viewModel.mOneTwoPunch.getAvailability(a,b,this.viewModel.playerManueverSpec);
		} else {
			return false;
		}
	}
	,get_advManuevers: function() {
		var gotPlayer = this.player != null;
		var gotEnemy = this.currentOpponent != null;
	...
	,get_advTNArr: function() {
		var gotPlayer = this.player != null;
		if(gotPlayer) {
			return this.advTNs;
		} else {
			return this.advSimpTNs;
		}
	}
	,get_advCostArr: function() {
		var gotPlayer = this.player != null;
		if(gotPlayer) {
			return this.advCosts;
		} else {
			return this.advSimpCosts;
		}
	}
	,get_advCosLblArr: function() {
		var gotPlayer = this.player != null;
		if(gotPlayer) {
			return this.advCostStrs;
		} else {
			return this.advSimpCostStr;
		}
	}
	,get_browseAttackModeLabel: function() {
		return this.viewModel.getBrowseAttackModeLabel(false);
	}
	,get_browseAttackModeLabel2: function() {
		return this.viewModel.getBrowseAttackModeLabel(true);
	}
	,get_isBrowseHighlightLeft: function() {
		return this.viewModel.isBrowseHighlightLeft();
	}
	,get_isBrowseHighlightRight: function() {
	...
	,get_basicManuever: function() {
		return this.viewModel.getBasicManuever();
	}
	,get_basicManueverTN: function() {
		var m = this.basicManuever;
		var gotPlayer = this.player != null;
		if(m != null) {
			if(gotPlayer) {
				return m.getTN(this.viewModel.playerManueverSpec);
			} else {
				return m.tn;
			}
		} else {
	...
	,get_advCostStrs: function() {
		...
		while(_g1 < _g) {
			...
			if(sec != null && !sec.getAvailability(bout,player,spec)) {
				sec = null;
			}
			val[i] = (m.costFuncInputs == -2 ? "\\" : "") + arrCosts[i] + (sec != null ? "," + sec.getCost(bout,player,null) : "");
		}
		return val;
	}
	,get_advSimpCosts: function() {
		var arr = this.advManuevers;
		var val = [];
		var _g1 = 0;
		var _g = arr.length;
	...
	,get_advSimpCostStr: function() {
		...
		while(_g1 < _g) {
			var i = _g1++;
			var m = arr[i];
			val[i] = (m.costFuncInputs == -2 ? "\\" : "") + m.cost;
		}
		return val;
	}
	,advHiddenAt: function(i) {
		return (this.advNotAvailMask & 1 << i) != 0;
	}
	,advDisabledAt: function(i) {
		return (this.advNotAvailMask & 1 << i + 4) != 0;
	}
	,get_advNotAvailMask: function() {
		var arr = this.advManuevers;
	...
	,onNotAvailAdvMaskChange: function(newValue,oldValue) {
	...
	,Mounted: function() {
		...
		if(HxOverrides.substr(window.location.href,0,7) == "file://" || window.location.host == "localhost") {
			window.setTimeout(function() {
			},100);
		}
	}
	...
	,get_dollShieldCoverageBools: function() {
		var slugs = this.viewModel.DOLL_PART_Slugs;
		var arr = [];
		var _g1 = 0;
		var _g = slugs.length;
		while(_g1 < _g) {
			var i = _g1++;
			arr[i] = this.shieldCoveredAtDollHitLocation(i);
		}
		return arr;
	}
	...
	,bitMaskHitAtIndex: function(i,mask) {
		return (i & mask) != 0;
	}
	...
	,get_isDefBtnBlockVisible: function() {
	...
	,get_isDefBtnBlockAllowed: function() {
		return this.viewModel.isDefBtnBlockAllowed();
	}
	,changed_isDefBtnBlockVisible: function(val,oldVal) {
		this.viewModel.btnBlockInteract.disabled = !val;
	}
	,get_isDefBtnParryVisible: function() {
	...
	,changed_isDefBtnParryVisible: function(val,oldVal) {
		this.viewModel.btnParryInteract.disabled = !val;
	}
	,get_isDefBtnParryAllowed: function() {
		return this.viewModel.isDefBtnParryAllowed();
	}
	,get_thrustAvailabilityMask: function() {
		return this.viewModel.thrustAvailabilityMask;
	}
	,get_swingAvailabilityMask: function() {
		return this.viewModel.swingAvailabilityMask;
	}
	,changed_swingAvailabilityMask: function(newVal,oldVal) {
		this.viewModel.onSwingAvailabilityChange();
	}
	,changed_thrustAvailabilityMask: function(newVal,oldVal) {
		this.viewModel.onThrustAvailabilityChange();
	}
	,get_activePlayerItem: function() {
		return this.viewModel.playerManueverSpec.activeItem;
	}
	,changed_activePlayerItem: function(newVal,oldVal) {
		this.viewModel.updateSwingThrustAvails();
	}
	,get_bgDollStyle: function() {
		if(this.swingAvailabilityMask != 0) {
			return troshx_sos_vue_combat_components_DollView.DOLL_BG_SWINGS;
		} else {
			return troshx_sos_vue_combat_components_DollView.DOLL_BG_NO_SWINGS;
		}
	}
	...
	,_Init: function() {
		...
		this.computed = { pregenHeader : clsP.get_pregenHeader, boutModel : clsP.get_boutModel, remainingDisplayCP : clsP.get_remainingDisplayCP, fatique : clsP.get_fatique, CP : clsP.get_CP, remCP : clsP.get_remCP, BL : clsP.get_BL, pain : clsP.get_pain, rightItem : clsP.get_rightItem, leftItem : clsP.get_leftItem, rightItemAssign : clsP.get_rightItemAssign, leftItemAssign : clsP.get_leftItemAssign, rightItemHighlighted : clsP.get_rightItemHighlighted, leftItemHighlighted : clsP.get_leftItemHighlighted, leftTypeTag : clsP.get_leftTypeTag, rightTypeTag : clsP.get_rightTypeTag, enemyRightItem : clsP.get_enemyRightItem, enemyLeftItem : clsP.get_enemyLeftItem, enemyLeftTypeTag : clsP.get_enemyLeftTypeTag, enemyRightTypeTag : clsP.get_enemyRightTypeTag, styleObservePartProps : clsP.get_styleObservePartProps, focusManueverAtkType : clsP.get_focusManueverAtkType, advStyleProps : clsP.get_advStyleProps, stylePartProps : clsP.get_stylePartProps, armorPartProps : clsP.get_armorPartProps, isUsingNet : clsP.get_isUsingNet, styleSwingProps : clsP.get_styleSwingProps, styleDefBtnProps : clsP.get_styleDefBtnProps, stylePlHandBtnProps : clsP.get_stylePlHandBtnProps, basicTargetingLabels : clsP.get_basicTargetingLabels, focusedTextLbl : clsP.get_focusedTextLbl, observePrompt : clsP.get_observePrompt, partObserveLbl : clsP.get_partObserveLbl, player : clsP.get_player, opponents : clsP.get_opponents, dollScale : clsP.get_dollScale, gotPregens : clsP.get_gotPregens, thrustPointStyle : clsP.get_thrustPointStyle, clampedOpponentIndex : clsP.get_clampedOpponentIndex, currentOpponent : clsP.get_currentOpponent, gotCurrentOpponent : clsP.get_gotCurrentOpponent, canOneTwoPunch : clsP.get_canOneTwoPunch, advManuevers : clsP.get_advManuevers, advTNArr : clsP.get_advTNArr, advCostArr : clsP.get_advCostArr, advCosLblArr : clsP.get_advCosLblArr, browseAttackModeLabel : clsP.get_browseAttackModeLabel, browseAttackModeLabel2 : clsP.get_browseAttackModeLabel2, isBrowseHighlightLeft : clsP.get_isBrowseHighlightLeft, isBrowseHighlightRight : clsP.get_isBrowseHighlightRight, advTNs : clsP.get_advTNs, basicManuever : clsP.get_basicManuever, basicManueverTN : clsP.get_basicManueverTN, advSimpTNs : clsP.get_advSimpTNs, advCosts : clsP.get_advCosts, advCostStrs : clsP.get_advCostStrs, advSimpCosts : clsP.get_advSimpCosts, advSimpCostStr : clsP.get_advSimpCostStr, advNotAvailMask : clsP.get_advNotAvailMask, currentDollSheet : clsP.get_currentDollSheet, armorColorScale : clsP.get_armorColorScale, carriedDollShieldAssign : clsP.get_carriedDollShieldAssign, carriedDollShield : clsP.get_carriedDollShield, shieldIconStyle : clsP.get_shieldIconStyle, shieldLowProfiles : clsP.get_shieldLowProfiles, shieldHighProfiles : clsP.get_shieldHighProfiles, dollShieldCoverage : clsP.get_dollShieldCoverage, dollShieldCoverageBools : clsP.get_dollShieldCoverageBools, coverageHitLocations : clsP.get_coverageHitLocations, hitLocationZeroAVValues : clsP.get_hitLocationZeroAVValues, hitLocationZeroAVValues2 : clsP.get_hitLocationZeroAVValues2, armorDollColors : clsP.get_armorDollColors, hitLocationArmorValues : clsP.get_hitLocationArmorValues, hitLocationArmorValues2 : clsP.get_hitLocationArmorValues2, cpTrayCSSTransform : clsP.get_cpTrayCSSTransform, cpMeterAmount : clsP.get_cpMeterAmount, dragCPHtml : clsP.get_dragCPHtml, cpTrayStyle : clsP.get_cpTrayStyle, actingState : clsP.get_actingState, isDefBtnBlockVisible : clsP.get_isDefBtnBlockVisible, isDefBtnBlockAllowed : clsP.get_isDefBtnBlockAllowed, isDefBtnParryVisible : clsP.get_isDefBtnParryVisible, isDefBtnParryAllowed : clsP.get_isDefBtnParryAllowed, thrustAvailabilityMask : clsP.get_thrustAvailabilityMask, swingAvailabilityMask : clsP.get_swingAvailabilityMask, activePlayerItem : clsP.get_activePlayerItem, bgDollStyle : clsP.get_bgDollStyle, isDraggingCP : clsP.get_isDraggingCP};
		this.methods = { get_pregenHeader : clsP.get_pregenHeader, closePregens : clsP.closePregens, quickStartDebug : clsP.quickStartDebug, confirmPregens : clsP.confirmPregens, deserializeSheet : clsP.deserializeSheet, showSelfPregens : clsP.showSelfPregens, showOpponentPregens : clsP.showOpponentPregens, get_boutModel : clsP.get_boutModel, get_remainingDisplayCP : clsP.get_remainingDisplayCP, get_fatique : clsP.get_fatique, get_CP : clsP.get_CP, get_remCP : clsP.get_remCP, get_BL : clsP.get_BL, get_pain : clsP.get_pain, get_rightItem : clsP.get_rightItem, get_leftItem : clsP.get_leftItem, get_rightItemAssign : clsP.get_rightItemAssign, get_leftItemAssign : clsP.get_leftItemAssign, get_rightItemHighlighted : clsP.get_rightItemHighlighted, get_leftItemHighlighted : clsP.get_leftItemHighlighted, getTypeTagForItem : clsP.getTypeTagForItem, get_leftTypeTag : clsP.get_leftTypeTag, get_rightTypeTag : clsP.get_rightTypeTag, get_enemyRightItem : clsP.get_enemyRightItem, get_enemyLeftItem : clsP.get_enemyLeftItem, get_enemyLeftTypeTag : clsP.get_enemyLeftTypeTag, get_enemyRightTypeTag : clsP.get_enemyRightTypeTag, get_styleObservePartProps : clsP.get_styleObservePartProps, get_focusManueverAtkType : clsP.get_focusManueverAtkType, get_advStyleProps : clsP.get_advStyleProps, get_stylePartProps : clsP.get_stylePartProps, getArmorPartPropsOf : clsP.getArmorPartPropsOf, get_armorPartProps : clsP.get_armorPartProps, get_isUsingNet : clsP.get_isUsingNet, get_styleSwingProps : clsP.get_styleSwingProps, get_styleDefBtnProps : clsP.get_styleDefBtnProps, get_stylePlHandBtnProps : clsP.get_stylePlHandBtnProps, get_basicTargetingLabels : clsP.get_basicTargetingLabels, get_focusedTextLbl : clsP.get_focusedTextLbl, get_observePrompt : clsP.get_observePrompt, get_partObserveLbl : clsP.get_partObserveLbl, get_player : clsP.get_player, get_opponents : clsP.get_opponents, get_dollScale : clsP.get_dollScale, get_gotPregens : clsP.get_gotPregens, get_thrustPointStyle : clsP.get_thrustPointStyle, get_clampedOpponentIndex : clsP.get_clampedOpponentIndex, get_currentOpponent : clsP.get_currentOpponent, get_gotCurrentOpponent : clsP.get_gotCurrentOpponent, get_canOneTwoPunch : clsP.get_canOneTwoPunch, get_advManuevers : clsP.get_advManuevers, get_advTNArr : clsP.get_advTNArr, get_advCostArr : clsP.get_advCostArr, get_advCosLblArr : clsP.get_advCosLblArr, get_browseAttackModeLabel : clsP.get_browseAttackModeLabel, get_browseAttackModeLabel2 : clsP.get_browseAttackModeLabel2, get_isBrowseHighlightLeft : clsP.get_isBrowseHighlightLeft, get_isBrowseHighlightRight : clsP.get_isBrowseHighlightRight, get_advTNs : clsP.get_advTNs, get_basicManuever : clsP.get_basicManuever, get_basicManueverTN : clsP.get_basicManueverTN, get_advSimpTNs : clsP.get_advSimpTNs, get_advCosts : clsP.get_advCosts, get_advCostStrs : clsP.get_advCostStrs, get_advSimpCosts : clsP.get_advSimpCosts, get_advSimpCostStr : clsP.get_advSimpCostStr, advHiddenAt : clsP.advHiddenAt, advDisabledAt : clsP.advDisabledAt, get_advNotAvailMask : clsP.get_advNotAvailMask, onNotAvailAdvMaskChange : clsP.onNotAvailAdvMaskChange, startGame : clsP.startGame, get_currentDollSheet : clsP.get_currentDollSheet, get_armorColorScale : clsP.get_armorColorScale, get_carriedDollShieldAssign : clsP.get_carriedDollShieldAssign, get_carriedDollShield : clsP.get_carriedDollShield, get_shieldIconStyle : clsP.get_shieldIconStyle, get_shieldLowProfiles : clsP.get_shieldLowProfiles, get_shieldHighProfiles : clsP.get_shieldHighProfiles, get_dollShieldCoverage : clsP.get_dollShieldCoverage, get_dollShieldCoverageBools : clsP.get_dollShieldCoverageBools, shieldCoveredAtDollHitLocation : clsP.shieldCoveredAtDollHitLocation, get_coverageHitLocations : clsP.get_coverageHitLocations, get_hitLocationZeroAVValues : clsP.get_hitLocationZeroAVValues, get_hitLocationZeroAVValues2 : clsP.get_hitLocationZeroAVValues2, bitMaskHitAtIndex : clsP.bitMaskHitAtIndex, get_armorDollColors : clsP.get_armorDollColors, getVisibleArmorAtDollPartIndex : clsP.getVisibleArmorAtDollPartIndex, getHitLocationArmorValues : clsP.getHitLocationArmorValues, get_hitLocationArmorValues : clsP.get_hitLocationArmorValues, get_hitLocationArmorValues2 : clsP.get_hitLocationArmorValues2, get_cpTrayCSSTransform : clsP.get_cpTrayCSSTransform, get_cpMeterAmount : clsP.get_cpMeterAmount, get_dragCPHtml : clsP.get_dragCPHtml, getRemainingMid : clsP.getRemainingMid, repeatTxt : clsP.repeatTxt, get_cpTrayStyle : clsP.get_cpTrayStyle, setupUIInteraction : clsP.setupUIInteraction, get_actingState : clsP.get_actingState, get_isDefBtnBlockVisible : clsP.get_isDefBtnBlockVisible, get_isDefBtnBlockAllowed : clsP.get_isDefBtnBlockAllowed, changed_isDefBtnBlockVisible : clsP.changed_isDefBtnBlockVisible, get_isDefBtnParryVisible : clsP.get_isDefBtnParryVisible, changed_isDefBtnParryVisible : clsP.changed_isDefBtnParryVisible, get_isDefBtnParryAllowed : clsP.get_isDefBtnParryAllowed, get_thrustAvailabilityMask : clsP.get_thrustAvailabilityMask, get_swingAvailabilityMask : clsP.get_swingAvailabilityMask, changed_swingAvailabilityMask : clsP.changed_swingAvailabilityMask, changed_thrustAvailabilityMask : clsP.changed_thrustAvailabilityMask, get_activePlayerItem : clsP.get_activePlayerItem, changed_activePlayerItem : clsP.changed_activePlayerItem, get_bgDollStyle : clsP.get_bgDollStyle, get_isDraggingCP : clsP.get_isDraggingCP, onActingStateChanged : clsP.onActingStateChanged, onOpponentsStateChange : clsP.onOpponentsStateChange, handleImageMap : clsP.handleImageMap, onResize : clsP.onResize, refreshLayout : clsP.refreshLayout};
		this.watch = { advNotAvailMask : clsP.onNotAvailAdvMaskChange, isDefBtnBlockVisible : clsP.changed_isDefBtnBlockVisible, isDefBtnParryVisible : clsP.changed_isDefBtnParryVisible, swingAvailabilityMask : clsP.changed_swingAvailabilityMask, thrustAvailabilityMask : clsP.changed_thrustAvailabilityMask, activePlayerItem : clsP.changed_activePlayerItem, actingState : clsP.onActingStateChanged, gotCurrentOpponent : clsP.onOpponentsStateChange};
	}
...
$hxClasses["troshx.sos.vue.combat.components.ZoneItemView"] = troshx_sos_vue_combat_components_ZoneItemView;
troshx_sos_vue_combat_components_ZoneItemView.__name__ = ["troshx","sos","vue","combat","components","ZoneItemView"];
troshx_sos_vue_combat_components_ZoneItemView.drawEllipse = function(context,centerX,centerY,width,height) {
	context.beginPath();
	context.moveTo(centerX,centerY - height * .5);
	context.bezierCurveTo(centerX + width * .5,centerY - height * .5,centerX + width * .5,centerY + height * .5,centerX,centerY + height / 2);
	context.bezierCurveTo(centerX - width * .5,centerY + height * .5,centerX - width * .5,centerY - height * .5,centerX,centerY - height / 2);
...
troshx_sos_vue_combat_components_ZoneItemView.__super__ = haxevx_vuex_core_VComponent;
troshx_sos_vue_combat_components_ZoneItemView.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	get_computedStyle: function() {
		var obj = { left : this.x + "px", top : this.y + "px", width : this.width + "px", height : this.height + "px", boxSizing : "border-box"};
		if(troshx_sos_vue_GlobalCanvas2D.CONTEXT != null) {
			return obj;
	...
	,get_item: function() {
		var d = this.mapData;
		return this.mapData.layoutItemList[this.index];
	}
	,get_x: function() {
		var d = this.mapData;
		var r = d.renderCount;
	...
	,get_title: function() {
		var d = this.mapData;
		return d.titleList[this.index];
	}
	,_Init: function() {
		var cls = troshx_sos_vue_combat_components_ZoneItemView;
		var clsP = cls.prototype;
		this.updated = clsP.Updated;
		this.template = this.Template();
		this.computed = { computedStyle : clsP.get_computedStyle, gotSVG : clsP.get_gotSVG, pStyle : clsP.get_pStyle, polyPoints : clsP.get_polyPoints, polyDecompPoints : clsP.get_polyDecompPoints, titleClasses : clsP.get_titleClasses, item : clsP.get_item, x : clsP.get_x, y : clsP.get_y, width : clsP.get_width, height : clsP.get_height, title : clsP.get_title};
		this.methods = { get_computedStyle : clsP.get_computedStyle, get_gotSVG : clsP.get_gotSVG, get_pStyle : clsP.get_pStyle, get_polyPoints : clsP.get_polyPoints, getPolyString : clsP.getPolyString, renderToCanvas : clsP.renderToCanvas, get_polyDecompPoints : clsP.get_polyDecompPoints, get_titleClasses : clsP.get_titleClasses, get_item : clsP.get_item, get_x : clsP.get_x, get_y : clsP.get_y, get_width : clsP.get_width, get_height : clsP.get_height, get_title : clsP.get_title};
		this.props = { debug : { "default" : false, type : Boolean}, fillColor : { "default" : "rgba(0,255,0,0.4)", type : String}, mapData : { }, strokeColor : { "default" : "#00F", type : String}, showShape : { "default" : false, type : Boolean}, strokeWidth : { "default" : 1, type : Number}, index : { type : Number}};
	}
	,__class__: troshx_sos_vue_combat_components_ZoneItemView
});
...
troshx_sos_vue_pregen_PregenSelectView.prototype = $extend(haxevx_vuex_core_VComponent.prototype,{
	Data: function() {
		return { selectedChar : -1, selectedChars : []};
	}
	,onConfirm: function() {
		if(this.multiSelect) {
...
var troshx_util__$AbsStringMap_AbsStringMap_$Impl_$ = {};
$hxClasses["troshx.util._AbsStringMap.AbsStringMap_Impl_"] = troshx_util__$AbsStringMap_AbsStringMap_$Impl_$;
troshx_util__$AbsStringMap_AbsStringMap_$Impl_$.__name__ = ["troshx","util","_AbsStringMap","AbsStringMap_Impl_"];
troshx_util__$AbsStringMap_AbsStringMap_$Impl_$._new = function() {
	var this1 = { };
	return this1;
};
troshx_util__$AbsStringMap_AbsStringMap_$Impl_$.set = function(this1,p,val) {
	this1[p] = val;
};
troshx_util__$AbsStringMap_AbsStringMap_$Impl_$.get = function(this1,p) {
	return this1[p];
};
troshx_util__$AbsStringMap_AbsStringMap_$Impl_$.exists = function(this1,p) {
	return this1[p] != null;
};
...
troshx_components_FightLink.SIDE_A = -1;
troshx_components_FightLink.SIDE_B = 1;
troshx_components_FightLink.SIDE_NONE = 0;
...
troshx_core_ManueverSpec.NO_ZONE = -1;
troshx_core_ManueverSpec.LEFT_HAND_ZONE = -2;
troshx_core_ManueverSpec.RIGHT_HAND_ZONE = -3;
...
troshx_sos_core_Manuever.ORIENTATION_DEFENSIVE = 3;
troshx_sos_core_Manuever.ORIENTATION_CAUTIOUS = 2;
troshx_sos_core_Manuever.ORIENTATION_AGGRESSIVE = 1;
...
troshx_sos_core_Manuever.ATTACK_TYPE_SHOOTING = 4;
troshx_sos_core_Manuever.ATTACK_TYPE_THROWING = 8;
troshx_sos_core_Manuever.IS_RANGED_ATTACK_TYPE = 12;
...
troshx_sos_core_Manuever.TARGET_ZONE_AUTO = 0;
troshx_sos_core_Manuever.TARGET_ZONE_WEAPON = 1;
troshx_sos_core_Manuever.TARGET_ZONE_SHIELD = 2;
troshx_sos_core_Manuever.TARGET_ZONE_OPPONENT = 4;
...
troshx_sos_core_Manuever.TAG_ADVANCED = 512;
troshx_sos_core_Manuever.TAG_INSTANT = 1024;
troshx_sos_core_Manuever.TAG_MOVEMENT = 2048;
...
troshx_sos_core_Manuever.NO_INVEST = -1;
troshx_sos_core_Manuever.DEFER_COST = -2;
...
troshx_sos_manuevers_NetItems.STUFF = ["Net (Retiarius)"];
...
troshx_sos_vue_combat_CombatViewModel.B_ATTACK_1H = 1;
troshx_sos_vue_combat_CombatViewModel.B_ATTACK_SHIELD = 2;
troshx_sos_vue_combat_CombatViewModel.B_ATTACK_2H = 3;
troshx_sos_vue_combat_CombatViewModel.B_ATTACK_UNARMED = 0;
troshx_sos_vue_combat_CombatViewModel.B_ATTACK_UNARMED_2 = 7;
troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_MELEE = ["Thrust at","Swing at"];
troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_SHOOT = ["Shoot at",null];
troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_THROW = ["Throw at",null];
troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_THROW_NET = ["Throw net at",null];
troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_SHIELD = ["Shield bash at",null];
troshx_sos_vue_combat_CombatViewModel.TARGETING_LABELS_UNARMED = ["Straight punch at","Hook punch to","Downward hook to","Upward hook to"];
troshx_sos_vue_combat_CombatViewModel.EMPTY_ARR = [];
...
troshx_sos_vue_combat_UIInteraction.HOVER_SWITCH = 262144;
troshx_sos_vue_combat_UIInteraction.MASK_HOVER = 266240;
...
troshx_sos_vue_combat_components_DollView.DOLL_BG_NO_SWINGS = { "background-image" : "url(images/dollscreen_clear3.png)"};
troshx_sos_vue_combat_components_DollView.DOLL_BG_SWINGS = { "background-image" : "url(images/dollscreen_clear2.png)"};
